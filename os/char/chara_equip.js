...
function restore_equipment(me, obj) {
    if (!obj) return false;
    if (obj.eq(me, true) == false) return false;
    me.items.remove(obj);
    me.equipment[obj.eq_type] = obj;
    if (obj.on_use) {
        obj.notify_action(me, true);
    }
    if (obj.is_shortcut) {
        me.send("{type:'removeAction',id:'" + obj.id + "'}");
    }
    if (obj.eq_type == EQUIP_TYPE.WEAPON) {
        me.weapon_changed(true);
    }
    return true;
}
...
CHARACTER.prototype.equip = function (obj) {
    ...
    if (obj.eq(this) == false) {
        if (equiped) {
            restore_equipment(this, equiped);
            this.recount();
        }
        return false;
    }
