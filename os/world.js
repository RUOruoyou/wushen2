...
WORLD = {
    ...
    disconnect: function (socket) {
        ...
        const savePromise = db.saveRequest(WORLD.RECEIVED);
    ...
    login_out: function (user) {
        ...
        const savePromise = db.saveLogs(WORLD.LOGS);
        WORLD.LOGS.length = 0;
        return savePromise;
    }
    ,
    ...
    wait_user_saves: async function (users) {
        users = users || WORLD.USERS;
        const saves = [];
        for (let user of users) {
            if (user && user.wait_save) saves.push(user.wait_save());
        }
    ...
    save: async function () {
        ...
        await this.wait_user_saves(users);
        for (var i = 0; i < users.length; i++) {
            if (users[i].serverid != WORLD.SERVERID) continue;
            roles.push(users[i].getData());
        }
        try {
            console.time('saved');
