...
ROOM.prototype.set_item = function (id, name, desc, commands) {
    ...
    if (commands) {
        for (var j = 0; j < commands.length; j++) {
            this.add_action(commands[j][0], commands[j][1], commands[j][2]);
        }
...
ROOM.prototype.set_difficulty = function (type) {
    type = type || 0;
    if (type > 0 && !this.on_set_difficulty) {
        this.apply_default_difficulty(type);
    }
    this.on_set_difficulty && this.on_set_difficulty(type);
}
ROOM.prototype.apply_default_difficulty = function (type) {
    if (this.difficulty_type) return;
    this.difficulty_type = type;
    var hp_rate = type === 2 ? 2 : 1.5;
    var prop_rate = type === 2 ? 1.35 : 1.2;
    for (var i = 0; i < this.items.length; i++) {
        var item = this.items[i];
        if (!item || item.is_player || item.master || typeof item.is_living !== "function") continue;
        if (item.max_hp > 0) {
            item.max_hp = Math.ceil(item.max_hp * hp_rate);
            item.hp = item.max_hp;
        }
        if (item.max_mp > 0) {
            item.max_mp = Math.ceil(item.max_mp * prop_rate);
            item.mp = item.max_mp;
        }
        var props = ["str", "con", "dex", "int"];
        for (var j = 0; j < props.length; j++) {
            var prop = props[j];
            if (item[prop] > 0) item[prop] = Math.ceil(item[prop] * prop_rate);
        }
        if (!item.prop) continue;
        var combat_props = ["gj", "fy", "mz", "ds", "zj"];
        for (var k = 0; k < combat_props.length; k++) {
            var name = combat_props[k];
            if (item.prop[name] > 0) item.prop[name] = Math.ceil(item.prop[name] * prop_rate);
        }
    }
}
