...
const Combat = {
    ...
    , create_skillItems: function (items) {
        ...
        for (var i = 0; i < items.length; i++) {
            ...
            html.push("<span class='pfm-cd-fill'></span>");
            html.push("<span class='pfm-label'>");
            html.push(items[i].name);
            html.push("</span>");
            html.push("<span class='pfm-cd-text'></span>");
            html.push("</span>");
            items[i].elem = $(html.join("")).appendTo(elem);
        }
        if (this.dis_pfms && this.dis_pfms.length && !this.time_handler) {
            this.ANI_PFM();
        }
    ...
    }, updatePFMCooldown: function (skill, percent, remaining) {
        if (!skill || !skill.elem) return;
        ...
        if (remaining > 0 && percent > 0) {
            skill.elem
                .addClass("cooldown")
                .css("--cooldown-percent", percent + "%")
                .css("backgroundSize", percent + "% 100%");
            skill.elem.find(".pfm-cd-text").text(Math.ceil(remaining / 1000));
        } else {
            skill.elem
                .removeClass("cooldown")
                .css("--cooldown-percent", "0%")
                .css("backgroundSize", "0% 100%");
            skill.elem.find(".pfm-cd-text").text("");
        }
    }, PFM_INTERVAL: 300
    , ANI_PFM: function () {
        var p = 0;
        ...
        for (var i = 0; i < Combat.Skills.length; i++) {
            ...
            for (var j = 0; j < Combat.dis_pfms.length; j++) {
                if (Combat.dis_pfms[j].id == skill.id && Combat.dis_pfms[j].distime) {
                    ...
                    remaining = Combat.dis_pfms[j].ani_time;
                    if (cur_per < 0) {
            ...
            Combat.updatePFMCooldown(skill, cur_per, remaining);
        }
    ...
    , CountWidth: function (d1, d2) {
        ...
        if (elem.hasClass("cooldown")) return;
