...
const COMBAT_COMMANDS = ["fight", "kill", "perform", "biwu"];

function isCombatCommand(cmd) {
    if (!cmd) return false;
    var name = cmd.trim().split(/\s+/, 1)[0];
    return COMBAT_COMMANDS.indexOf(name) >= 0;
}
...
`;
    ...
    on_mount() {
        ...
        $(".room-title>.map-icon").on("click", MAP.OpenDialog.bind(MAP));
    }
}
...
function open_map(e) {
    ...
    if (MAP.SuppressRoomClick) return;
...
function ContainerCommand(e) {
    ...
    if (cmd) {
        ...
        if (char == "_") {
            ...
            var combatCommand = isCombatCommand(cmd);
            if (combatCommand) {
                closeDialogForCombatCommand();
            ...
            if (!elem.closest('.dialog-fb').length &&
                !elem.closest('.dialog-item-subdialog').length &&
