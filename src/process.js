...
const Process = {
    ...
    clear: function () {
        ...
        Dialog.skills.skills = null;
        Dialog.skills.autoPfm = null;
        Dialog.skills.performSkills = null;
        Dialog.skills.sk_group = null;
        Combat.Skills = null;
        Combat.dis_pfms = [];
    ...
    before_click_exits: function (e) {
        ...
        if (elem.is("rect"))
            elem.attr("fill", "var(--theme-surface-2)");
        else if (elem.is("text"))
            elem.prev().attr("fill", "var(--theme-surface-2)");
    },
    click_exits: function (e) {
        ...
        if (!dir) return;
        if (elem.is("rect"))
            elem.attr("fill", "var(--theme-surface)");
        else if (elem.is("text"))
            elem.prev().attr("fill", "var(--theme-surface)");
        SendCommand("go " + dir);
    }, query_rmitem: function (id) {
        if (!this.cur_room || !this.cur_room.items) return;
        for (let item of this.cur_room.items) {
            if (item.id === id) return item;
        }
    }, isLivingRoomItem: function (item) {
        return !!(item && (item.me || item.p || item.m || item.f || item.l || item.hp > 0 || item.max_hp > 0));
    },
    item: function (item) {
        ...
        html.push("</div>");
        if (Process.isLivingRoomItem(item)) {
            Process.itemsElement.find(".item-commands").remove();
            Dialog.item.open(item);
            return;
        }
    ...
    actions: function (data) {
        ...
        if (Dialog.curItem === "item" && Dialog.item && Dialog.item.isShow) {
            if (Dialog.item.isCapturingInteraction && Dialog.item.isCapturingInteraction()) {
                Dialog.item.appendInteractionCommands(data.items);
                return;
            }
    ...
    , map: function (x) {
        ...
        if (MAP.OpenDialogAfterLoad) {
            MAP.OpenDialogAfterLoad = false;
            $(".map-panel").hide();
            Dialog.show("map");
        }
        ...
        if (data.action == "stop") MAP.StopAutoMove();
        ...
        Combat.ShowPFM(data);
        Dialog.skills.updatePerformSkills(data.skills);
