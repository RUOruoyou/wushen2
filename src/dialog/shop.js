...
export default {
    ...
    close: function () {
        ...
        if (data.money) {
            ...
            if (ms.length > 2) {
                this.footers = ["黄金", "元宝", data.mname || "活动"];
                this.act_money = ms[2];
                ...
                if (this.selected_item > 1) this.selected_item = 0;
            }
            if (data.selected !== undefined) this.selected_item = parseInt(data.selected);
            this.create_footer();
            if (!data.selllist && Dialog.jh && Dialog.jh.items) SendCommand("jh mj 0");
        }
        if (data.remove) {
        ...
        if (!data.idx) return;
        this.command = data.command || "shop";
        this.dialogTitle = data.title || "商品列表";
        this.idx = data.idx;
        ...
        if (!Dialog.isShow || Dialog.curItem !== "shop" || !this.isShow) {
            Dialog.show("shop");
        }
        this.create_footer();
        this.show_items();
    ...
    , show_items: function () {
        if (!this.isShow) return;
        const items = [this.list0, this.list1, this.list2][this.selected_item] || [];
        if (!items.length) {
            const text = this.selected_item === 2
                ? "当前没有可兑换的功绩商品。"
                : "当前没有可购买的商品。";
            this.element.find('.dialog-shop').html("<div class='empty'>" + text + "</div>");
            return;
        }
        this.create_items(items);
    ...
    show: function (data) {
        ...
        Dialog.title(this.dialogTitle || "商品列表");
        Dialog.icon("shopping-cart");
        this.isShow = true;
        this.element.appendTo(Dialog.contentElement);
        let command = this.command || "shop";
        if (!this.idx) SendCommand(command);
        else SendCommand(command + " " + this.idx);
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            str.push('cmd="_confirm ', this.command || "shop", ' ', item.id);
            if (item.limit > 0) {
