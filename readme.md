...
详细项目说明、架构、启动流程、数据存档、部署和维护建议见 [PROJECT_DOC.md](PROJECT_DOC.md)。
...
管理员后台入口：`/admin.html`。使用权限等级不低于 5 的现有账号登录后，可以创建、修改、撤回和重新发布公告，也可以向多个指定角色或全部角色发送、修改和撤回系统邮件。指定玩家使用可搜索的下拉多选框，邮件附件从服务端分类白名单中选择。内容操作由游戏进程实时落盘并同步给在线玩家，正常发布内容不需要重启服务。

后台相关 API、游戏进程管理通道或存档结构首次部署后需要重启一次，以完成模块加载。游戏内管理命令仍保留，可用于脚本热更新和兼容现有任务发信：
