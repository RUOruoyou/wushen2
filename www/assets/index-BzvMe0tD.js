...
`,Ye=`  <div class="setting dialog-custom">
        ...
        </div>`,Xe=` <div class="setting dialog-skeys"></div>`,Ze=` <div class="setting dialog-extend"></div>`,Qe=`
.setting {
...
`;function lt(e){var t=Math.max(0,e||0)/1e3;return(t%1?t.toFixed(1):t.toFixed(0))+`秒`}var ut=`
...
`,e.reward),t.push(V.renderDrops(e.drops)),t.push(`</pre>`),this.descElement.html(t.join(``)),this.select(e.index)},append_status:function(e,t){let n=t.status??[];if(n.length){e.push(`<div class="fb-actions">`);for(let r=0;r<n.length;r++){let i=n[r];i&&(i[0]===1?(e.push(`<div class="fb-action finshed">`),e.push(`<span class="action-desc">由`,i[1],`首次通过`,`</span>`),e.push(`<span class="action-name" cmd="cr2 `,t.index,` `,r,`">`,this.fb_models[r],`</span>`),e.push(`</div>`)):(e.push(`<div class="fb-action">`),e.push(`<span class="action-desc">该模式尚未完成首杀`,i[1]?`，称号奖励：`+i[1]:``,`</span>`),e.push(`<span class="action-name"  cmd="cr2 `,t.index,` `,r,`">`,this.fb_models[r],`</span>`),e.push(`</div>`)))}e.push(`</div>`)}},fb_buttons:[`进入副本`,`困难模式`,`组队进入`],append_footer:function(){let e=this.items[this.selected_index],t=[];if(e.unlock)for(let n=0;n<e.diffs.length;n++)e.diffs[n]&&t.push(`<span cmd="jh fb `,e.index,` start`,n+1,`">`,this.fb_buttons[n],`</span>`);Dialog.footerElement.find(`.item-commands`).html(t.join(``))}},U={name:`禁地`,items:null,type:`ar`,selected_index:0,select:V.select,onClickItem:V.onClickItem,append_status:H.append_status,append_actions:V.append_actions,fb_models:[`普通`,`普通`,`组队`],onDetail:function(e){var t=this.items[e.index];if(t)return t.type=`禁地`,t.desc=e.desc,t.actions=e.actions,t.status=e.status,t.reward=e.reward,t.drops=e.drops,this.showDetail(t)},update_unlock:function(e){for(let t=0;t<this.items.length;t++)this.items[t].unlock=(e&2**t)!=0},show:function(e,t){var n=[`<div class='fb-content'>`];let r=Math.max(this.items.length,10);for(var i=0;i<r;i++){var a=this.items[i];n.push(`<div class="fb-item`),a?(a.unlock||n.push(` lock`),n.push(`" index="`,i,`">`,a.name,`</div>`),a.index=i):n.push(`">&nbsp;</div>`)}n.join(`</div>`),this.listElement=e,this.descElement=t,this.listElement.html(n.join(``)),this.onClickItem(this.selected_index)},showDetail:function(e){var t=[`<pre>`];if(t.push(e.name),e.unlock?t.push(`
<hig>已解锁</hig>
`):t.push(`
<red>未解锁</red>
`),t.push(e.desc,`
...
`,e.reward),t.push(V.renderDrops(e.drops)),t.push(`</pre>`),this.descElement.html(t.join(``)),this.select(e.index)},append_footer:function(){let e=this.items[this.selected_index];e.unlock?Dialog.footerElement.find(`.item-commands`).html(`<span cmd="jh ar ${e.index} start">进入地图</span>`):Dialog.footerElement.find(`.item-commands`).empty()}};function Dt(e){if(!e||!e.active||!(e.expiresAt>0))return`未开始`;let t=Math.max(0,Math.ceil((e.expiresAt-Date.now())/1e3));return t?Math.floor(t/60)+`分`+String(t%60).padStart(2,`0`)+`秒`:`即将结算`}var Ot=[{name:`潜能`,color_name:`潜能`,grade:2},{name:`经验`,color_name:`经验`,grade:2}],kt={name:`秘境`,type:`mj`,items:null,selected_index:0,select:V.select,onClickItem:function(e){let t=this.items&&this.items[e];t&&(t.desc?this.showDetail(t):SendCommand(`jh mj `+e),this.select(e))},onDetail:function(e){let t=this.items&&this.items[e.index];t&&(t.type=`秘境`,t.desc=e.desc,t.drops=e.drops||Ot,t.status=e.status||t.status,t.actions=e.actions,this.showDetail(t))},show:function(e,t){this.listElement=e,this.descElement=t;let n=[`<div class='fb-content'>`],r=this.items||[];for(let e=0;e<r.length;e++){let t=r[e];t.index=e,n.push(`<div class="fb-item mj-item" index="`,e,`">`,t.name,`</div>`)}n.push(`</div>`),this.listElement.html(n.join(``));let i=this.items&&this.items[this.selected_index];i&&(i.desc=null),this.onClickItem(this.selected_index),this.startTimer()},showDetail:function(e){let t=e.status||{},n=[`<pre>`,`<hig>`,e.name,`</hig>
`];n.push(e.desc||``),n.push(`

<hiy>挑战状态</hiy>
`),t.active?n.push(`进行中：击杀 `,t.kills||0,`/`,t.maxKills||100,`，剩余 `,Dt(t),`。
`):n.push(`今日挑战：`,t.dailyCount||0,`/`,t.dailyLimit||1,`，归墟种：`,t.ticket||0,`枚。
`),n.push(Et(`掉落物`,e.drops)),n.push(`</pre>`),this.descElement.html(n.join(``)),this.select(e.index),this.append_footer()},append_footer:function(){let e=this.items&&this.items[this.selected_index];if(!e)return Dialog.footerElement.find(`.item-commands`).empty();let t=e.status||{},n=[];t.active?n.push(`<span cmd="mijing over">结束挑战</span>`):(t.dailyCount||0)>=(t.dailyLimit||1)?n.push(`<span class="disabled">今日挑战次数已用尽</span>`):(t.ticket||0)>0?n.push(`<span cmd="jh mj `,e.index,` start">进入秘境</span>`):n.push(`<span cmd="shop">购买归墟种</span>`),Dialog.footerElement.find(`.item-commands`).html(n.join(``))},updateStatus:function(e,t){let n=this.items&&this.items[t];n&&(n.status=e,this.selected_index===t&&this.descElement&&this.showDetail(n))},startTimer:function(){clearInterval(this.timer),this.timer=setInterval(()=>{let e=this.items&&this.items[this.selected_index];e&&e.status&&e.status.active&&this.descElement&&this.isVisible()&&this.showDetail(e)},1e3)},isVisible:function(){return this.listElement&&this.listElement.closest(`.dialog-fb`).length>0},close:function(){clearInterval(this.timer),this.timer=null}},At={fb:H,fam:V,ar:U,mj:kt},jt={init:function(){Dialog.injectStyle(Mt)},close:function(){kt.close(),this.hideSkillDetail(),this.element.remove(),this.isShow=!1},onData:function(e){if(e.close)return Dialog.isShow&&Dialog.hide();if(e.skill)return this.showSkillDetail(e.skill);if(e.drop)return this.showDropDetail(e.drop);if(e.desc){let t=At[e.t]||this.selected_item;return t&&t.onDetail(e)}if(e.t===`mj`){e.status&&kt.updateStatus(e.status,e.index||0);return}if(e.unlock!==void 0||e.unlock2!==void 0)return this.update_lock(e);if(e.refresh!==void 0&&this.isLoad){let t=At[e.t],n=t.items[e.refresh];if(n&&n.desc){n.desc=null;let e=t.items.indexOf(n);t.selected_index==e&&t.onClickItem(e)}return}!e.fbs&&!e.mijings||(V.items=(e.families||[]).map(function(e){return{name:e,unlock:!1}}),H.items=(e.fbs||[]).map(function(e){return{name:e}}),U.items=(e.areas||[]).map(function(e){return{name:e,unlock:!1}}),kt.items=(e.mijings||[]).map(function(e){return{name:e}}),this.selected_item.show(this.listElement,this.descElement))},show:function(){this.isShow||(this.element||=$(`<div class='dialog-fb'><div class='fb-left'></div><div class='fb-right'></div></div>`),this.listElement=this.element.find(`.fb-left`).off(`click.jhList`).on(`click.jhList`,`.fb-item,.fam-item,.mj-item`,this.item_click),this.descElement=this.element.find(`.fb-right`),this.element.off(`click.jhSkill`).on(`click.jhSkill`,`.jh-skill-detail-close,.jh-skill-detail-mask`,this.skill_detail_close).on(`click.jhSkill`,`.jh-skill-detail`,function(e){e.stopPropagation()}),Dialog.title(`江湖`),Dialog.icon(`home`),this.element.appendTo(Dialog.contentElement),this.isShow=!0,this.isLoad?SendCommand(`jh fb lock`):(SendCommand(`jh`),this.isLoad=!0,this.selected_item=this.footers[0]),this.create_footer())},selected_item:null,footers:[V,H,U,kt],create_footer:function(){for(var e=[],t=0;t<this.footers.length;t++){let n=this.footers[t];e.push(`<span class='footer-item`+(n==this.selected_item?` select`:``)+`' for='`+t+`'>`+this.footers[t].name+`</span>`)}e.push(`<div class="item-commands"></div>`),Dialog.footerElement.html(e.join(``))},item_click:function(){var e=$(this);if(e.is(`.selected`))return;let t=e.attr(`index`);t!==void 0&&Dialog.jh.selected_item.onClickItem(t)},skill_detail_close:function(){Dialog.jh.hideSkillDetail()},hideSkillDetail:function(){this.skillDetailElement&&=(this.skillDetailElement.remove(),null)},showSkillDetail:function(e){this.showDetailPopup(e)},showDropDetail:function(e){this.showDetailPopup(e)},showDetailPopup:function(e){this.hideSkillDetail();var t=[`<div class="jh-skill-detail-mask"><div class="jh-skill-detail grade`,e.grade||0,`">`];t.push(`<div class="jh-skill-detail-header"><span class="jh-skill-detail-title">`),t.push(e.color_name||e.name),t.push(`</span><span class="jh-skill-detail-close">关闭</span></div>`),t.push(`<pre class="jh-skill-detail-body">`),t.push(e.desc),t.push(`</pre></div></div>`),this.skillDetailElement=$(t.join(``)).appendTo(this.element)},update_lock:function(e){e.unlock>=0&&H.items&&(H.update_unlock(e.unlock),this.selected_item===H&&H.show(this.listElement,this.descElement)),e.unlock2>=0&&U.items&&(U.update_unlock(e.unlock2),this.selected_item===U&&U.show(this.listElement,this.descElement))},footerChanged:function(e){let t=this.footers[e];t!=this.selected_item&&(this.selected_item=t,Dialog.footerElement.find(`.item-commands`).empty(),t.show(this.listElement,this.descElement))}},Mt=`
...
`}showServers(){if(!Y){j(`正在获取服务器列表`),J.GetServer(e=>{if(!e||typeof e==`string`){k(`#login_pwd`,`获取服务器列表出错`);return}Y=e,this.displayServer(e),this.showServers()});return}var e=Y;if(!e||!e.length)A(`#login_panel`),k(`#login_pwd`,`获取服务器列表出错`);else{var t=S.GetUserCookie(`s`),n=t?Y[t]:e.length==1?Y[0]:null;if(n)return j(`正在连接服务器`),fe(n);A(`#slist_panel`)}}selectServer(){if(Y){var e=parseInt($(`.server-list>.select`).attr(`index`));if(!(e>=0&&e<Y.length))return C.Show({content:`你没有选择要连接的服务器。`});var t=Y[e];t||C.Show({content:`你没有选择要连接的服务器。`}),j(`正在连接服务器`),fe(t),S.SetCookie(`s`,e)}}displayServer(){if(Y){var e=location.hostname.startsWith(`127.0.0.1`)||location.hostname.startsWith(`localhost`),t=location.search.startsWith(`?test`);e&&Y.push({id:100,name:`本地测试1`,ip:`127.0.0.1`,port:31300});for(var n=[],r=`武神传说2`,i=0;i<Y.length;i++)!t&&!e&&Y[i].istest||(n.push(`<li class='role-item`),i==0&&n.push(` select`),n.push(`' index='`+i+`'>`),n.push(r),n.push(`&nbsp;&nbsp;`),n.push(Y[i].name),Y[i].isdef&&n.push(`<span class='server-badge'>&nbsp;（推荐）</span>`),n.push(`</li>`));$(`.server-list`).html(n.join(``)).on(`click`,`li`,function(){var e=$(this);e.is(`.select`)||(e.parent().find(`.select`).removeClass(`select`),e.addClass(`select`))})}}},nn=class extends q{constructor(){super(),this.initReg=!1,this.template=`
...
`}on_mount(){}open(){this.initReg||=(this.GetValidationImage(),$(`.validnum-box>.validnum-img`).on(`click`,()=>this.GetValidationImage()),!0)}regist(){var e=$(`#regist_name`).val().toLowerCase(),t=$(`#regist_pwd1`).val();if(!e)return k(`#regist_name`,`请输入用户名`);if(!/^[a-z0-9]{5,15}$/.test(e))return k(`#regist_name`,`用户名需要是5-10个英文字符`);if(!t)return k(`#regist_pwd1`,`请输入密码`);if(t.length<6||t.length>20)return k(`#regist_pwd1`,`密码长度在6到20之间`);if(t!=$(`#regist_pwd2`).val())return k(`#regist_pwd2`,`重复密码输入不一致，请重新输入`);var n=$(`#regist_val`).val();if(!n)return k(`#regist_valpanel`,`请输入图片中的验证码`);if(n.length!=4)return k(`#regist_valpanel`,`请输入图片中的四位验证码`);let r=0,i=/u(\d+)/.exec(location.pathname);i&&(r=parseInt(i[1]),r>0||(r=0)),j(`正在注册账号`),J.Regist({name:e,pwd:t,valno:n,guider:r},e=>{e.code==1?(j(`注册成功，正在获取服务器列表`),setTimeout(()=>window.location.reload(),500)):(k(`#regist_name`,e.result||`注册失败`),A($(`#regist_panel`)))})}GetValidationImage(){J.ValidationImage(function(e){$(`.validnum-box>.validnum-img`).attr(`src`,`data:image/svg+xml;base64,`+e)})}},rn=`万俟司马上官欧阳夏侯诸葛闻人东方赫连皇甫尉迟公羊澹台公冶宗政濮阳淳于单于太叔申屠公孙仲孙轩辕令狐锺离宇文长孙慕容鲜于闾丘司徒司空丌官司寇子车颛孙端木巫马公西乐正公良拓拔夹谷谷梁梁丘左丘东门西门`,an=`赵钱孙李周吴郑王冯陈楮卫蒋沈韩杨朱秦尤许何吕施张孔曹严华金魏陶姜戚谢邹喻柏水窦章云苏潘葛奚范彭郎`,on=`世舜丞主产仁仇仓仕仞任伋众伸佐佺侃侪促俟信俣修倝倡倧偿储僖僧僳儒俊伟列则刚创前剑助劭势勘参叔吏嗣士壮孺守宽宾宋宗宙宣实宰尊峙峻崇崈川州巡帅庚战才承拯操斋昌晁暠曹曾珺玮珹琒琛琩琮琸瑎玚璟璥瑜生畴矗矢石磊砂碫示社祖祚祥禅稹穆竣竦综缜绪舱舷船蚩襦轼辑轩子杰榜碧葆莱蒲天乐东钢铎铖铠铸铿锋镇键镰馗旭骏骢骥驹驾骄诚诤赐慕端征坚建弓强彦御悍擎攀旷昂晷健冀凯劻啸柴木林森朴骞寒函高魁魏鲛鲲鹰丕乒候冕勰备宪宾密封山峰弼彪彭旁日明昪昴胜汉涵汗浩涛淏清澜浦澉澎澔瀚瀛灏沧虚豪豹辅辈迈邶合部阔雄霆震韩俯颁颇频颔风飒飙飚马亮仑仝代儋利力劼勒卓哲喆展帝弛弢弩彰征律德志忠思振挺掣旲旻昊昮晋晟晸朕朗段殿泰滕炅炜煜煊炎选玄勇君稼黎利贤谊金鑫辉墨欧有友闻问`,sn=`筠柔竹霭凝晓欢霄枫芸菲寒伊亚宜姬舒影荔枝思丽秀娟英华慧巧美娜静淑惠珠翠雅芝玉萍红娥玲芬芳燕彩春菊勤珍贞莉兰凤洁梅琳素云莲真环雪荣妹霞香月莺媛艳瑞凡佳嘉琼桂娣叶璧璐娅琦晶妍茜秋珊莎锦黛青倩婷姣婉娴瑾颖露瑶怡婵雁蓓纨仪荷丹蓉眉君琴蕊薇菁梦岚苑婕馨瑗琰韵融园艺咏卿聪澜纯毓悦昭冰爽琬茗羽希宁欣飘育滢馥`;function cn(e,t){t||=parseInt(Math.random()*2)+1;var n=[];if(t==2){var r=parseInt(Math.random()*rn.length);r%2==1&&--r,n.push(rn[r++]),n.push(rn[r])}else n.push(an[parseInt(Math.random()*an.length)]);return e==0?n.push(on[parseInt(Math.random()*on.length)]):n.push(sn[parseInt(Math.random()*sn.length)]),parseInt(Math.random()*4)>1&&(e==0?n.push(on[parseInt(Math.random()*on.length)]):n.push(sn[parseInt(Math.random()*sn.length)])),n.join(``)}function ln(){for(var e=`abcdefghijklmnopqrstuvwxyz`,t=`123456789`,n=[],r=parseInt(Math.random()*3)+3,i=0;i<r;i++)i<3?n.push(e[parseInt(Math.random()*e.length)]):n.push(t[parseInt(Math.random()*t.length)]);return n.join(``)}function un(){for(var e=20,t=[],n=0;n<4;n++){var r=parseInt(Math.random()*15+1);e>=r?(n==3?r=e:e-=r,t[n]=r):(t[n]=e,e=0)}var i={};return i.str=t[0]+15,i.con=t[1]+15,i.dex=t[2]+15,i.int=t[3]+15,i}function dn(e){switch(e){case`name`:$(`#reg_name`).val(cn(+!$(`#gender_0`).is(`:checked`)));break;case`id`:$(`#reg_id`).val(ln());break;case`prop`:var t=un();$(`#reg_str`).val(t.str),$(`#reg_con`).val(t.con),$(`#reg_dex`).val(t.dex),$(`#reg_int`).val(t.int);break}}var fn=class extends q{constructor(){super(),this.template=`
...
`}select(){var e=$(`.role-list>.select`);if(e.length){var t=e.attr(`roleid`);SendCommand(`login `+t),j(`正在进入游戏`,`#role_panel`)}}addRole(){if($(`.role-list>.role-item`).length>4)return C.Show({content:`你只能最多创建五个角色`});A($(`#addrole_panel`)),dn(`name`),dn(`prop`),dn(`id`)}delete(){var e=$(`.role-list>.select`);if(e.length){var t=e.attr(`roleid`);t&&C.Show({content:`是否确认删除角色：`+e.html(),onOK:function(){SendCommand(`deleterole `+t)}})}}},pn=class extends q{constructor(){super(),this.template=`
...
`}on_mount(){}reset(){var e=$(`#reset_name`).val();if(!e)return k(`#reset_name`,`请输入用户名`);if(!/^[a-z0-9]{5,15}$/.test(e))return k(`#reset_name`,`用户名格式错误,需要5-15位字母开头的字母，数字或下划线，不区分大小写`);var t=$(`#reset_phone`).val();if(!t)return k(`#reset_phone`,`请输入你的帐号绑定的手机号码`);if(!/^1\d{10}$/.test(t))return k(`#reset_phone`,`手机号码格式错误`);var n=``,r=$(`#reset_pwd1`).val();if(!r)return k(`#reset_pwd1`,`请输入你的新密码`);var i=$(`#reset_pwd2`).val();if(!i)return k(`#reset_pwd2`,`请重复输入你的新密码`);if(i.length<6||i.length>20)return k(`#update_pwd2`,`密码长度在6到20之间`);if(i!=r)return k(`#reset_pwd2`,`两次密码输入不一致`);j(`正在修改密码`,`#reset_panel`),J.ResetPasswordByPhone(e,t,n,r,function(e){e.code?A(`#login_panel`):(k(`#reset_pwd2`,e.result??`重置失败`),A(`#reset_panel`))})}},mn=class extends q{constructor(){super(),this.template=`
...
`}open(){A(`#pwd_panel`),J.GetPhone(function(e){if(e.code!==1)return k(`#update_pwd1`,`获取绑定的手机号失败`);e.result?($(`#pwd_phone`).prop(`disabled`,!0).val(e.result),$(`#pwd_bind`).show()):($(`#pwd_phone`).prop(`disabled`,!1).val(``),$(`#pwd_bind`).hide())})}update(){$(`#pwd_panel`).find(`.input-error`).remove();var e=$(`#update_pwd1`).val(),t=$(`#update_pwd2`).val(),n=$(`#update_pwd3`).val();if(e.length<6||e.length>20)return k(`#update_pwd1`,`密码长度在6到20之间`);if(t.length<6||t.length>20)return k(`#update_pwd2`,`密码长度在6到20之间`);if(n!=t)return k(`#update_pwd3`,`两次密码输入不一致`);var r;if($(`#pwd_bind`).is(`:visible`)&&(r=$(`#pwd_no`).val(),!r||!/^\d{4}$/.test(r)))return k($(`#pwd_no`).parent(),`请输入你绑定的手机尾号`);j(`正在修改密码`,`#pwd_panel`),J.ChangePassword(e,t,r,function(e){e.code?A($(`#slist_panel`)):(k(`#update_pwd1`,e.result||`修改失败`),A(`#pwd_panel`))})}},hn=class extends q{constructor(){super(),this.template=`
...
`}bind(){A(`#bind_panel`),J.GetPhone(function(e){if($(`#phone_valid`).val(``),$(`#phone_pwd`).val(``),e.code!==1)return $(`.input-error`).html(e.result);$(`.input-error`).remove();let t=e.result;t?($(`#phone_no`).prop(`disabled`,!0).val(t),$(`#phone_valid`).parent().show().prev().show(),$(`#phone_no`).prev().html(`你已绑定手机，再次验证会取消绑定`),$(`#phone_no`).parent().next().find(`span:last()`).html(`解除绑定`)):($(`#phone_no`).prop(`disabled`,!1).val(``),$(`#phone_no`).prev().html(`你要绑定的手机(不验证，目前仅作为二级密码验证使用)`),$(`#phone_valid`).parent().hide().prev().hide(),$(`#phone_no`).parent().next().find(`span:last()`).html(`绑定`))})}check(){var e=$(`#phone_no`),t=``,n=``;if(e.is(`:disabled`)){if(n=$(`#phone_valid`).val(),!n)return k($(`#phone_valid`).parent(),`请输入你接收到的六位验证码`);if(!/^\d{4}$/.test(n))return k($(`#phone_valid`).parent(),`请输入六位数字的验证码`)}else{if(t=e.val(),!t)return k(`#phone_no`,`请输入你的帐号绑定的手机号码`);if(!/^1\d{10}$/.test(t))return k(`#phone_no`,`手机号码格式错误`)}var r=$(`#phone_pwd`).val();if(!r)return k(`#phone_pwd`,`请重复输入你的新密码`);if(r.length<6||r.length>20)return k(`#phone_pwd`,`密码长度在6到20之间`);J.BindPhone(n,t,r,function(e){e.code<1?(k($(`#phone_valid`).parent(),e.result??`绑定失败`),A(`#bind_panel`)):A(`#role_panel`)})}};window.RefreshInput=dn;var gn=class extends q{constructor(){super(),this.template=`
...
`}create(){var e={};if(e.name=$(`#reg_name`).val(),e.gender=$(`#gender_0`).is(`:checked`)?1:2,e.str=parseInt($(`#reg_str`).val()),e.con=parseInt($(`#reg_con`).val()),e.dex=parseInt($(`#reg_dex`).val()),e.int=parseInt($(`#reg_int`).val()),!/^[\u4E00-\u9FA5]{2,5}$/.test(e.name))return k(`#reg_name`,`名称格式错误，只能使用2-5位中文字符`);if(e.str<15||e.str>30)return k(`#reg_name`,`臂力需要在15-30之间`);if(e.con<15||e.con>30)return k(`#reg_name`,`根骨需要在15-30之间`);if(e.dex<15||e.dex>30)return k(`#reg_name`,`身法需要在15-30之间`);if(e.int<15||e.int>30)return k(`#reg_name`,`悟性需要在15-30之间`);if(e.str+e.con+e.dex+e.int!=80)return k(`#reg_name`,`先天属性需要在15-30之间，并且总和等于80`);j(`正在创建角色`,`#addrole_panel`),SendCommand(`createrole `+e.name+` `+e.gender+` `+e.str+` `+e.con+` `+e.dex+` `+e.int)}},_n=class extends q{constructor(){super(),this.template=`
...
`}relogin(){A($(`#login_panel`));var e=new Date;e.setTime(-1e3);for(var t=document.cookie.split(`; `),n=0;n<t.length;n++){var r=t[n].split(`=`);document.cookie=r[0]+`=''; expires=`+e.toGMTString()}}loginIn(){var e=$(`#login_name`).val().toLowerCase(),t=$(`#login_pwd`).val();if(!e)return k(`#login_name`,`请输入用户名`);if(!/^[a-z0-9]{5,15}$/.test(e))return k(`#login_name`,`用户名格式错误,需要5-15位字母开头的字母，数字或下划线，不区分大小写`);if(!t)return k(`#login_pwd`,`请输入密码`);if(t.length<6||t.length>20)return k(`#login_pwd`,`密码长度在6到20之间`);j(`正在登录`,`#login_panel`),J.Login(e,t,e=>{e.code?yn.showServers():(k(`#login_name`,e.result||`登陆失败`),A(`#login_panel`))})}},xn=new tn,Sn=new bn,Cn=new nn,X=new fn,wn=new pn,Tn=new mn,En=new hn,Dn=new gn,On=new _n;function kn(){let e=$(this).attr(`nid`);A($(`#new_panel `)),$(`#news_frame`).attr(`src`,`/news/`+e+`.html`)}var An=class extends q{constructor(){super(),this.template=`
...
`}on_mount(){if($(`.login-content`).on(`click`,`.panel_item`,e=>this.LoginCommand(e)),$(`.role-list`).on(`click`,`.role-item`,function(){$(this).parent().find(`.select`).removeClass(`select`),$(this).addClass(`select`)}),$(`.new-list>li`).on(`click`,kn),!S.GetUserCookie(`p`))return $(`#login_panel`).show();xn.showServers()}LoginCommand(e){switch($(e.currentTarget).attr(`command`)){case`ToRolePanel`:A($(`#role_panel`));break;case`ToServerPanel`:ge(),A($(`#slist_panel`));break;case`ToLogin`:A($(`#login_panel`));break;case`Forget`:A($(`#reset_panel`));break;case`CancleRegist`:A($(`#login_panel`));break;case`Down`:A($(`#download`));break;case`ToRegist`:A($(`#regist_panel`)),Cn.open();break;case`Regist`:Cn.regist();break;case`SelectServer`:xn.selectServer();break;case`LoginIn`:Sn.loginIn();break;case`ResetPwd`:wn.reset();break;case`AddRole`:X.addRole();break;case`SelectRole`:X.select();break;case`CreateRole`:Dn.create();break;case`BindPhone`:En.bind();break;case`CheckValid`:En.check();break;case`UpdatePwd`:Tn.update();break;case`ToUpdate`:Tn.open();break;case`ReLogin`:Sn.relogin();break;case`DeleteRole`:X.delete();break}}},jn=()=>$(`.content-message`),Mn={append:e=>$(`.content-message`).append(e)},Z={itemsElement:null,contentScroll:!0,message:null,channel:null,relogin(){A(`#login_panel`)},clear:function(){Dialog.pack.items=null,Dialog.skills.items=null,Dialog.skills.skills=null,Dialog.skills.autoPfm=null,Dialog.skills.performSkills=null,Dialog.skills.sk_group=null,L.Skills=null,L.dis_pfms=[],this.state(null)},init:function(){Z.itemsElement=$(`.room_items`),this.message=en.create($(`.content-message`)),this.ChannelElement=$(`.channel`),this.ChannelElement.on(`click`,Dialog.channel.show.bind(Dialog.channel)),this.channel=en.create(this.ChannelElement,4,200)},startMoveMessage:function(e){window.addEventListener(`mousemove`,Z.moveMessage),window.addEventListener(`mouseup`,Z.endMoveMessage),Z.mouseY=e.clientY},moveMessage:function(e){let t=Z.mouseY-e.clientY,n=jn(),r=n[0],i=n.height(),a=r.style.marginBottom;if(a=a?parseInt(a.replace(`px`,``)):0,a+=t,a<0)a=0;else if(a>i*.7)return;r.style.marginBottom=a+`px`,Z.mouseY=e.clientY,e.preventDefault()},endMoveMessage:function(){window.removeEventListener(`mousemove`,Z.moveMessage),window.removeEventListener(`mouseup`,Z.endMoveMessage)},regist:function(e){e.result&&(A(`#addrole_panel`),$(`#addrole_panel .input-error`).html(e.result))},emote:function(e){Z.emotes=e.items||0;for(var t=[],n=0;n<Z.emotes.length;n++)t.push(`<span>`),t.push(Z.emotes[n]),t.push(`</span>`);$(`.channel-emotes`).html(t.join(``))},deleterole:function(e){if(e.result){var t=$(`#role_panel>ul>.content>.role-list>.role-item[roleid='`+e.id+`']`);t.remove();var n=$(`#role_panel>ul>.content>.role-list>.role-item`);t.is(`.select`)&&n.length?$(n[0]).addClass(`select`):n.length||X.addRole()}else Confirm.Show({content:`<span class='input-error'>`+(e.message||`删除失败`)+`</span>`})},cross:function(e){for(var t=null,n=0;n<Y.length;n++)Y[n].ID==e.sid&&(t=Y[n]);t&&(E.ChangeServer=!0,E.Close(),Dialog.pack.items=null,e.cross_type==`duizhan`&&(Dialog.skills.items=null,Dialog.skills.isShow=!1),console.log(`重新连接到`,t.Name),e.pid||Z.die({relive:!0}),fe(t,e.pid))},roles:function(e){var t=e.roles;if(!t.length)X.addRole();else{A(`#role_panel`);for(var n=[],r=0;r<t.length;r++)n.push(`<li class='role-item`),r==0&&n.push(` select`),n.push(`' roleid='`+t[r].id+`'>`),n.push(t[r].title),n.push(`&nbsp;&nbsp;`),n.push(t[r].name),n.push(`</li>`);$(`.role-list`).html(n.join(``))}},loginerror:function(e){$(`.container`).hide(),$(`.login-content`).show(),j(`<strong>登陆失败：</strong>`+e.msg)},login:function(e){Z.player||A(`.container`),Z.player=e.id,Z.level=e.level,F.load(e.setting),me()},levelup:function(e){Z.level=e.level},selectItem:function(e){if($(e.target).is(`.status-item`)){var t=e.target.getAttribute(`sid`);let n=$(e.target).closest(`.room-item`).attr(`itemid`);return t?n===Z.player?D(`status `+t):D(`status `+t+` `+n):void 0}var n=$(this).attr(`itemid`);if(console.log(n),n){if(n==Z.player){D(`select `+n);return}D(`select `+n)}},countwidth:function(e,t){var n=e*100/t;return n<0&&(n=0),n>100&&(n=100),n},itemremove:function(e){var t=L.STATUS[e.id];if(t){for(var n in t.items)clearInterval(t.items[n].handler);var r=t.elem.parent();r.next().is(`.item-commands`)&&r.next().remove(),r.remove(),delete L.STATUS[e.id]}Z.cur_room.items.RemoveAt(t=>t.id===e.id)},itemadd:function(e){if(!(F.off_plist&&e.p&&e.id!=Z.player)){var t=e,n=F.item_firstme&&t.id==Z.player?$(Z.create_roomitem(t)).prependTo(Z.itemsElement):$(Z.create_roomitem(t)).appendTo(Z.itemsElement);L.STATUS[e.id]&&Z.itemremove(e),L.AppendStatusItem(t.id,n.find(`.item-status-bar`),t.status),Z.cur_room.items.push(t)}},items:function(e){Z.itemsElement.empty(),L.STATUS={};for(var t=0;t<e.items.length;t++){var n=e.items[t];if(n&&(n.player=n.p,n.m&&(n.type=`师父`,n.master=1),n.f&&(n.type=`随从`,n.follower=1),n.l&&(n.type=`商人`,n.trader=1),!(F.off_plist&&n.p&&n.id!=Z.player))){var r=F.item_firstme&&n.id==Z.player?$(Z.create_roomitem(n)).prependTo(Z.itemsElement):$(Z.create_roomitem(n)).appendTo(Z.itemsElement);L.AppendStatusItem(n.id,r.find(`.item-status-bar`),n.status)}}Z.cur_room||={},Z.cur_room.items=e.items},get_hpnum:function(e,t){var n=e/t;return n>.8?`<hiy>`+e+`</hiy>`:n>.5?`<yel>`+e+`</yel>`:n>.2?`<red>`+e+`</red>`:`<hir>`+e+`</hir>`},create_roomitem:function(e){var t=[];return t.push(`<div class='room-item' itemid='`+e.id+`'>`),e.max_hp&&(t.push(`<div class="item-status"`),(!L.IsShow||F.off_hp)&&t.push(` style="display:none;"`),t.push(`>`),t.push(`<div class="progress hp"><div class="progress-bar" max="`+e.max_hp+`"  style="width:`+Z.countwidth(e.hp,e.max_hp)+`%"></div></div>`),t.push(`<div class="progress mp"><div class="progress-bar" max="`+e.max_mp+`"   style="width:`+Z.countwidth(e.mp,e.max_mp)+`%"></div></div>`),t.push(`</div>`)),t.push(`<span class='item-status-bar'>`),t.push(`</span>`),t.push(`<span class='item-name'>`),t.push(e.name),F.show_hpnum&&e.max_hp&&t.push(`<span class="progress-num">[`+this.get_hpnum(e.hp,e.max_hp)+`<nor>/</nor><hiy>`+e.max_hp+`</hiy>]</span>`),t.push(`</span>`),t.push(`</div>`),t.join(``)},room:function(e){$(`.room_items`).html(``),$(`.room-name`).html(e.name),$(`.room_desc`).html(e.desc),Z.room_name=e.name,F.keep_msg?F.keep_msg&&Q(`你来到了`+e.name+`。`):Z.message.clear(),Z.room_path!=e.path&&(F.show_roomitem&&Z.searchItems(e),L.ShowRoomCommands(e),Z.room_path=e.path,Z.cur_room=e,M.SetRoom(e))},roomHiddenItemsReg:/<\w{3}\scmd=['"](.+?)['"]>(.+?)<\/\w{3}>/g,searchItems:function(e){for(var t=null,n=e.desc;(t=this.roomHiddenItemsReg.exec(n))!==null;)e.commands.push({cmd:t[1],name:t[2]})},exits:function(e){var t=e?e.items:Z.room_exits;if(t)if(Z.room_exits=t,M.SetExits(t),F.exits_dir==1){for(var n=[`这里明显的出口有：`],r=[],i=0;i<M.DIRS.length;i++)t[M.DIRS[i]]&&r.push(M.DIRS[i]);for(var i=0;i<r.length;i++)i>0&&n.push(i==r.length-1?` 和 `:`、`),n.push(`<span class='exits-item' dir='`+r[i]+`'>`+r[i]+`</span>`);r.length?$(`.room_exits`).html(n.join(``)):$(`.room_exits`).html(`<HIK>这里没有明显的出口。<HIK>`)}else $(`.room_exits`).html(M.CreateExitsMap(t,$(`.container`).width(),Z.room_name))},before_click_exits:function(e){var t=$(e.target);t.attr(`dir`)&&(t.is(`rect`)?t.attr(`fill`,`var(--theme-surface-2)`):t.is(`text`)&&t.prev().attr(`fill`,`var(--theme-surface-2)`))},click_exits:function(e){var t=$(e.target),n=t.attr(`dir`);n&&(t.is(`rect`)?t.attr(`fill`,`var(--theme-surface)`):t.is(`text`)&&t.prev().attr(`fill`,`var(--theme-surface)`),D(`go `+n))},query_rmitem:function(e){if(!(!this.cur_room||!this.cur_room.items)){for(let t of this.cur_room.items)if(t.id===e)return t}},isLivingRoomItem:function(e){return!!(e&&(e.me||e.p||e.m||e.f||e.l||e.hp>0||e.max_hp>0))},item:function(e){e.commands=e.commands??[];let t=Z.query_rmitem(e.id);t&&(e=Object.assign(e,t)),N.LAST_OBJ=e,Dialog.extend.append(e.commands,`item`,e);for(var n=[`<div class='item-commands'>`],r=0;r<e.commands.length;r++)n.push(`<span cmd='`+e.commands[r].cmd+`'>`),n.push(e.commands[r].name),n.push(`</span>`);if(n.push(`</div>`),Z.isLivingRoomItem(e)){Z.itemsElement.find(`.item-commands`).remove(),Dialog.item.open(e);return}if(Q(e.desc),F.show_command&&L.STATUS[e.id]){Z.itemsElement.find(`.item-commands`).remove();var i=L.STATUS[e.id].elem.parent();return $(n.join(``)).insertAfter(i),Z.message.scroll2end()}Q(n.join(``))},actions:function(e){L.ShowActions(e)},cmds:function(e){if(e.items){if(Dialog.curItem===`item`&&Dialog.item&&Dialog.item.isShow){if(Dialog.item.isCapturingInteraction&&Dialog.item.isCapturingInteraction()){Dialog.item.appendInteractionCommands(e.items);return}Dialog.item.appendCommands(e.items);return}if(Dialog.isShow){w.Show(e.items);return}var t=[`<div class='item-commands'>`];e.items.length||(e.items=[e.items]);for(var n=0;n<e.items.length;n++)t.push(`<span cmd='`+e.items[n].cmd+`'>`),t.push(e.items[n].name),t.push(`</span>`);t.push(`</div>`),Q(t.join(``))}},map:function(e){M.SetMapBuffer(e.map,e.path),M.ShowMap(e.map,e.path),M.OpenDialogAfterLoad&&(M.OpenDialogAfterLoad=!1,$(`.map-panel`).hide(),Dialog.show(`map`))},updatemap:function(e){M.UpdateMap(e.map,e)},path:function(e){e.message&&Q(e.message),e.action==`stop`&&M.StopAutoMove()},dialog:function(e){Dialog.show(e.dialog,e)},sc:function(e){L.StatusChanged(e)},perform:function(e){L.ShowPFM(e),Dialog.skills.updatePerformSkills(e.skills)},disobj:function(e){L.DisObj(e)},changepfm:function(e){L.ChangeDistime(e)},clearDistime:function(e){L.ClearDistime(e)},pay:function(e){if(e.pay===3){Q(`<yel>请打开微信扫描二维码支付：</yel>
`);let t=$(`<div style="width:100%;text-align:center;"><img style="border:solid 2px #808088" src="`+e.url+`"/></div>`);t.children(0).on(`load`,function(){Q(``)}),Mn.append(t)}else window.location.href=e.url},dispfm:function(e){L.On_Perform(e)},status:function(e){L.StatusItemChanged(e)},combat:function(e){e.start&&(F.auto_showcombat==1&&!L.IsShow&&L.Show(),F.auto_hideroom==1&&(F.hide_roomdesc||$(`.room_desc`).hide())),e.end&&F.auto_hideroom==1&&(F.hide_roomdesc||$(`.room_desc`).show())},state:function(e){if(e&&e.state){var t=[`<span class='title'>`+e.state+`</span>`];if(e.commands)for(var n=0;n<e.commands.length;n++)t.push(`<span class='item-command' cmd='`+e.commands[n].cmd+`'>`),t.push(e.commands[n].name),t.push(`</span>`);$(`.state-bar`).html(t.join(``)).css(`visibility`,`visible`),e.no_stop?$(`.state-tool`).hide():$(`.state-tool`).show(),Z.states=e.desc,Z.timer&&clearInterval(Z.timer),Z.states&&Z.states.length&&(typeof Z.states==`string`&&(Z.states=[Z.states]),Z.timer=setInterval(Z.updatestate,e.interval||5e3))}else $(`.state-bar`).empty().css(`visibility`,`hidden`),$(`.state-tool`).hide(),clearInterval(Z.timer)},updatestate:function(){if(Z.states&&pe()){var e=Z.states.length;Q(Z.states[parseInt(Math.random()*e)])}},die:function(e){if(e.relive)return Z.state({});Z.state({state:`<hiw>你已经死亡：</hiw>`,no_stop:!0,desc:[`<blk>一股阴冷的气息包围着你。</blk>`,`<blu>朦胧中你好像听到有人在喊：过来吧，过来吧！</blu>`],commands:e.commands,interval:12e3})},warn:function(e){Warn.Show(e)},msg:function(e){var t=Dialog.channel.createElement(e,!F.no_spmsg);t&&(F.no_spmsg?Q(t):(Z.channel.push(t),Z.channel.scroll2end()))},addAction:function(e){L.AddObj(e.id,e.name,e.distime)},removeAction:function(e){L.DisObj({id:e.id,remove:!0})}};function Q(e){Z.message.push(e),Z.message.scroll2end()}var Nn=!1,Pn=[`fight`,`kill`,`perform`,`biwu`];function Fn(e){if(!e)return!1;var t=e.trim().split(/\s+/,1)[0];return Pn.indexOf(t)>=0}function In(){K.isShow&&K.hide(),C.Close(),w.Close()}var Ln=class extends q{constructor(){super(),this.template=`
...
`}on_mount(){$(`.container`).on(`click`,qn),$(`.channel-box`).on(`click`,`span`,Gn),$(`.combat-commands`).on(`click`,`.pfm-item`,L.Perform).on(`wheel`,L.Scroll),$(`.room-commands`).on(`wheel`,L.Scroll),$(`.sender-box`).on(`keyup`,Un),$(`.room_items`).on(`click`,`.room-item`,Z.selectItem),$(`.bottom-bar`).on(`click`,`.tool-item,.state-bar,.item-command`,Bn),$(`.map-panel`).on(`click`,zn),$(`.sender-btn`).on(`click`,Wn),$(`.room_exits`).on(`pointerdown`,Z.before_click_exits).on(`pointerup`,Z.click_exits),$(`.room-title>.map-icon`).on(`click`,M.OpenDialog.bind(M))}},Rn=0;function zn(e){if(!$(e.currentTarget).closest(`.dialog-map`).length&&!M.SuppressRoomClick){if(Rn||=0,Date.now()-Rn>500){Rn=Date.now();return}M.OpenDialog()}}function Bn(e){var t=$(this).attr(`command`);return t?Vn(t):(t=$(this).attr(`cmd`),t&&SendCommand(t),!1)}function Vn(e){switch(e){case`showtool`:vt();break;case`showchat`:return Hn();case`showcombat`:return L.Show();case`stopstate`:if(K.extend.is_record)return K.extend.stop_record();SendCommand(`state stop`);break;case`stateinfo`:SendCommand(`state info`);break;default:K.show(e);break}return!1}function Hn(){var e=$(`.chat-panel`).toggleClass(`hide`);return e.is(`.hide`)?Nn=!1:(Nn=!0,e.find(`input`).val(``).focus()),!1}function Un(e){e.keyCode==13&&Wn()}function Wn(){var e=$(`.sender-box`).val();if(e){if(e.length>100)return ReceiveMessage(`<hir>你输入的内容太多了。</hir>`);var t=$(`.channel-box`).attr(`channel`);$(`.sender-box`).val(``).focus(),SendCommand(t+` `+e)}}function Gn(){var e=$(this),t=e.attr(`channel`);if(t==`emote`)return Kn();if(!e.is(`.selected`)){var n=e.parent();return n.children().removeClass(`selected`),e.addClass(`selected`),n.attr(`channel`,t),$(`.sender-box`).focus(),!1}}function Kn(){var e=$(`.channel-emotes`);e.is(`.hide`)?(e.removeClass(`hide`),Z.emtoes||(SendCommand(`emote`),Z.emtoes=[],$(`.sender-box`).blur(),e.on(`click`,`span`,function(){var e=$(this).html();$(`.sender-box`).val(`*`+e).focus(),$(`.channel-emotes`).addClass(`hide`)}))):$(`.channel-emotes`).addClass(`hide`)}function qn(e){var t=$(e.target);if(K.isShow&&t.is(`.container.dialog-open`))return K.hide(),C.Close(),w.Close(),!1;var n=t.attr(`cmd`);if(n||=t.parent().attr(`cmd`),n){let e=n[0];if(e==`_`){var r=n.split(` `);switch(r[0]){case`_confirm`:C.Process(r);break;case`_setting`:F.save(r[1],r[2]);break;case`_trade`:K.trade.confirm(r[1]);break;case`_close`:oe.Close(t);break;case`_hide`:break;case`_closed`:K.hide();case`_party`:K.party.command(r[1]);break}}else e===`#`?N.run(n):(Fn(n)?In():K.curItem===`item`&&K.item&&K.item.isShow&&t.closest(`.dialog-item-dialog`).length>0?K.item.captureNextPrompt():K.isShow&&w.captureNext(),SendCommand(n),!t.closest(`.dialog-fb`).length&&!t.closest(`.dialog-item-subdialog`).length&&t.closest(`.dialog-content`).length>0&&t.closest(`.item-commands`).remove());return!1}else Nn&&(t.closest(`.chat-panel`).length||($(`.chat-panel`).addClass(`hide`),Nn=!1));C.Close(),w.Close()}window.HandlerMenuCommand=Vn;var Jn=new An,Yn=new Ln,Xn=class extends q{constructor(){super(),this.template=`${Jn.render()}\n${Yn.render()}`}on_mount(){Jn.on_mount(),Yn.on_mount()}};globalThis.$=e,globalThis.SendCommand=D,globalThis.ReceiveMessage=O,globalThis.Confirm=C,globalThis.Warn=oe,globalThis.CmdPrompt=w,globalThis.Process=Z,globalThis.Dialog=K;function Zn(){let e=new Xn;e.onCompile(),e.mount(document.body),Z.init()}e(Zn);