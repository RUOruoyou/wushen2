...
`,$e=`  <div class="setting dialog-custom">
        ...
        </div>`,et=` <div class="setting dialog-skeys"></div>`,tt=` <div class="setting dialog-extend"></div>`,nt=`
.setting {
...
`;function pt(e){var t=Math.max(0,e||0)/1e3;return(t%1?t.toFixed(1):t.toFixed(0))+`秒`}var mt=`
...
`;function G(e){return String(e??``).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`).replace(/'/g,`&#39;`)}function an(e){e=Math.max(0,parseInt(e)||0);let t=[];return e>=1e4&&(t.push(Math.floor(e/1e4)+` 两黄金`),e%=1e4),e>=100&&(t.push(Math.floor(e/100)+` 两白银`),e%=100),e&&t.push(e+` 个铜板`),t.join(` `)||`0 个铜板`}function on(e,t){let n=new Set([`gj_per`,`mz_per`,`fy_per`,`ds_per`,`zj_per`,`hp_per`,`lianxi_per`,`dazuo_per`,`study_per`,`add_sh_per`,`add_bjsh_per`,`diff_bj`,`diff_sh_per`,`expend_mp_per`,`diff_busy_per`,`gjsd_per`,`diff_fy_per`,`releasetime_per`,`bj_per`,`busy_per`,`diff_downside_per`,`distime_per`]),r=new Set([`gjsd`,`busy`,`diff_busy`,`distime`,`releasetime`]);return n.has(e)?`+`+t+`%`:r.has(e)?`+`+t/1e3+` 秒`:`+`+t}function sn(e){return G(String(e.name||``).replace(/<[^>]+>/g,``))+`×`+e.count}function cn(e){return String(e??``).replace(/<[^>]+>/g,``)}var ln={state:null,previewData:null,pickerData:null,error:``,message:``,pending:!1,init:function(){Dialog.injectStyle(un),this.element=$(`<div class='custom-equipment'></div>`),this.element.on(`click`,`.custom-refresh`,this.refresh.bind(this)),this.element.on(`click`,`.custom-wash`,this.washNow.bind(this)),this.element.on(`click`,`.custom-open-add`,this.openAddPicker.bind(this)),this.element.on(`click`,`.custom-upgrade`,this.previewUpgrade.bind(this)),this.element.on(`click`,`.custom-open-replace`,this.openReplacePicker.bind(this)),this.element.on(`click`,`.custom-open-ability`,this.openAbilityPicker.bind(this)),this.element.on(`click`,`.custom-picker-item`,this.onPickerSelect.bind(this)),this.element.on(`click`,`.custom-picker-cancel`,this.closePicker.bind(this)),this.element.on(`click`,`.custom-rename`,this.previewRename.bind(this)),this.element.on(`click`,`.custom-confirm`,this.commit.bind(this)),this.element.on(`click`,`.custom-preview-cancel`,this.cancelPreview.bind(this))},ensureOpen:function(){(!Dialog.isShow||Dialog.curItem!==`customEquipment`)&&Dialog.select(`customEquipment`),this.show()},show:function(){Dialog.isShow||Dialog.init(),Dialog.element.addClass(`dialog-custom-equipment`),this.element.detach(),Dialog.contentElement.empty().append(this.element),Dialog.footer(``),Dialog.icon(`wrench`),this.isShow=!0,this.render()},hide:function(){Dialog.element&&Dialog.element.removeClass(`dialog-custom-equipment`),this.element&&this.element.detach(),this.isShow=!1},close:function(){this.hide(),this.previewData=null,this.pickerData=null,this.pending=!1},onData:function(e){if(this.pending=!1,e.phase===`error`){this.error=e.message||`自制装备操作失败。`,this.message=``,this.ensureOpen();return}if(this.error=``,e.phase===`preview`){this.previewData=e,this.ensureOpen();return}(e.phase===`state`||e.phase===`result`)&&(this.state=e,this.previewData=null,this.pickerData=null,this.message=e.message||``,this.ensureOpen())},sendPreview:function(e,t){if(this.pending||!this.state)return;this.pending=!0,this.error=``,this.message=``,this.render();let n=t&&t.length?` `+t.join(` `):``;SendCommand(`zizhi preview `+this.state.itemId+` `+e+n)},refresh:function(){this.pending||!this.state||(this.pending=!0,this.render(),SendCommand(`zizhi open `+this.state.itemId))},washNow:function(){this.pending||!this.state||(this.pending=!0,this.error=``,this.message=``,this.render(),SendCommand(`zizhi wash `+this.state.itemId))},openAddPicker:function(){if(this.pending||!this.state)return;let e=[];for(let t of this.state.categories||[])t.remaining>0&&t.available.length&&e.push({name:t.name,hint:`已用 `+t.used+`/`+t.limit,options:t.available});e.length&&(this.pickerData={mode:`add`,title:`添加词条`,subtitle:`选择要添加的词条属性（消耗 1 块对应属性晶石）：`,groups:e},this.render())},openReplacePicker:function(e){if(this.pending||!this.state)return;let t=$(e.currentTarget).attr(`data-key`),n=null;for(let e of this.state.categories||[]){let r=(e.affixes||[]).find(e=>e.key===t);if(r){n=r;break}}!n||!n.replacements||!n.replacements.length||(this.pickerData={mode:`replace`,oldKey:n.key,title:`替换【`+n.name+`】`,subtitle:`选择新的词条属性（消耗 100 两黄金，返还旧词条全部晶石）：`,groups:[{name:``,hint:``,options:n.replacements}]},this.render())},openAbilityPicker:function(){if(this.pending||!this.state)return;let e=this.state.ability||{};!e.unlocked||!e.options||!e.options.length||(this.pickerData={mode:`ability`,title:`设置能力词条`,subtitle:`选择要强化的已学武学，穿戴时有效等级 +1（消耗 1 个元晶）：`,groups:[{name:``,hint:``,options:e.options.map(e=>({key:e.id,name:e.name+`强化`,ability:!0}))}]},this.render())},onPickerSelect:function(e){let t=$(e.currentTarget);if(t.hasClass(`disabled`)||this.pending||!this.pickerData)return;let n=t.attr(`data-key`);if(!n)return;let r=this.pickerData;this.pickerData=null,r.mode===`add`?this.sendPreview(`add`,[n]):r.mode===`replace`?this.sendPreview(`replace`,[r.oldKey,n]):r.mode===`ability`&&this.sendPreview(`ability`,[n])},closePicker:function(){this.pending||(this.pickerData=null,this.render())},previewUpgrade:function(e){this.sendPreview(`upgrade`,[$(e.currentTarget).attr(`data-key`)])},previewRename:function(){let e=String(this.element.find(`.custom-rename-input`).val()||``).trim();e&&this.sendPreview(`rename`,[e])},commit:function(){this.pending||!this.previewData||!this.previewData.token||(this.pending=!0,this.render(),SendCommand(`zizhi commit `+this.previewData.token))},cancelPreview:function(){this.pending||(this.previewData=null,this.render())},render:function(){if(!this.element)return;if(!this.state){this.element.html(`<div class='custom-empty'>`+G(this.error||`正在读取装备状态……`)+`</div>`);return}let e=this.state;Dialog.title((e.plainName||`自制装备`)+` - 重铸`);let t=this.pending||e.locked||e.socketed||e.washCount>=e.maxWashCount,n=[];if(n.push(`<div class='custom-head'>`,`<div class='custom-head-main'><span class='custom-name'>`,e.itemName,`<i>`,G(e.partName),`</i></span>`,`<button type='button' class='custom-icon-button custom-refresh' title='刷新' aria-label='刷新'`,this.pending?` disabled`:``,`><span class='glyphicon glyphicon-refresh'></span></button></div>`,`<div class='custom-meta'>洗练 <b>`,e.washCount,`/`,e.maxWashCount,`</b> · 词条上限 <b>`,e.levelLimit,` 级</b> · 固定`,G(e.fixed.name),` <b>+`,e.fixed.value,`</b></div>`,`<div class='custom-res'><button type='button' class='custom-button custom-primary custom-wash'`,t?` disabled`:``,` title='消耗 1 个元晶，立即洗练'>洗练</button>`,`<span>元晶 `,e.resources.yuanjing,`</span>`,`<span>黄金 `,G(an(e.resources.money)),`</span>`,`<span>改名符 `,e.resources.rename,`</span></div>`,`</div>`),e.locked&&n.push(`<div class='custom-alert'>装备已锁定，解锁后才能修改。</div>`),e.socketed&&n.push(`<div class='custom-alert'>装备已镶嵌宝石，清理宝石后才能修改。</div>`),this.error&&n.push(`<div class='custom-alert custom-error'>`,G(cn(this.error)),`</div>`),this.message&&n.push(`<div class='custom-success'>`,G(cn(this.message)),`</div>`),this.pending&&n.push(`<div class='custom-pending'>正在核对装备和材料……</div>`),n.push(`<div class='custom-scroll'>`),n.push(this.renderAffixes(e)),n.push(this.renderRename(e)),n.push(`</div>`),this.pickerData&&n.push(this.renderPicker(this.pickerData)),this.previewData&&n.push(this.renderPreview(this.previewData)),this.element.html(n.join(``)),e.focus===`rename`){let t=this.element.find(`.custom-scroll`)[0];t&&(t.scrollTop=t.scrollHeight),e.focus=``}},renderAffixes:function(e){let t=this.pending||e.locked||e.socketed,n=e.categories||[],r=0,i=0,a=!1;for(let e of n)r+=e.used,i+=e.limit,e.remaining>0&&e.available.length&&(a=!0);let o=[];o.push(`<section class='custom-affixes'><div class='custom-affixes-head'><h3>词条 <span>`,r,`/`,i,`</span></h3>`,`<button type='button' class='custom-button custom-open-add'`,t||!a?` disabled`:``,`>+ 添加词条</button></div>`);let s=!0;for(let e of n)if(e.affixes.length){s=!1,o.push(`<div class='custom-group'><div class='custom-group-head'>`,G(e.name),`<span>`,e.used,`/`,e.limit,`</span></div>`);for(let n of e.affixes)o.push(this.renderAffixRow(n,t));o.push(`</div>`)}return s&&o.push(`<div class='custom-none'>暂无词条，点击右上方添加。</div>`),o.push(this.renderAbility(e)),o.push(`</section>`),o.join(``)},renderAffixRow:function(e,t){let n=e.material||{},r=e.canUpgrade?`升至 `+(e.level+1)+` 级，消耗 `+e.nextCost+` 块`+n.name+`（持有 `+n.count+`）`:`已达当前洗练次数的升级上限`,i=[];return i.push(`<div class='custom-affix-row'><div class='custom-affix-info'><strong>`,G(e.name),`</strong><em>`,G(on(e.key,e.value)),`</em><span>`,e.level,`/`,e.levelLimit,` 级`,e.legacy?` · 旧`:``,`</span></div><div class='custom-affix-actions'>`),i.push(`<button type='button' class='custom-button custom-upgrade' data-key='`,e.key,`' title='`,G(r),`'`,t||!e.canUpgrade?` disabled`:``,`>升级</button>`),e.replacements&&e.replacements.length&&i.push(`<button type='button' class='custom-button custom-open-replace' data-key='`,e.key,`' title='消耗 100 两黄金，返还旧词条全部晶石'`,t?` disabled`:``,`>替换</button>`),i.push(`</div></div>`),i.join(``)},renderAbility:function(e){let t=e.ability||{};if(!t.unlocked)return`<div class='custom-ability-tip'>能力词条：洗练 25 次后解锁</div>`;let n=this.pending||e.locked||e.socketed,r=[];return r.push(`<div class='custom-group custom-group-ability'><div class='custom-group-head'>能力词条<span>`,t.skillId?`1/1`:`0/1`,`</span></div>`),t.skillId?r.push(`<div class='custom-affix-row'><div class='custom-affix-info'><strong>`,G(t.name),`强化</strong><em>有效等级 +1</em></div><div class='custom-affix-actions'>`,`<button type='button' class='custom-button custom-open-ability'`,n?` disabled`:``,`>更换</button></div></div>`):t.options&&t.options.length?r.push(`<div class='custom-affix-row'><div class='custom-affix-info'><strong>未设置</strong>`,`<em>穿戴时武学有效等级 +1</em></div><div class='custom-affix-actions'>`,`<button type='button' class='custom-button custom-open-ability'`,n?` disabled`:``,`>设置</button></div></div>`):r.push(`<div class='custom-none'>当前没有与该部位匹配的已学武学</div>`),r.push(`</div>`),r.join(``)},renderRename:function(e){let t=this.pending||e.locked||e.socketed||e.resources.rename<1;return`<div class='custom-rename-row'><span>改名</span><input class='custom-input custom-rename-input' maxlength='5' value='`+G(e.plainName)+`'`+(t?` disabled`:``)+`><button type='button' class='custom-button custom-rename'`+(t?` disabled`:``)+`>确定</button></div>`},renderPicker:function(e){let t=[];t.push(`<div class='custom-picker-mask'><div class='custom-picker-dialog'>`),t.push(`<div class='custom-picker-header'><h3>`,G(e.title),`</h3>`),t.push(`<button type='button' class='custom-icon-button custom-picker-cancel' aria-label='关闭'>✕</button></div>`),t.push(`<div class='custom-picker-subtitle'>`,G(e.subtitle||``),`</div>`),t.push(`<div class='custom-picker-list'>`);for(let n of e.groups||[]){n.name&&t.push(`<div class='custom-picker-group'>`,G(n.name),n.hint?`<span>`+G(n.hint)+`</span>`:``,`</div>`);for(let e of n.options)t.push(this.renderPickerItem(e))}return t.push(`</div>`),t.push(`<div class='custom-picker-footer'><button type='button' class='custom-button custom-picker-cancel'>取消</button></div>`),t.push(`</div></div>`),t.join(``)},renderPickerItem:function(e){if(e.ability)return`<div class='custom-picker-item' data-key='`+e.key+`'><div class='custom-picker-item-main'><div class='custom-picker-item-title'><strong>`+G(e.name)+`</strong></div><div class='custom-picker-item-mat'>穿戴时该武学有效等级 +1 · 消耗 1 个元晶</div></div><button type='button' class='custom-button custom-primary'>选择</button></div>`;let t=e.material&&e.material.count||0,n=t>=1;return`<div class='custom-picker-item`+(n?``:` disabled`)+`' data-key='`+e.key+`'><div class='custom-picker-item-main'><div class='custom-picker-item-title'><strong>`+G(e.name)+`</strong><span class='custom-picker-item-val'>`+G(on(e.key,e.value))+`</span></div><div class='custom-picker-item-mat'>材料：`+G(e.material?e.material.name:`晶石`)+`（持有 <span class='`+(n?`text-success`:`text-danger`)+`'>`+t+`</span> 块）</div></div><button type='button' class='custom-button`+(n?` custom-primary`:``)+`'`+(n?``:` disabled`)+`>选择</button></div>`},renderPreview:function(e){let t=(e.costs||[]).map(sn);e.money&&t.push(G(an(e.money)));let n=(e.refunds||[]).map(sn);return`<div class='custom-preview'><div class='custom-preview-dialog'><h3>`+G(e.operationName||`确认操作`)+`</h3><p>`+G(e.summary)+`</p><dl><dt>消耗</dt><dd>`+(t.join(`、`)||`无`)+`</dd>`+(n.length?`<dt>返还</dt><dd>`+n.join(`、`)+`</dd>`:``)+`</dl><div class='custom-preview-actions'><button type='button' class='custom-button custom-preview-cancel'`+(this.pending?` disabled`:``)+`>取消</button><button type='button' class='custom-button custom-primary custom-confirm'`+(this.pending?` disabled`:``)+`>确认</button></div></div></div>`}},un=`
...
`}on_mount(){}open(){this.initReg||=(this.GetValidationImage(),$(`.validnum-box>.validnum-img`).on(`click`,()=>this.GetValidationImage()),!0)}regist(){var e=$(`#regist_name`).val().toLowerCase(),t=$(`#regist_pwd1`).val();if(!e)return O(`#regist_name`,`请输入用户名`);if(!/^[a-z0-9]{5,15}$/.test(e))return O(`#regist_name`,`用户名需要是5-10个英文字符`);if(!t)return O(`#regist_pwd1`,`请输入密码`);if(t.length<6||t.length>20)return O(`#regist_pwd1`,`密码长度在6到20之间`);if(t!=$(`#regist_pwd2`).val())return O(`#regist_pwd2`,`重复密码输入不一致，请重新输入`);var n=$(`#regist_val`).val();if(!n)return O(`#regist_valpanel`,`请输入图片中的验证码`);if(n.length!=4)return O(`#regist_valpanel`,`请输入图片中的四位验证码`);let r=0,i=/u(\d+)/.exec(location.pathname);i&&(r=parseInt(i[1]),r>0||(r=0)),A(`正在注册账号`),J.Regist({name:e,pwd:t,valno:n,guider:r},e=>{e.code==1?(A(`注册成功，正在获取服务器列表`),setTimeout(()=>window.location.reload(),500)):(O(`#regist_name`,e.result||`注册失败`),k($(`#regist_panel`)))})}GetValidationImage(){J.ValidationImage(function(e){$(`.validnum-box>.validnum-img`).attr(`src`,`data:image/svg+xml;base64,`+e)})}},hn=`万俟司马上官欧阳夏侯诸葛闻人东方赫连皇甫尉迟公羊澹台公冶宗政濮阳淳于单于太叔申屠公孙仲孙轩辕令狐锺离宇文长孙慕容鲜于闾丘司徒司空丌官司寇子车颛孙端木巫马公西乐正公良拓拔夹谷谷梁梁丘左丘东门西门`,gn=`赵钱孙李周吴郑王冯陈楮卫蒋沈韩杨朱秦尤许何吕施张孔曹严华金魏陶姜戚谢邹喻柏水窦章云苏潘葛奚范彭郎`,_n=`世舜丞主产仁仇仓仕仞任伋众伸佐佺侃侪促俟信俣修倝倡倧偿储僖僧僳儒俊伟列则刚创前剑助劭势勘参叔吏嗣士壮孺守宽宾宋宗宙宣实宰尊峙峻崇崈川州巡帅庚战才承拯操斋昌晁暠曹曾珺玮珹琒琛琩琮琸瑎玚璟璥瑜生畴矗矢石磊砂碫示社祖祚祥禅稹穆竣竦综缜绪舱舷船蚩襦轼辑轩子杰榜碧葆莱蒲天乐东钢铎铖铠铸铿锋镇键镰馗旭骏骢骥驹驾骄诚诤赐慕端征坚建弓强彦御悍擎攀旷昂晷健冀凯劻啸柴木林森朴骞寒函高魁魏鲛鲲鹰丕乒候冕勰备宪宾密封山峰弼彪彭旁日明昪昴胜汉涵汗浩涛淏清澜浦澉澎澔瀚瀛灏沧虚豪豹辅辈迈邶合部阔雄霆震韩俯颁颇频颔风飒飙飚马亮仑仝代儋利力劼勒卓哲喆展帝弛弢弩彰征律德志忠思振挺掣旲旻昊昮晋晟晸朕朗段殿泰滕炅炜煜煊炎选玄勇君稼黎利贤谊金鑫辉墨欧有友闻问`,vn=`筠柔竹霭凝晓欢霄枫芸菲寒伊亚宜姬舒影荔枝思丽秀娟英华慧巧美娜静淑惠珠翠雅芝玉萍红娥玲芬芳燕彩春菊勤珍贞莉兰凤洁梅琳素云莲真环雪荣妹霞香月莺媛艳瑞凡佳嘉琼桂娣叶璧璐娅琦晶妍茜秋珊莎锦黛青倩婷姣婉娴瑾颖露瑶怡婵雁蓓纨仪荷丹蓉眉君琴蕊薇菁梦岚苑婕馨瑗琰韵融园艺咏卿聪澜纯毓悦昭冰爽琬茗羽希宁欣飘育滢馥`;function yn(e,t){t||=parseInt(Math.random()*2)+1;var n=[];if(t==2){var r=parseInt(Math.random()*hn.length);r%2==1&&--r,n.push(hn[r++]),n.push(hn[r])}else n.push(gn[parseInt(Math.random()*gn.length)]);return e==0?n.push(_n[parseInt(Math.random()*_n.length)]):n.push(vn[parseInt(Math.random()*vn.length)]),parseInt(Math.random()*4)>1&&(e==0?n.push(_n[parseInt(Math.random()*_n.length)]):n.push(vn[parseInt(Math.random()*vn.length)])),n.join(``)}function bn(){for(var e=`abcdefghijklmnopqrstuvwxyz`,t=`123456789`,n=[],r=parseInt(Math.random()*3)+3,i=0;i<r;i++)i<3?n.push(e[parseInt(Math.random()*e.length)]):n.push(t[parseInt(Math.random()*t.length)]);return n.join(``)}function xn(){for(var e=20,t=[],n=0;n<4;n++){var r=parseInt(Math.random()*15+1);e>=r?(n==3?r=e:e-=r,t[n]=r):(t[n]=e,e=0)}var i={};return i.str=t[0]+15,i.con=t[1]+15,i.dex=t[2]+15,i.int=t[3]+15,i}function Sn(e){switch(e){case`name`:$(`#reg_name`).val(yn(+!$(`#gender_0`).is(`:checked`)));break;case`id`:$(`#reg_id`).val(bn());break;case`prop`:var t=xn();$(`#reg_str`).val(t.str),$(`#reg_con`).val(t.con),$(`#reg_dex`).val(t.dex),$(`#reg_int`).val(t.int);break}}var Cn=class extends q{constructor(){super(),this.template=`
...
`}select(){var e=$(`.role-list>.select`);if(e.length){var t=e.attr(`roleid`);SendCommand(`login `+t),A(`正在进入游戏`,`#role_panel`)}}addRole(){if($(`.role-list>.role-item`).length>4)return S.Show({content:`你只能最多创建五个角色`});k($(`#addrole_panel`)),Sn(`name`),Sn(`prop`),Sn(`id`)}delete(){var e=$(`.role-list>.select`);if(e.length){var t=e.attr(`roleid`);t&&S.Show({content:`是否确认删除角色：`+e.html(),onOK:function(){SendCommand(`deleterole `+t)}})}}},wn=class extends q{constructor(){super(),this.template=`
...
`}create(){var e={};if(e.name=$(`#reg_name`).val(),e.gender=$(`#gender_0`).is(`:checked`)?1:2,e.str=parseInt($(`#reg_str`).val()),e.con=parseInt($(`#reg_con`).val()),e.dex=parseInt($(`#reg_dex`).val()),e.int=parseInt($(`#reg_int`).val()),!/^[\u4E00-\u9FA5]{2,5}$/.test(e.name))return O(`#reg_name`,`名称格式错误，只能使用2-5位中文字符`);if(e.str<15||e.str>30)return O(`#reg_name`,`臂力需要在15-30之间`);if(e.con<15||e.con>30)return O(`#reg_name`,`根骨需要在15-30之间`);if(e.dex<15||e.dex>30)return O(`#reg_name`,`身法需要在15-30之间`);if(e.int<15||e.int>30)return O(`#reg_name`,`悟性需要在15-30之间`);if(e.str+e.con+e.dex+e.int!=80)return O(`#reg_name`,`先天属性需要在15-30之间，并且总和等于80`);A(`正在创建角色`,`#addrole_panel`),SendCommand(`createrole `+e.name+` `+e.gender+` `+e.str+` `+e.con+` `+e.dex+` `+e.int)}},On=class extends q{constructor(){super(),this.template=`
...
`}relogin(){k($(`#login_panel`));var e=new Date;e.setTime(-1e3);for(var t=document.cookie.split(`; `),n=0;n<t.length;n++){var r=t[n].split(`=`);document.cookie=r[0]+`=''; expires=`+e.toGMTString()}}loginIn(){var e=$(`#login_name`).val().toLowerCase(),t=$(`#login_pwd`).val();if(!e)return O(`#login_name`,`请输入用户名`);if(!/^[a-z0-9]{5,15}$/.test(e))return O(`#login_name`,`用户名格式错误,需要5-15位字母开头的字母，数字或下划线，不区分大小写`);if(!t)return O(`#login_pwd`,`请输入密码`);if(t.length<6||t.length>20)return O(`#login_pwd`,`密码长度在6到20之间`);A(`正在登录`,`#login_panel`),J.Login(e,t,e=>{e.code?An.showServers():(O(`#login_name`,e.result||`登陆失败`),k(`#login_panel`))})}},Mn=new pn,Nn=new jn,Pn=new mn,Fn=new Cn,In=new wn,Ln=new Tn,Rn=new En,zn=new Dn,Bn=new On;function Vn(){let e=$(this).attr(`nid`);k($(`#new_panel `)),$(`#news_frame`).attr(`src`,`/news/`+e+`.html`)}var Hn=class extends q{constructor(){super(),this.template=`
...
`}on_mount(){if($(`.login-content`).on(`click`,`.panel_item`,e=>this.LoginCommand(e)),$(`.role-list`).on(`click`,`.role-item`,function(){$(this).parent().find(`.select`).removeClass(`select`),$(this).addClass(`select`)}),$(`.new-list>li`).on(`click`,Vn),!x.GetUserCookie(`p`))return $(`#login_panel`).show();Mn.showServers()}LoginCommand(e){switch($(e.currentTarget).attr(`command`)){case`ToRolePanel`:k($(`#role_panel`));break;case`ToServerPanel`:_e(),k($(`#slist_panel`));break;case`ToLogin`:k($(`#login_panel`));break;case`Forget`:k($(`#reset_panel`));break;case`CancleRegist`:k($(`#login_panel`));break;case`Down`:k($(`#download`));break;case`ToRegist`:k($(`#regist_panel`)),Pn.open();break;case`Regist`:Pn.regist();break;case`SelectServer`:Mn.selectServer();break;case`LoginIn`:Nn.loginIn();break;case`ResetPwd`:In.reset();break;case`AddRole`:Fn.addRole();break;case`SelectRole`:Fn.select();break;case`CreateRole`:zn.create();break;case`BindPhone`:Rn.bind();break;case`CheckValid`:Rn.check();break;case`UpdatePwd`:Ln.update();break;case`ToUpdate`:Ln.open();break;case`ReLogin`:Nn.relogin();break;case`DeleteRole`:Fn.delete();break}}},Un={rally:`集结准备`,normal:`常规战斗`,side:`副位机制`,aspect:`四象化劫`,vulnerability:`逆脉易伤`,enrage:`七灭归劫`,terminal:`战斗终结`},Wn={rally:`phase-rally`,normal:`phase-normal`,side:`phase-side`,aspect:`phase-aspect`,vulnerability:`phase-vulnerability`,enrage:`phase-enrage`,terminal:`phase-terminal`},Gn=[`东`,`南`,`西`,`北`];function X(e){return e?String(e).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`).replace(/'/g,`&#39;`):``}var Kn={currentEventId:null,isTest:!1,stateData:null,playerId:null,expanded:!1,raidMessages:[],raidMembers:[],timerHandle:null,serverTimeOffset:0,domMounted:!1,getPlayerId(e){let t=e=>{if(e==null)return null;if(typeof e==`object`)return e.id!==void 0&&e.id!==null?String(e.id):e.playerId!==void 0&&e.playerId!==null?String(e.playerId):e.userid!==void 0&&e.userid!==null?String(e.userid):e.user_id!==void 0&&e.user_id!==null?String(e.user_id):null;let t=String(e).trim();return t.length>0?t:null};if(e!=null){let n=t(e);if(n)return n}if(typeof Process<`u`&&Process.player){let e=t(Process.player);if(e)return e}return t(this.playerId)},init(){this.domMounted||=(this.bindEvents(),!0)},bindEvents(){let e=$(`.container`);e.length&&(e.off(`.qimie`),e.on(`click.qimie`,`.qimie-bar-toggle, .qimie-summary-clickable`,e=>{e.stopPropagation(),this.toggleExpand()}),e.on(`click.qimie`,`.qimie-raid-btn`,e=>{e.stopPropagation(),this.openRaidChat()}))},toggleExpand(){this.expanded=!this.expanded;let e=$(`.qimie-status-bar`);if(e.length){e.toggleClass(`expanded`,this.expanded);let t=e.find(`.qimie-bar-toggle .glyphicon`);this.expanded?(t.removeClass(`glyphicon-chevron-down`).addClass(`glyphicon-chevron-up`),e.find(`.qimie-bar-toggle .toggle-text`).text(`收起`)):(t.removeClass(`glyphicon-chevron-up`).addClass(`glyphicon-chevron-down`),e.find(`.qimie-bar-toggle .toggle-text`).text(`详情`))}},openRaidChat(){let e=$(`.chat-panel`);if(e.length){e.removeClass(`hide`);let t=e.find(`.channel-box`);t.find(`span`).removeClass(`selected`);let n=t.find(`span[channel="raid"]`);if(!n.length){let e=t.find(`span[channel="pty"]`);n=e.length?$(`<span channel="raid">战团</span>`).insertAfter(e):$(`<span channel="raid">战团</span>`).appendTo(t)}n.addClass(`selected`),t.attr(`channel`,`raid`);let r=e.find(`.sender-box`);r.length&&r.val(``).focus()}else typeof E==`function`&&E(`raid `)},onBossEvent(e,t){if(e){if(t?this.playerId=t:typeof Process<`u`&&Process.player&&(this.playerId=Process.player),this.init(),e.action===`clear`){(!this.currentEventId||!e.eventId||this.currentEventId===e.eventId)&&this.clear();return}e.eventId&&this.currentEventId&&this.currentEventId!==e.eventId&&(this.clear(),t?this.playerId=t:typeof Process<`u`&&Process.player&&(this.playerId=Process.player),this.init()),this.currentEventId=e.eventId||this.currentEventId,this.isTest=!!(this.currentEventId&&String(this.currentEventId).indexOf(`test`)>=0),typeof e.serverNow==`number`&&!isNaN(e.serverNow)&&e.serverNow>0&&(this.serverTimeOffset=e.serverNow-Date.now()),this.stateData=Object.assign({},this.stateData||{},e),this.render(),this.startTimer()}},onRaid(e){if(e){if(this.init(),e.action===`close`){(!this.currentEventId||!e.eventId||this.currentEventId===e.eventId)&&(this.raidMessages=[],this.raidMembers=[],this.renderRaidSection());return}if(e.eventId&&this.currentEventId&&this.currentEventId!==e.eventId&&(this.raidMessages=[],this.raidMembers=[]),e.eventId&&(this.currentEventId=e.eventId),e.action===`state`)Array.isArray(e.members)?this.raidMembers=e.members:e.members&&typeof e.members==`object`&&(this.raidMembers=Object.values(e.members)),e.messages&&Array.isArray(e.messages)&&(this.raidMessages=e.messages.slice(-20).map(e=>({id:e.id||e.playerId||e.senderId,name:e.name||e.senderName||`侠士`,content:e.content||e.message||``,time:e.time||e.at||Date.now()}))),this.renderRaidSection();else if(e.action===`message`||e.content||e.message){let t={id:e.id||e.playerId||e.senderId,name:e.name||e.senderName||`侠士`,content:e.content||e.message||``,time:e.time||e.at||Date.now()};this.raidMessages.push(t),this.raidMessages.length>20&&this.raidMessages.splice(0,this.raidMessages.length-20),this.renderRaidSection()}}},clear(){this.timerHandle&&=(clearInterval(this.timerHandle),null),this.currentEventId=null,this.stateData=null,this.raidMessages=[],this.raidMembers=[],this.isTest=!1,this.playerId=null,this.serverTimeOffset=0,$(`.qimie-status-bar`).remove()},startTimer(){this.timerHandle||=setInterval(()=>{this.updateCountdown()},1e3)},getNow(){return Date.now()+(this.serverTimeOffset||0)},updateCountdown(){if(!this.stateData)return;let e=this.getNow(),t=Number(this.stateData.expiresAt);if(!isNaN(t)&&t>0){let n=Math.max(0,Math.ceil((t-e)/1e3)),r=Math.floor(n/60),i=n%60,a=(r<10?`0`+r:r)+`:`+(i<10?`0`+i:i);$(`.qimie-timer-val`).text(a)}else $(`.qimie-timer-val`).text(`--:--`);let n=this.stateData.mechanic,r=n?Number(n.expiresAt):NaN;if(!isNaN(r)&&r>0){let t=Math.max(0,Math.ceil((r-e)/1e3));$(`.qimie-mech-timer`).text(t+`s`)}else $(`.qimie-mech-timer`).text(``)},render(){if(!this.stateData)return;let e=$(`.qimie-status-bar`);if(!e.length){e=$(`<div class="qimie-status-bar"></div>`);let t=$(`.content-message`);t.length?e.insertBefore(t):$(`.container`).append(e)}let t=this.stateData,n=t.phase||`rally`,r=(this.isTest?`【测试】`:``)+(Un[n]||n),i=Wn[n]||`phase-normal`,a=t.stacks||0,o=a>=6?`stacks-danger`:a>=3?`stacks-warn`:`stacks-normal`,s=Math.max(0,Number(t.bossHp)||0),c=Math.max(1,Number(t.bossMaxHp)||1),l=Math.min(100,Math.max(0,Math.round(s/c*1e3)/10)),u=``,d=!1;if(t.mechanic&&t.mechanic.type){let e=t.mechanic,n=this.getPlayerId();Array.isArray(e.targetIds)&&n&&e.targetIds.some(e=>this.getPlayerId(e)===n||String(e)===n)&&(d=!0);let r=e.type;e.type===`yinyang`?r=`阴阳断脉`:e.type===`tiangang`?r=`天罡共劫`:e.type===`aspect`?r=`四象化劫`:e.type===`miehun`||e.type===`灭魂指`?r=`灭魂指`:(e.type===`tianyin`||e.type===`七灭天音`)&&(r=`七灭天音`);let i=``;d?i=`<span class="qimie-target-me">【你被点名】</span>`:e.targetRoom?i=`<span>地点: ${X({center:`镇劫中宫`,east:`东·问剑台`,south:`南·焚天台`,west:`西·断金台`,north:`北·镇魂台`}[e.targetRoom]||e.targetRoom)}</span>`:Array.isArray(e.targetRooms)&&e.targetRooms.length?i=`<span>地点: ${X(e.targetRooms.map(e=>({center:`镇劫中宫`,east:`东·问剑台`,south:`南·焚天台`,west:`西·断金台`,north:`北·镇魂台`})[e]||e).join(`/`))}</span>`:Array.isArray(e.targetIds)&&e.targetIds.length&&(i=`<span>点名: ${e.targetIds.length}人</span>`);let a=``;if(e.requiredCount){let t=0;Array.isArray(e.participants)?t=e.participants.length:typeof e.participants==`number`?t=e.participants:e.participants&&typeof e.participants==`object`&&(t=Object.keys(e.participants).length),a=`<span>分摊: ${t}/${e.requiredCount}</span>`}u=`
            ...
            `}let f=``;if(t.aspects&&Array.isArray(t.aspects)&&t.aspects.length){f=`<div class="qimie-aspects-grid">`;for(let e=0;e<t.aspects.length;e++){let n=t.aspects[e]||{},r=Gn[e]||``,i=Math.max(0,Number(n.hp)||0),a=Math.max(1,Number(n.maxHp)||1),o=Math.min(100,Math.max(0,Math.round(i/a*100))),s=n.alive!==!1&&i>0;f+=`
...
`}on_mount(){$(`.container`).on(`click`,lr),$(`.channel-box`).on(`click`,`span`,sr),$(`.combat-commands`).on(`click`,`.pfm-item`,L.Perform).on(`wheel`,L.Scroll),$(`.room-commands`).on(`wheel`,L.Scroll),$(`.sender-box`).on(`keyup`,ar),$(`.room_items`).on(`click`,`.room-item`,Z.selectItem),$(`.bottom-bar`).on(`click`,`.tool-item,.state-bar,.item-command`,nr),$(`.map-panel`).on(`click`,tr),$(`.sender-btn`).on(`click`,or),$(`.room_exits`).on(`pointerdown`,Z.before_click_exits).on(`pointerup`,Z.click_exits),$(`.room-title>.map-icon`).on(`click`,j.OpenDialog.bind(j))}},er=0;function tr(e){if(!$(e.currentTarget).closest(`.dialog-map`).length&&!j.SuppressRoomClick){if(er||=0,Date.now()-er>500){er=Date.now();return}j.OpenDialog()}}function nr(e){var t=$(this).attr(`command`);return t?rr(t):(t=$(this).attr(`cmd`),t&&SendCommand(t),!1)}function rr(e){switch(e){case`showtool`:St();break;case`showchat`:return ir();case`showcombat`:return L.Show();case`stopstate`:if(K.extend.is_record)return K.extend.stop_record();SendCommand(`state stop`);break;case`stateinfo`:SendCommand(`state info`);break;default:K.show(e);break}return!1}function ir(){var e=$(`.chat-panel`).toggleClass(`hide`);return e.is(`.hide`)?Yn=!1:(Yn=!0,e.find(`input`).val(``).focus()),!1}function ar(e){e.keyCode==13&&or()}function or(){var e=$(`.sender-box`),t=e.val(),n=t?String(t).trim():``;if(!n){e.val(``).focus();return}if(n.length>100)return ReceiveMessage(`<hir>你输入的内容太多了。</hir>`);var r=$(`.channel-box`).attr(`channel`)||`chat`;e.val(``).focus(),SendCommand(r+` `+n)}function sr(){var e=$(this),t=e.attr(`channel`);if(t==`emote`)return cr();if(!e.is(`.selected`)){var n=e.parent();return n.children().removeClass(`selected`),e.addClass(`selected`),n.attr(`channel`,t),$(`.sender-box`).focus(),!1}}function cr(){var e=$(`.channel-emotes`);e.is(`.hide`)?(e.removeClass(`hide`),Z.emtoes||(SendCommand(`emote`),Z.emtoes=[],$(`.sender-box`).blur(),e.on(`click`,`span`,function(){var e=$(this).html();$(`.sender-box`).val(`*`+e).focus(),$(`.channel-emotes`).addClass(`hide`)}))):$(`.channel-emotes`).addClass(`hide`)}function lr(e){var t=$(e.target);if(K.isShow&&t.is(`.container.dialog-open`))return K.hide(),S.Close(),C.Close(),!1;var n=t.attr(`cmd`);if(n||=t.parent().attr(`cmd`),n){let e=n[0];if(e==`_`){var r=n.split(` `);switch(r[0]){case`_confirm`:S.Process(r);break;case`_setting`:F.save(r[1],r[2]);break;case`_trade`:K.trade.confirm(r[1]);break;case`_close`:se.Close(t);break;case`_hide`:break;case`_closed`:K.hide();case`_party`:K.party.command(r[1]);break}}else e===`#`?M.run(n):(Zn(n)?Qn():K.curItem===`item`&&K.item&&K.item.isShow&&t.closest(`.dialog-item-dialog`).length>0?K.item.captureNextPrompt():K.isShow&&C.captureNext(),SendCommand(n),!t.closest(`.dialog-fb`).length&&!t.closest(`.dialog-item-subdialog`).length&&t.closest(`.dialog-content`).length>0&&t.closest(`.item-commands`).remove());return!1}else Yn&&(t.closest(`.chat-panel`).length||($(`.chat-panel`).addClass(`hide`),Yn=!1));S.Close(),C.Close()}window.HandlerMenuCommand=rr;var ur=new Hn,dr=new $n,fr=class extends q{constructor(){super(),this.template=`${ur.render()}\n${dr.render()}`}on_mount(){ur.on_mount(),dr.on_mount()}};globalThis.$=e,globalThis.SendCommand=E,globalThis.ReceiveMessage=D,globalThis.Confirm=S,globalThis.Warn=se,globalThis.CmdPrompt=C,globalThis.Process=Z,globalThis.Dialog=K;function pr(){let e=new fr;e.onCompile(),e.mount(document.body),Z.init()}e(pr);