...
`,et=`  <div class="setting dialog-custom">
        ...
        </div>`,tt=` <div class="setting dialog-skeys"></div>`,nt=` <div class="setting dialog-extend"></div>`,rt=`
.setting {
...
`;function mt(e){var t=Math.max(0,e||0)/1e3;return(t%1?t.toFixed(1):t.toFixed(0))+`秒`}var ht=`
...
`,e.reward),t.push(B.renderDrops(e.drops)),t.push(`</pre>`),this.descElement.html(t.join(``)),this.select(e.index)},append_status:function(e,t){let n=t.status??[];if(n.length){e.push(`<div class="fb-actions">`);for(let r=0;r<n.length;r++){let i=n[r];i&&(i[0]===1?(e.push(`<div class="fb-action finshed">`),e.push(`<span class="action-desc">由`,i[1],`首次通过`,`</span>`),e.push(`<span class="action-name" cmd="cr2 `,t.index,` `,r,`">`,this.fb_models[r],`</span>`),e.push(`</div>`)):(e.push(`<div class="fb-action">`),e.push(`<span class="action-desc">该模式尚未完成首杀`,i[1]?`，称号奖励：`+i[1]:``,`</span>`),e.push(`<span class="action-name"  cmd="cr2 `,t.index,` `,r,`">`,this.fb_models[r],`</span>`),e.push(`</div>`)))}e.push(`</div>`)}},fb_buttons:[`进入副本`,`困难模式`,`组队进入`],append_footer:function(){let e=this.items[this.selected_index],t=[];if(e.unlock)for(let n=0;n<e.diffs.length;n++)e.diffs[n]&&t.push(`<span cmd="jh fb `,e.index,` start`,n+1,`">`,this.fb_buttons[n],`</span>`);Dialog.footerElement.find(`.item-commands`).html(t.join(``))}},H={name:`禁地`,items:null,type:`ar`,selected_index:0,select:B.select,onClickItem:B.onClickItem,append_status:V.append_status,append_actions:B.append_actions,fb_models:[`普通`,`普通`,`组队`],onDetail:function(e){var t=this.items[e.index];if(t)return t.type=`禁地`,t.desc=e.desc,t.actions=e.actions,t.status=e.status,t.reward=e.reward,t.drops=e.drops,this.showDetail(t)},update_unlock:function(e){for(let t=0;t<this.items.length;t++)this.items[t].unlock=(e&2**t)!=0},show:function(e,t){var n=[`<div class='fb-content'>`];let r=Math.max(this.items.length,10);for(var i=0;i<r;i++){var a=this.items[i];n.push(`<div class="fb-item`),a?(a.unlock||n.push(` lock`),n.push(`" index="`,i,`">`,a.name,`</div>`),a.index=i):n.push(`">&nbsp;</div>`)}n.join(`</div>`),this.listElement=e,this.descElement=t,this.listElement.html(n.join(``)),this.onClickItem(this.selected_index)},showDetail:function(e){var t=[`<pre>`];if(t.push(e.name),e.unlock?t.push(`
<hig>已解锁</hig>
`):t.push(`
<red>未解锁</red>
`),t.push(e.desc,`
...
`,e.reward),t.push(B.renderDrops(e.drops)),t.push(`</pre>`),this.descElement.html(t.join(``)),this.select(e.index)},append_footer:function(){let e=this.items[this.selected_index];e.unlock?Dialog.footerElement.find(`.item-commands`).html(`<span cmd="jh ar ${e.index} start">进入地图</span>`):Dialog.footerElement.find(`.item-commands`).empty()}};function Mt(e){if(!e||!e.active||!(e.expiresAt>0))return`未开始`;let t=Math.max(0,Math.ceil((e.expiresAt-Date.now())/1e3));return t?Math.floor(t/60)+`分`+String(t%60).padStart(2,`0`)+`秒`:`即将结算`}var Nt=[{name:`潜能`,color_name:`潜能`,grade:2},{name:`经验`,color_name:`经验`,grade:2}],Pt={name:`秘境`,type:`mj`,items:null,selected_index:0,select:B.select,onClickItem:function(e){let t=this.items&&this.items[e];t&&(t.desc?this.showDetail(t):SendCommand(`jh mj `+e),this.select(e))},onDetail:function(e){let t=this.items&&this.items[e.index];t&&(t.type=`秘境`,t.desc=e.desc,t.drops=e.drops||Nt,t.status=e.status||t.status,t.actions=e.actions,this.showDetail(t))},show:function(e,t){this.listElement=e,this.descElement=t;let n=[`<div class='fb-content'>`],r=this.items||[];for(let e=0;e<r.length;e++){let t=r[e];t.index=e,n.push(`<div class="fb-item mj-item" index="`,e,`">`,t.name,`</div>`)}n.push(`</div>`),this.listElement.html(n.join(``));let i=this.items&&this.items[this.selected_index];i&&(i.desc=null),this.onClickItem(this.selected_index),this.startTimer()},showDetail:function(e){let t=e.status||{},n=[`<pre>`,`<hig>`,e.name,`</hig>
`];n.push(e.desc||``),n.push(`

<hiy>挑战状态</hiy>
`),t.active?n.push(`进行中：击杀 `,t.kills||0,`/`,t.maxKills||100,`，剩余 `,Mt(t),`。
`):n.push(`今日挑战：`,t.dailyCount||0,`/`,t.dailyLimit||1,`，归墟种：`,t.ticket||0,`枚。
`),n.push(jt(`掉落物`,e.drops)),n.push(`</pre>`),this.descElement.html(n.join(``)),this.select(e.index),this.append_footer()},append_footer:function(){let e=this.items&&this.items[this.selected_index];if(!e)return Dialog.footerElement.find(`.item-commands`).empty();let t=e.status||{},n=[];t.active?n.push(`<span cmd="mijing over">结束挑战</span>`):(t.dailyCount||0)>=(t.dailyLimit||1)?n.push(`<span class="disabled">今日挑战次数已用尽</span>`):(t.ticket||0)>0?n.push(`<span cmd="jh mj `,e.index,` start">进入秘境</span>`):n.push(`<span cmd="shop">购买归墟种</span>`),Dialog.footerElement.find(`.item-commands`).html(n.join(``))},updateStatus:function(e,t){let n=this.items&&this.items[t];n&&(n.status=e,this.selected_index===t&&this.descElement&&this.showDetail(n))},startTimer:function(){clearInterval(this.timer),this.timer=setInterval(()=>{let e=this.items&&this.items[this.selected_index];e&&e.status&&e.status.active&&this.descElement&&this.isVisible()&&this.showDetail(e)},1e3)},isVisible:function(){return this.listElement&&this.listElement.closest(`.dialog-fb`).length>0},close:function(){clearInterval(this.timer),this.timer=null}},Ft={fb:V,fam:B,ar:H,mj:Pt},It={init:function(){Dialog.injectStyle(Lt)},close:function(){Pt.close(),this.hideSkillDetail(),this.element.remove(),this.isShow=!1},onData:function(e){if(e.close)return Dialog.isShow&&Dialog.hide();if(e.skill)return this.showSkillDetail(e.skill);if(e.drop)return this.showDropDetail(e.drop);if(e.desc){let t=Ft[e.t]||this.selected_item;return t&&t.onDetail(e)}if(e.t===`mj`){e.status&&Pt.updateStatus(e.status,e.index||0);return}if(e.unlock!==void 0||e.unlock2!==void 0)return this.update_lock(e);if(e.refresh!==void 0&&this.isLoad){let t=Ft[e.t],n=t.items[e.refresh];if(n&&n.desc){n.desc=null;let e=t.items.indexOf(n);t.selected_index==e&&t.onClickItem(e)}return}!e.fbs&&!e.mijings||(B.items=(e.families||[]).map(function(e){return{name:e,unlock:!1}}),V.items=(e.fbs||[]).map(function(e){return{name:e}}),H.items=(e.areas||[]).map(function(e){return{name:e,unlock:!1}}),Pt.items=(e.mijings||[]).map(function(e){return{name:e}}),this.selected_item.show(this.listElement,this.descElement))},show:function(){this.isShow||(this.element||=$(`<div class='dialog-fb'><div class='fb-left'></div><div class='fb-right'></div></div>`),this.listElement=this.element.find(`.fb-left`).off(`click.jhList`).on(`click.jhList`,`.fb-item,.fam-item,.mj-item`,this.item_click),this.descElement=this.element.find(`.fb-right`),this.element.off(`click.jhSkill`).on(`click.jhSkill`,`.jh-skill-detail-close,.jh-skill-detail-mask`,this.skill_detail_close).on(`click.jhSkill`,`.jh-skill-detail`,function(e){e.stopPropagation()}),Dialog.title(`江湖`),Dialog.icon(`home`),this.element.appendTo(Dialog.contentElement),this.isShow=!0,this.isLoad?SendCommand(`jh fb lock`):(SendCommand(`jh`),this.isLoad=!0,this.selected_item=this.footers[0]),this.create_footer())},selected_item:null,footers:[B,V,H,Pt],create_footer:function(){for(var e=[],t=0;t<this.footers.length;t++){let n=this.footers[t];e.push(`<span class='footer-item`+(n==this.selected_item?` select`:``)+`' for='`+t+`'>`+this.footers[t].name+`</span>`)}e.push(`<div class="item-commands"></div>`),Dialog.footerElement.html(e.join(``))},item_click:function(){var e=$(this);if(e.is(`.selected`))return;let t=e.attr(`index`);t!==void 0&&Dialog.jh.selected_item.onClickItem(t)},skill_detail_close:function(){Dialog.jh.hideSkillDetail()},hideSkillDetail:function(){this.skillDetailElement&&=(this.skillDetailElement.remove(),null)},showSkillDetail:function(e){this.showDetailPopup(e)},showDropDetail:function(e){this.showDetailPopup(e)},showDetailPopup:function(e){this.hideSkillDetail();var t=[`<div class="jh-skill-detail-mask"><div class="jh-skill-detail grade`,e.grade||0,`">`];t.push(`<div class="jh-skill-detail-header"><span class="jh-skill-detail-title">`),t.push(e.color_name||e.name),t.push(`</span><span class="jh-skill-detail-close">关闭</span></div>`),t.push(`<pre class="jh-skill-detail-body">`),t.push(e.desc),t.push(`</pre></div></div>`),this.skillDetailElement=$(t.join(``)).appendTo(this.element)},update_lock:function(e){e.unlock>=0&&V.items&&(V.update_unlock(e.unlock),this.selected_item===V&&V.show(this.listElement,this.descElement)),e.unlock2>=0&&H.items&&(H.update_unlock(e.unlock2),this.selected_item===H&&H.show(this.listElement,this.descElement))},footerChanged:function(e){let t=this.footers[e];t!=this.selected_item&&(this.selected_item=t,Dialog.footerElement.find(`.item-commands`).empty(),t.show(this.listElement,this.descElement))}},Lt=`
...
`;function Rt(e){(!e||e<0)&&(e=0);var t=Math.floor(e/36e5),n=Math.floor(e%36e5/6e4);return t>0?t+`小时`+n+`分`:n>0?n+`分钟`:`片刻`}var zt={init:function(){},createElement:function(){return $(`<div class="dialog-relation"></div>`)},inner_show:function(){SendCommand(`relation`),this.isShow=!0,Dialog.title(`关系`),Dialog.icon(`heart`),this.element.off(`click.household`).on(`click.household`,`.household-open`,function(){SendCommand(`household view`)})},onData:function(e){var t=[];if(t.push(`<div class='relation-item'>`),t.push(`<div class='relation-desc'>`),e.husband?(t.push(`你的丈夫：`),t.push(e.husband)):e.wife?(t.push(`你的妻子：`),t.push(e.wife)):t.push(`你目前没有结婚。`),t.push(`</div>`),(e.wife||e.husband)&&(t.push(`<div class='relation-cmd' cmd='_confirm greet wife'><him>❀送花❀</him></div>`),t.push(`<div class='relation-cmd' cmd='rel marry'>解除关系</div>`)),t.push(`</div>`),t.push(`<div class='relation-item'>`),t.push(`<div class='relation-desc'>`),e.shifu?(t.push(`你的师父：`),t.push(e.shifu)):e.tudi?(t.push(`你的徒弟：`),t.push(e.tudi)):t.push(`你目前没有拜师，也没有收徒。`),t.push(`</div>`),e.shifu?(t.push(`<div class='relation-cmd' cmd='greet master'><hig>请安</hig></div>`),t.push(`<div class='relation-cmd' cmd='rel st'>出师</div>`),t.push(`</div>`)):e.tid&&t.push(`<div class='relation-cmd' cmd='rel st'>解除关系</div>`),t.push(`</div>`),e.st!=null&&(t.push(`<div class='relation-item'><div class='relation-desc'>`),t.push(`当师徒组队完成副本后将获得额外奖励，本周已完成`+e.st+`/10。`,`</div>`),t.push(`<div class='relation-cmd' cmd='team add `,e.tid??e.shifu,`'>邀请组队</div>`),t.push(`</div>`)),e.reward&&(t.push(`<div class='relation-item'>`),t.push(e.reward),t.push(`</div>`)),t.push(`</div>`),t.push(`<div class='relation-item'><div class='relation-desc'>武道家族</div><div class='relation-cmd household-open'>打开家族面板</div></div>`),e.fls)for(let n of e.fls)n&&(t.push(`<div class='relation-item'>`),t.push(`<div class='relation-desc'>你的家人：`,n[0]),n[2]?(t.push(`，已`,n[2],Rt(n[3])),t.push(`</div>`),t.push(`<div class='relation-cmd' cmd='rel `,n[1],` stop'>停止</div>`)):(t.push(`空闲中</div>`),t.push(`<div class='relation-cmd' cmd='household assign `,n[1],` planting'>种植</div>`),t.push(`<div class='relation-cmd' cmd='household assign `,n[1],` alchemy'>炼药</div>`),t.push(`<div class='relation-cmd' cmd='household assign `,n[1],` teaching'>授课</div>`),t.push(`<div class='relation-cmd' cmd='household assign `,n[1],` mining'>挖矿</div>`)),t.push(`</div>`));this.element.html(t.join(``))},inner_close:function(){this.element.remove(),this.isShow=!1}},Bt={init:function(){},createElement:function(){return $(`<div class="dialog-team"></div>`)},inner_show:function(){SendCommand(`team`),this.isShow=!0,Dialog.title(`队伍`),this.element.on(`click`,`.team-item`,this.clickItem),Dialog.icon(`list`)},items:[],onData:function(e){if(e.items&&(this.items=e.items,e.items.length?this.isCap=e.items[0].id==Process.player:this.isCap=0),e.dismiss&&(this.items.length=0,this.isCap=!1),e.remove){if(!this.items.length)return;for(var t=0;t<this.items.length;t++)if(this.items[t].id==e.remove){this.items.splice(t,1);break}}this.createItems()},inner_close:function(){this.element.remove(),this.isShow=!1},createItems:function(){if(this.element){for(var e=[],t=0;t<this.items.length;t++){var n=this.items[t];e.push(`<div class='team-item' index='`+t+`'>`),e.push(`<span class='team-flag'>`),e.push(t>0?``:`<span class='glyphicon glyphicon-flag'></span>`),e.push(`</span>`),e.push(`<span class='team-title'>`),e.push(n.name),e.push(`</span>`),e.push(`</div>`)}e.length||e.push(`<div class="empty">你还没有加入任何队伍。</div>`),this.element.html(e.join(``))}},clickItem:function(){var e=$(this),t=Dialog.team.items[e.attr(`index`)];if(t){var n=[`<div class='item-commands'>`];n.push(`<span cmd="look3 `+t.id+`">查看</span>`);var r=Dialog.team.items[0].id==Process.player;r&&t.id!=Process.player?n.push(`<span cmd="team remove `+t.id+`">移出队伍</span>`):t.id==Process.player&&n.push(`<span cmd="team out `+t.id+`">退出队伍</span>`),r&&t.id==Process.player&&n.push(`<span cmd="team set">更改分配方式</span>`),n.push(`</div>`),Dialog.team.element.find(`.item-commands`).remove(),$(n.join(``)).appendTo(e)}}},Vt=`
.dialog-party>wht {
    display: inline-block;
    height: 15rem;
    line-height: 15rem;
...
`;function W(e){return String(e??``).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`).replace(/'/g,`&#39;`)}function sn(e){e=Math.max(0,parseInt(e)||0);let t=[];return e>=1e4&&(t.push(Math.floor(e/1e4)+` 两黄金`),e%=1e4),e>=100&&(t.push(Math.floor(e/100)+` 两白银`),e%=100),e&&t.push(e+` 个铜板`),t.join(` `)||`0 个铜板`}function cn(e,t){let n=new Set([`gj_per`,`mz_per`,`fy_per`,`ds_per`,`zj_per`,`hp_per`,`lianxi_per`,`dazuo_per`,`study_per`,`add_sh_per`,`add_bjsh_per`,`diff_bj`,`diff_sh_per`,`expend_mp_per`,`diff_busy_per`,`gjsd_per`,`diff_fy_per`,`releasetime_per`,`bj_per`,`busy_per`,`diff_downside_per`,`distime_per`]),r=new Set([`gjsd`,`busy`,`diff_busy`,`distime`,`releasetime`]);return n.has(e)?`+`+t+`%`:r.has(e)?`+`+t/1e3+` 秒`:`+`+t}function ln(e){return W(String(e.name||``).replace(/<[^>]+>/g,``))+`×`+e.count}function un(e){return String(e??``).replace(/<[^>]+>/g,``)}var dn={state:null,previewData:null,pickerData:null,error:``,message:``,pending:!1,init:function(){Dialog.injectStyle(fn),this.element=$(`<div class='custom-equipment'></div>`),this.element.on(`click`,`.custom-refresh`,this.refresh.bind(this)),this.element.on(`click`,`.custom-wash`,this.washNow.bind(this)),this.element.on(`click`,`.custom-open-add`,this.openAddPicker.bind(this)),this.element.on(`click`,`.custom-upgrade`,this.previewUpgrade.bind(this)),this.element.on(`click`,`.custom-open-replace`,this.openReplacePicker.bind(this)),this.element.on(`click`,`.custom-open-ability`,this.openAbilityPicker.bind(this)),this.element.on(`click`,`.custom-picker-item`,this.onPickerSelect.bind(this)),this.element.on(`click`,`.custom-picker-cancel`,this.closePicker.bind(this)),this.element.on(`click`,`.custom-rename`,this.previewRename.bind(this)),this.element.on(`click`,`.custom-confirm`,this.commit.bind(this)),this.element.on(`click`,`.custom-preview-cancel`,this.cancelPreview.bind(this))},ensureOpen:function(){(!Dialog.isShow||Dialog.curItem!==`customEquipment`)&&Dialog.select(`customEquipment`),this.show()},show:function(){Dialog.isShow||Dialog.init(),Dialog.element.addClass(`dialog-custom-equipment`),this.element.detach(),Dialog.contentElement.empty().append(this.element),Dialog.footer(``),Dialog.icon(`wrench`),this.isShow=!0,this.render()},hide:function(){Dialog.element&&Dialog.element.removeClass(`dialog-custom-equipment`),this.element&&this.element.detach(),this.isShow=!1},close:function(){this.hide(),this.previewData=null,this.pickerData=null,this.pending=!1},onData:function(e){if(this.pending=!1,e.phase===`error`){this.error=e.message||`自制装备操作失败。`,this.message=``,this.ensureOpen();return}if(this.error=``,e.phase===`preview`){this.previewData=e,this.ensureOpen();return}(e.phase===`state`||e.phase===`result`)&&(this.state=e,this.previewData=null,this.pickerData=null,this.message=e.message||``,this.ensureOpen())},sendPreview:function(e,t){if(this.pending||!this.state)return;this.pending=!0,this.error=``,this.message=``,this.render();let n=t&&t.length?` `+t.join(` `):``;SendCommand(`zizhi preview `+this.state.itemId+` `+e+n)},refresh:function(){this.pending||!this.state||(this.pending=!0,this.render(),SendCommand(`zizhi open `+this.state.itemId))},washNow:function(){this.pending||!this.state||(this.pending=!0,this.error=``,this.message=``,this.render(),SendCommand(`zizhi wash `+this.state.itemId))},openAddPicker:function(){if(this.pending||!this.state)return;let e=[];for(let t of this.state.categories||[])t.remaining>0&&t.available.length&&e.push({name:t.name,hint:`已用 `+t.used+`/`+t.limit,options:t.available});e.length&&(this.pickerData={mode:`add`,title:`添加词条`,subtitle:`选择要添加的词条属性（消耗 1 块对应属性晶石）：`,groups:e},this.render())},openReplacePicker:function(e){if(this.pending||!this.state)return;let t=$(e.currentTarget).attr(`data-key`),n=null;for(let e of this.state.categories||[]){let r=(e.affixes||[]).find(e=>e.key===t);if(r){n=r;break}}!n||!n.replacements||!n.replacements.length||(this.pickerData={mode:`replace`,oldKey:n.key,title:`替换【`+n.name+`】`,subtitle:`选择新的词条属性（消耗 100 两黄金，返还旧词条全部晶石）：`,groups:[{name:``,hint:``,options:n.replacements}]},this.render())},openAbilityPicker:function(){if(this.pending||!this.state)return;let e=this.state.ability||{};!e.unlocked||!e.options||!e.options.length||(this.pickerData={mode:`ability`,title:`设置能力词条`,subtitle:`选择要强化的已学武学，穿戴时有效等级 +1（消耗 1 个元晶）：`,groups:[{name:``,hint:``,options:e.options.map(e=>({key:e.id,name:e.name+`强化`,ability:!0}))}]},this.render())},onPickerSelect:function(e){let t=$(e.currentTarget);if(t.hasClass(`disabled`)||this.pending||!this.pickerData)return;let n=t.attr(`data-key`);if(!n)return;let r=this.pickerData;this.pickerData=null,r.mode===`add`?this.sendPreview(`add`,[n]):r.mode===`replace`?this.sendPreview(`replace`,[r.oldKey,n]):r.mode===`ability`&&this.sendPreview(`ability`,[n])},closePicker:function(){this.pending||(this.pickerData=null,this.render())},previewUpgrade:function(e){this.sendPreview(`upgrade`,[$(e.currentTarget).attr(`data-key`)])},previewRename:function(){let e=String(this.element.find(`.custom-rename-input`).val()||``).trim();e&&this.sendPreview(`rename`,[e])},commit:function(){this.pending||!this.previewData||!this.previewData.token||(this.pending=!0,this.render(),SendCommand(`zizhi commit `+this.previewData.token))},cancelPreview:function(){this.pending||(this.previewData=null,this.render())},render:function(){if(!this.element)return;if(!this.state){this.element.html(`<div class='custom-empty'>`+W(this.error||`正在读取装备状态……`)+`</div>`);return}let e=this.state;Dialog.title((e.plainName||`自制装备`)+` - 重铸`);let t=this.pending||e.locked||e.socketed||e.washCount>=e.maxWashCount,n=[];if(n.push(`<div class='custom-head'>`,`<div class='custom-head-main'><span class='custom-name'>`,e.itemName,`<i>`,W(e.partName),`</i></span>`,`<button type='button' class='custom-icon-button custom-refresh' title='刷新' aria-label='刷新'`,this.pending?` disabled`:``,`><span class='glyphicon glyphicon-refresh'></span></button></div>`,`<div class='custom-meta'>洗练 <b>`,e.washCount,`/`,e.maxWashCount,`</b> · 词条上限 <b>`,e.levelLimit,` 级</b> · 固定`,W(e.fixed.name),` <b>+`,e.fixed.value,`</b></div>`,`<div class='custom-res'><button type='button' class='custom-button custom-primary custom-wash'`,t?` disabled`:``,` title='消耗 1 个元晶，立即洗练'>洗练</button>`,`<span>元晶 `,e.resources.yuanjing,`</span>`,`<span>黄金 `,W(sn(e.resources.money)),`</span>`,`<span>改名符 `,e.resources.rename,`</span></div>`,`</div>`),e.locked&&n.push(`<div class='custom-alert'>装备已锁定，解锁后才能修改。</div>`),e.socketed&&n.push(`<div class='custom-alert'>装备已镶嵌宝石，清理宝石后才能修改。</div>`),this.error&&n.push(`<div class='custom-alert custom-error'>`,W(un(this.error)),`</div>`),this.message&&n.push(`<div class='custom-success'>`,W(un(this.message)),`</div>`),this.pending&&n.push(`<div class='custom-pending'>正在核对装备和材料……</div>`),n.push(`<div class='custom-scroll'>`),n.push(this.renderAffixes(e)),n.push(this.renderRename(e)),n.push(`</div>`),this.pickerData&&n.push(this.renderPicker(this.pickerData)),this.previewData&&n.push(this.renderPreview(this.previewData)),this.element.html(n.join(``)),e.focus===`rename`){let t=this.element.find(`.custom-scroll`)[0];t&&(t.scrollTop=t.scrollHeight),e.focus=``}},renderAffixes:function(e){let t=this.pending||e.locked||e.socketed,n=e.categories||[],r=0,i=0,a=!1;for(let e of n)r+=e.used,i+=e.limit,e.remaining>0&&e.available.length&&(a=!0);let o=[];o.push(`<section class='custom-affixes'><div class='custom-affixes-head'><h3>词条 <span>`,r,`/`,i,`</span></h3>`,`<button type='button' class='custom-button custom-open-add'`,t||!a?` disabled`:``,`>+ 添加词条</button></div>`);let s=!0;for(let e of n)if(e.affixes.length){s=!1,o.push(`<div class='custom-group'><div class='custom-group-head'>`,W(e.name),`<span>`,e.used,`/`,e.limit,`</span></div>`);for(let n of e.affixes)o.push(this.renderAffixRow(n,t));o.push(`</div>`)}return s&&o.push(`<div class='custom-none'>暂无词条，点击右上方添加。</div>`),o.push(this.renderAbility(e)),o.push(`</section>`),o.join(``)},renderAffixRow:function(e,t){let n=e.material||{},r=e.canUpgrade?`升至 `+(e.level+1)+` 级，消耗 `+e.nextCost+` 块`+n.name+`（持有 `+n.count+`）`:`已达当前洗练次数的升级上限`,i=[];return i.push(`<div class='custom-affix-row'><div class='custom-affix-info'><strong>`,W(e.name),`</strong><em>`,W(cn(e.key,e.value)),`</em><span>`,e.level,`/`,e.levelLimit,` 级`,e.legacy?` · 旧`:``,`</span></div><div class='custom-affix-actions'>`),i.push(`<button type='button' class='custom-button custom-upgrade' data-key='`,e.key,`' title='`,W(r),`'`,t||!e.canUpgrade?` disabled`:``,`>升级</button>`),e.replacements&&e.replacements.length&&i.push(`<button type='button' class='custom-button custom-open-replace' data-key='`,e.key,`' title='消耗 100 两黄金，返还旧词条全部晶石'`,t?` disabled`:``,`>替换</button>`),i.push(`</div></div>`),i.join(``)},renderAbility:function(e){let t=e.ability||{};if(!t.unlocked)return`<div class='custom-ability-tip'>能力词条：洗练 25 次后解锁</div>`;let n=this.pending||e.locked||e.socketed,r=[];return r.push(`<div class='custom-group custom-group-ability'><div class='custom-group-head'>能力词条<span>`,t.skillId?`1/1`:`0/1`,`</span></div>`),t.skillId?r.push(`<div class='custom-affix-row'><div class='custom-affix-info'><strong>`,W(t.name),`强化</strong><em>有效等级 +1</em></div><div class='custom-affix-actions'>`,`<button type='button' class='custom-button custom-open-ability'`,n?` disabled`:``,`>更换</button></div></div>`):t.options&&t.options.length?r.push(`<div class='custom-affix-row'><div class='custom-affix-info'><strong>未设置</strong>`,`<em>穿戴时武学有效等级 +1</em></div><div class='custom-affix-actions'>`,`<button type='button' class='custom-button custom-open-ability'`,n?` disabled`:``,`>设置</button></div></div>`):r.push(`<div class='custom-none'>当前没有与该部位匹配的已学武学</div>`),r.push(`</div>`),r.join(``)},renderRename:function(e){let t=this.pending||e.locked||e.socketed||e.resources.rename<1;return`<div class='custom-rename-row'><span>改名</span><input class='custom-input custom-rename-input' maxlength='5' value='`+W(e.plainName)+`'`+(t?` disabled`:``)+`><button type='button' class='custom-button custom-rename'`+(t?` disabled`:``)+`>确定</button></div>`},renderPicker:function(e){let t=[];t.push(`<div class='custom-picker-mask'><div class='custom-picker-dialog'>`),t.push(`<div class='custom-picker-header'><h3>`,W(e.title),`</h3>`),t.push(`<button type='button' class='custom-icon-button custom-picker-cancel' aria-label='关闭'>✕</button></div>`),t.push(`<div class='custom-picker-subtitle'>`,W(e.subtitle||``),`</div>`),t.push(`<div class='custom-picker-list'>`);for(let n of e.groups||[]){n.name&&t.push(`<div class='custom-picker-group'>`,W(n.name),n.hint?`<span>`+W(n.hint)+`</span>`:``,`</div>`);for(let e of n.options)t.push(this.renderPickerItem(e))}return t.push(`</div>`),t.push(`<div class='custom-picker-footer'><button type='button' class='custom-button custom-picker-cancel'>取消</button></div>`),t.push(`</div></div>`),t.join(``)},renderPickerItem:function(e){if(e.ability)return`<div class='custom-picker-item' data-key='`+e.key+`'><div class='custom-picker-item-main'><div class='custom-picker-item-title'><strong>`+W(e.name)+`</strong></div><div class='custom-picker-item-mat'>穿戴时该武学有效等级 +1 · 消耗 1 个元晶</div></div><button type='button' class='custom-button custom-primary'>选择</button></div>`;let t=e.material&&e.material.count||0,n=t>=1;return`<div class='custom-picker-item`+(n?``:` disabled`)+`' data-key='`+e.key+`'><div class='custom-picker-item-main'><div class='custom-picker-item-title'><strong>`+W(e.name)+`</strong><span class='custom-picker-item-val'>`+W(cn(e.key,e.value))+`</span></div><div class='custom-picker-item-mat'>材料：`+W(e.material?e.material.name:`晶石`)+`（持有 <span class='`+(n?`text-success`:`text-danger`)+`'>`+t+`</span> 块）</div></div><button type='button' class='custom-button`+(n?` custom-primary`:``)+`'`+(n?``:` disabled`)+`>选择</button></div>`},renderPreview:function(e){let t=(e.costs||[]).map(ln);e.money&&t.push(W(sn(e.money)));let n=(e.refunds||[]).map(ln);return`<div class='custom-preview'><div class='custom-preview-dialog'><h3>`+W(e.operationName||`确认操作`)+`</h3><p>`+W(e.summary)+`</p><dl><dt>消耗</dt><dd>`+(t.join(`、`)||`无`)+`</dd>`+(n.length?`<dt>返还</dt><dd>`+n.join(`、`)+`</dd>`:``)+`</dl><div class='custom-preview-actions'><button type='button' class='custom-button custom-preview-cancel'`+(this.pending?` disabled`:``)+`>取消</button><button type='button' class='custom-button custom-primary custom-confirm'`+(this.pending?` disabled`:``)+`>确认</button></div></div></div>`}},fn=`
...
`;function mn(e,t){return e&&e[t]?e[t]:0}function hn(e){if(!e||e<=0)return``;let t=Math.floor(e/6e4),n=Math.floor(t/60),r=t%60;return n>0?n+`小时`+r+`分`:r+`分钟`}var gn={running:`运行中`,paused:`已暂停`,awaiting_collect:`待领取`,stopped:`已停止`,completed:`已完成`,failed:`失败`},_n={offered:`待接取`,accepted:`已接取`,completed:`已完成`,expired:`已过期`,claimed:`已交付`},vn={ore:`矿石`,herb:`药材`,pill:`丹药`,tuition:`学费`,tool:`工具`,seed:`种子`,宣传费:`宣传费`};function yn(e){return vn[e]||e}function bn(e,t){let n=t&&t[e];return n&&n.name||e||``}function xn(e,t,n){let r=t&&t[e];return!r||!r.period?``:Math.round(r.period*n/36e5*10)/10}function Sn(e,t){if(t&&t.name)return t.name;if(typeof Process<`u`&&Process.cur_room&&Process.cur_room.items){let t=Process.cur_room.items.find(function(t){return t&&t.id===e});if(t)return t.name||e}return e}function Cn(e,t){return({main:`主屋`,warehouse:`仓库`,mine:`矿场`,garden:`药圃`,alchemy:`炼药房`,school:`武馆`,training:`练功房`,rest:`休息设施`}[e]||e)+` Lv.`+(t&&t.level||0)+` / 耐久`+(t&&t.durability||0)}var wn={init:function(){Dialog.injectStyle(pn)},createElement:function(){return $(`<div class='dialog-household'></div>`)},show:function(){this.element||=this.createElement(),this.element.appendTo(Dialog.contentElement),this.isShow=!0,Dialog.title(`武道家族`),Dialog.icon(`home`),SendCommand(`household view`)},close:function(){this.element&&this.element.detach(),this.isShow=!1},hide:function(){this.close()},onData:function(e){let t=e.data||e;t&&(Dialog.curItem!==`household`&&Dialog.select(`household`),this.element||=this.createElement(),this.element.parent()[0]!==Dialog.contentElement[0]&&Dialog.contentElement.empty().append(this.element),Dialog.title(`武道家族`),Dialog.icon(`home`),this.isShow=!0,this.data=t,this.render())},render:function(){let e=this.data||{},t=e.facilities||{},n=e.members||{},r=e.tasks||{},i=e.warehouse||{},a=e.jobDefs||{},o=e.defaultCycles||2,s=[];s.push(`<div class='household-summary'>`),s.push(`<strong>住宅等级 `,e.level||0,`</strong>　成员位 `,e.memberSlots||0,`　仓库容量 `,e.capacity||0),s.push(`<br>家族资金 `,e.funds||0,`两　声望 `,e.reputation||0,`　待维护 `,e.upkeepDue||0),s.push(`<div class='household-actions'><span cmd='household upgrade 住宅'>扩建住宅</span><span cmd='household collect'>领取全部产出</span><span cmd='household upgrade 仓库'>升级仓库</span><span cmd='household pay'>支付维护费</span><span cmd='household playertrain'>练功房训练</span></div></div>`),s.push(`<div class='household-section'><div class='household-section-title'>设施</div><div class='household-grid'>`),Object.keys(t).forEach(function(e){s.push(`<div class='household-cell'>`,Cn(e,t[e]),`</div>`)}),s.push(`</div><div class='household-actions'><span cmd='household upgrade 矿场'>升级矿场</span><span cmd='household upgrade 药圃'>升级药圃</span><span cmd='household upgrade 炼药房'>升级炼药房</span><span cmd='household upgrade 武馆'>升级武馆</span></div></div>`),s.push(`<div class='household-section'><div class='household-section-title'>家族成员</div>`);let c=Object.keys(n);c.length||s.push(`<div class='household-muted'>暂无成员；已有随从会在登录后自动加入家族资料。</div>`),c.forEach(function(e){let t=n[e]||{},r=hn(t.remainMs);s.push(`<div class='household-member'><strong>`,Sn(e,t),`</strong>　`,t.statusText||t.job||`待命`,r?`　剩余`+r:``,`　职业Lv.`,t.jobLevel||1,`<br>资质 `,t.aptitude||0,`　疲劳 `,t.fatigue||0,`　心情 `,t.mood||0,`　忠诚 `,t.loyalty||0),s.push(`<div class='household-actions'>`),[[`mining`,`挖矿`],[`planting`,`种植`],[`alchemy`,`炼药`],[`teaching`,`授课`]].forEach(function(t){s.push(`<span cmd='household assign `,e,` `,t[0],` `,o,`'>`,t[1],`(`,xn(t[0],a,o),`小时)</span>`)}),s.push(`<span cmd='household stop `,e,`'>停止</span><span cmd='household rest `,e,`'>休息</span><span cmd='household train `,e,`'>培训</span></div></div>`)}),s.push(`</div><div class='household-section'><div class='household-section-title'>生产任务</div>`);let l=Object.keys(r);l.length||s.push(`<div class='household-muted'>暂无生产任务，给家族成员派工后即可开始生产。</div>`),l.forEach(function(e){let t=r[e]||{},i=t.pendingOutput||{},o=Object.keys(i).map(function(e){return yn(e)+` x`+i[e]}).join(`、`)||`暂无待领取产出`,c=t.memberId&&n[t.memberId],l=hn(t.remainMs);s.push(`<div class='household-task'><strong>`,Sn(t.memberId,c),` `,bn(t.job,a),`</strong>　周期 `,t.cycles||0,`/`,t.plannedCycles||`-`,`　状态 `,gn[t.status]||t.status||``,l?`　剩余`+l:``,t.reason?`<br>说明：`+t.reason:``,`<br>待领取：`,o,`</div>`)}),s.push(`</div><div class='household-section'><div class='household-section-title'>家族事件</div>`);let u=Array.isArray(e.events)?e.events:[];u.length||s.push(`<div class='household-muted'>暂无待处理事件。</div>`),u.forEach(function(e){e&&(s.push(`<div class='household-task'><strong>`,e.type||`经营事件`,`</strong>　`,e.handled?e.result||`已处理`:`待处理`),e.handled||s.push(`<div class='household-actions'><span cmd='household event `,e.id,` continue'>维持现状</span><span cmd='household event `,e.id,` invest'>投入资源</span><span cmd='household event `,e.id,` rest'>暂停岗位</span></div>`),s.push(`</div>`))}),s.push(`</div><div class='household-section'><div class='household-section-title'>订单</div>`);let d=Array.isArray(e.orders)?e.orders:[];d.length||s.push(`<div class='household-muted'>暂无订单，持续经营后会有新订单。</div>`),d.forEach(function(e){e&&(s.push(`<div class='household-task'><strong>`,yn(e.product)||`订单`,` x`,e.count||0,`</strong>　奖励 `,e.rewardFunds||0,`两 / 声望`,e.rewardReputation||0,`　状态 `,_n[e.status]||e.status||``),e.status===`offered`&&s.push(`<div class='household-actions'><span cmd='household order accept `,e.id,`'>接取订单</span></div>`),e.status===`accepted`&&s.push(`<div class='household-actions'><span cmd='household order complete `,e.id,`'>交付订单</span></div>`),s.push(`</div>`))}),s.push(`</div><div class='household-section'><div class='household-section-title'>成就</div>`);let f=e.achievementDefs||{},p=e.achievements||{},m=Object.keys(f);m.length||s.push(`<div class='household-muted'>暂无成就记录。</div>`),m.forEach(function(e){let t=f[e]||{},n=p[e]||{};s.push(`<div class='household-task'><strong>`,t.name||e,`</strong>　进度 `,n.progress||0,`/`,t.target||0,`　`),n.completedAt&&!n.claimed?s.push(`<span cmd='household achievement claim `,e,`'>领取奖励</span>`):s.push(n.claimed?`已领取`:`进行中`),s.push(`</div>`)});let h=e.prices||{};s.push(`</div><div class='household-section'><div class='household-section-title'>仓库</div><div class='household-grid'>`),Object.keys(i.products||{}).forEach(function(t){let n=mn(i.products,t),r=t===`ore`&&e.oreTierName&&e.oreTierName!==`铁矿石`?`　领取为`+e.oreTierName:``;s.push(`<div class='household-cell'>`,yn(t),` x`,n,h.sell&&h.sell[t]?`　`+h.sell[t]+`两/份`:``,r);let a=[];if([`ore`,`herb`,`pill`,`tuition`].indexOf(t)>=0&&a.push(`<span cmd='household withdraw `,t,` 1'>领取1份</span>`),h.sell&&h.sell[t]&&n>0){let e=Math.min(5,n);a.push(`<span cmd='household sell `,t,` `,e,`'>出售`,e,`份</span>`)}a.length&&s.push(`<div class='household-actions'>`,a.join(``),`</div>`),s.push(`</div>`)}),Object.keys(i.materials||{}).forEach(function(e){s.push(`<div class='household-cell'>`,yn(e),` x`,mn(i.materials,e),h.buy&&h.buy[e]?`　`+h.buy[e]+`两/份`:``),h.buy&&h.buy[e]&&s.push(`<div class='household-actions'><span cmd='household buy `,e,` 5'>购5份</span></div>`),s.push(`</div>`)}),!Object.keys(i.products||{}).length&&!Object.keys(i.materials||{}).length&&s.push(`<div class='household-muted'>仓库暂时为空。</div>`),s.push(`</div></div>`),this.element.html(s.join(``)),Dialog.footer(``)}},Tn=`
.dialog,
...
`}showServers(){if(!J){k(`正在获取服务器列表`),q.GetServer(e=>{if(!e||typeof e==`string`){D(`#login_pwd`,`获取服务器列表出错`);return}J=e,this.displayServer(e),this.showServers()});return}var e=J;if(!e||!e.length)O(`#login_panel`),D(`#login_pwd`,`获取服务器列表出错`);else{var t=b.GetUserCookie(`s`),n=t?J[t]:e.length==1?J[0]:null;if(n)return k(`正在连接服务器`),me(n);O(`#slist_panel`)}}selectServer(){if(J){var e=parseInt($(`.server-list>.select`).attr(`index`));if(!(e>=0&&e<J.length))return x.Show({content:`你没有选择要连接的服务器。`});var t=J[e];t||x.Show({content:`你没有选择要连接的服务器。`}),k(`正在连接服务器`),me(t),b.SetCookie(`s`,e)}}displayServer(){if(J){var e=location.hostname.startsWith(`127.0.0.1`)||location.hostname.startsWith(`localhost`),t=location.search.startsWith(`?test`);e&&J.push({id:100,name:`本地测试1`,ip:`127.0.0.1`,port:31300});for(var n=[],r=`武神传说2`,i=0;i<J.length;i++)!t&&!e&&J[i].istest||(n.push(`<li class='role-item`),i==0&&n.push(` select`),n.push(`' index='`+i+`'>`),n.push(r),n.push(`&nbsp;&nbsp;`),n.push(J[i].name),J[i].isdef&&n.push(`<span class='server-badge'>&nbsp;（推荐）</span>`),n.push(`</li>`));$(`.server-list`).html(n.join(``)).on(`click`,`li`,function(){var e=$(this);e.is(`.select`)||(e.parent().find(`.select`).removeClass(`select`),e.addClass(`select`))})}}},On=class extends K{constructor(){super(),this.initReg=!1,this.template=`
...
`}on_mount(){}open(){this.initReg||=(this.GetValidationImage(),$(`.validnum-box>.validnum-img`).on(`click`,()=>this.GetValidationImage()),!0)}regist(){var e=$(`#regist_name`).val().toLowerCase(),t=$(`#regist_pwd1`).val();if(!e)return D(`#regist_name`,`请输入用户名`);if(!/^[a-z0-9]{5,15}$/.test(e))return D(`#regist_name`,`用户名需要是5-10个英文字符`);if(!t)return D(`#regist_pwd1`,`请输入密码`);if(t.length<6||t.length>20)return D(`#regist_pwd1`,`密码长度在6到20之间`);if(t!=$(`#regist_pwd2`).val())return D(`#regist_pwd2`,`重复密码输入不一致，请重新输入`);var n=$(`#regist_val`).val();if(!n)return D(`#regist_valpanel`,`请输入图片中的验证码`);if(n.length!=4)return D(`#regist_valpanel`,`请输入图片中的四位验证码`);let r=0,i=/u(\d+)/.exec(location.pathname);i&&(r=parseInt(i[1]),r>0||(r=0)),k(`正在注册账号`),q.Regist({name:e,pwd:t,valno:n,guider:r},e=>{e.code==1?(k(`注册成功，正在获取服务器列表`),setTimeout(()=>window.location.reload(),500)):(D(`#regist_name`,e.result||`注册失败`),O($(`#regist_panel`)))})}GetValidationImage(){q.ValidationImage(function(e){$(`.validnum-box>.validnum-img`).attr(`src`,`data:image/svg+xml;base64,`+e)})}},kn=`万俟司马上官欧阳夏侯诸葛闻人东方赫连皇甫尉迟公羊澹台公冶宗政濮阳淳于单于太叔申屠公孙仲孙轩辕令狐锺离宇文长孙慕容鲜于闾丘司徒司空丌官司寇子车颛孙端木巫马公西乐正公良拓拔夹谷谷梁梁丘左丘东门西门`,An=`赵钱孙李周吴郑王冯陈楮卫蒋沈韩杨朱秦尤许何吕施张孔曹严华金魏陶姜戚谢邹喻柏水窦章云苏潘葛奚范彭郎`,jn=`世舜丞主产仁仇仓仕仞任伋众伸佐佺侃侪促俟信俣修倝倡倧偿储僖僧僳儒俊伟列则刚创前剑助劭势勘参叔吏嗣士壮孺守宽宾宋宗宙宣实宰尊峙峻崇崈川州巡帅庚战才承拯操斋昌晁暠曹曾珺玮珹琒琛琩琮琸瑎玚璟璥瑜生畴矗矢石磊砂碫示社祖祚祥禅稹穆竣竦综缜绪舱舷船蚩襦轼辑轩子杰榜碧葆莱蒲天乐东钢铎铖铠铸铿锋镇键镰馗旭骏骢骥驹驾骄诚诤赐慕端征坚建弓强彦御悍擎攀旷昂晷健冀凯劻啸柴木林森朴骞寒函高魁魏鲛鲲鹰丕乒候冕勰备宪宾密封山峰弼彪彭旁日明昪昴胜汉涵汗浩涛淏清澜浦澉澎澔瀚瀛灏沧虚豪豹辅辈迈邶合部阔雄霆震韩俯颁颇频颔风飒飙飚马亮仑仝代儋利力劼勒卓哲喆展帝弛弢弩彰征律德志忠思振挺掣旲旻昊昮晋晟晸朕朗段殿泰滕炅炜煜煊炎选玄勇君稼黎利贤谊金鑫辉墨欧有友闻问`,Mn=`筠柔竹霭凝晓欢霄枫芸菲寒伊亚宜姬舒影荔枝思丽秀娟英华慧巧美娜静淑惠珠翠雅芝玉萍红娥玲芬芳燕彩春菊勤珍贞莉兰凤洁梅琳素云莲真环雪荣妹霞香月莺媛艳瑞凡佳嘉琼桂娣叶璧璐娅琦晶妍茜秋珊莎锦黛青倩婷姣婉娴瑾颖露瑶怡婵雁蓓纨仪荷丹蓉眉君琴蕊薇菁梦岚苑婕馨瑗琰韵融园艺咏卿聪澜纯毓悦昭冰爽琬茗羽希宁欣飘育滢馥`;function Nn(e,t){t||=parseInt(Math.random()*2)+1;var n=[];if(t==2){var r=parseInt(Math.random()*kn.length);r%2==1&&--r,n.push(kn[r++]),n.push(kn[r])}else n.push(An[parseInt(Math.random()*An.length)]);return e==0?n.push(jn[parseInt(Math.random()*jn.length)]):n.push(Mn[parseInt(Math.random()*Mn.length)]),parseInt(Math.random()*4)>1&&(e==0?n.push(jn[parseInt(Math.random()*jn.length)]):n.push(Mn[parseInt(Math.random()*Mn.length)])),n.join(``)}function Pn(){for(var e=`abcdefghijklmnopqrstuvwxyz`,t=`123456789`,n=[],r=parseInt(Math.random()*3)+3,i=0;i<r;i++)i<3?n.push(e[parseInt(Math.random()*e.length)]):n.push(t[parseInt(Math.random()*t.length)]);return n.join(``)}function Fn(){for(var e=20,t=[],n=0;n<4;n++){var r=parseInt(Math.random()*15+1);e>=r?(n==3?r=e:e-=r,t[n]=r):(t[n]=e,e=0)}var i={};return i.str=t[0]+15,i.con=t[1]+15,i.dex=t[2]+15,i.int=t[3]+15,i}function In(e){switch(e){case`name`:$(`#reg_name`).val(Nn(+!$(`#gender_0`).is(`:checked`)));break;case`id`:$(`#reg_id`).val(Pn());break;case`prop`:var t=Fn();$(`#reg_str`).val(t.str),$(`#reg_con`).val(t.con),$(`#reg_dex`).val(t.dex),$(`#reg_int`).val(t.int);break}}var Ln=class extends K{constructor(){super(),this.template=`
...
`}select(){var e=$(`.role-list>.select`);if(e.length){var t=e.attr(`roleid`);SendCommand(`login `+t),k(`正在进入游戏`,`#role_panel`)}}addRole(){if($(`.role-list>.role-item`).length>4)return x.Show({content:`你只能最多创建五个角色`});O($(`#addrole_panel`)),In(`name`),In(`prop`),In(`id`)}delete(){var e=$(`.role-list>.select`);if(e.length){var t=e.attr(`roleid`);t&&x.Show({content:`是否确认删除角色：`+e.html(),onOK:function(){SendCommand(`deleterole `+t)}})}}},Rn=class extends K{constructor(){super(),this.template=`
...
`}on_mount(){}reset(){var e=$(`#reset_name`).val();if(!e)return D(`#reset_name`,`请输入用户名`);if(!/^[a-z0-9]{5,15}$/.test(e))return D(`#reset_name`,`用户名格式错误,需要5-15位字母开头的字母，数字或下划线，不区分大小写`);var t=$(`#reset_phone`).val();if(!t)return D(`#reset_phone`,`请输入你的帐号绑定的手机号码`);if(!/^1\d{10}$/.test(t))return D(`#reset_phone`,`手机号码格式错误`);var n=``,r=$(`#reset_pwd1`).val();if(!r)return D(`#reset_pwd1`,`请输入你的新密码`);var i=$(`#reset_pwd2`).val();if(!i)return D(`#reset_pwd2`,`请重复输入你的新密码`);if(i.length<6||i.length>20)return D(`#update_pwd2`,`密码长度在6到20之间`);if(i!=r)return D(`#reset_pwd2`,`两次密码输入不一致`);k(`正在修改密码`,`#reset_panel`),q.ResetPasswordByPhone(e,t,n,r,function(e){e.code?O(`#login_panel`):(D(`#reset_pwd2`,e.result??`重置失败`),O(`#reset_panel`))})}},zn=class extends K{constructor(){super(),this.template=`
...
`}open(){O(`#pwd_panel`),q.GetPhone(function(e){if(e.code!==1)return D(`#update_pwd1`,`获取绑定的手机号失败`);e.result?($(`#pwd_phone`).prop(`disabled`,!0).val(e.result),$(`#pwd_bind`).show()):($(`#pwd_phone`).prop(`disabled`,!1).val(``),$(`#pwd_bind`).hide())})}update(){$(`#pwd_panel`).find(`.input-error`).remove();var e=$(`#update_pwd1`).val(),t=$(`#update_pwd2`).val(),n=$(`#update_pwd3`).val();if(e.length<6||e.length>20)return D(`#update_pwd1`,`密码长度在6到20之间`);if(t.length<6||t.length>20)return D(`#update_pwd2`,`密码长度在6到20之间`);if(n!=t)return D(`#update_pwd3`,`两次密码输入不一致`);var r;if($(`#pwd_bind`).is(`:visible`)&&(r=$(`#pwd_no`).val(),!r||!/^\d{4}$/.test(r)))return D($(`#pwd_no`).parent(),`请输入你绑定的手机尾号`);k(`正在修改密码`,`#pwd_panel`),q.ChangePassword(e,t,r,function(e){e.code?O($(`#slist_panel`)):(D(`#update_pwd1`,e.result||`修改失败`),O(`#pwd_panel`))})}},Bn=class extends K{constructor(){super(),this.template=`
...
`}create(){var e={};if(e.name=$(`#reg_name`).val(),e.gender=$(`#gender_0`).is(`:checked`)?1:2,e.str=parseInt($(`#reg_str`).val()),e.con=parseInt($(`#reg_con`).val()),e.dex=parseInt($(`#reg_dex`).val()),e.int=parseInt($(`#reg_int`).val()),!/^[\u4E00-\u9FA5]{2,5}$/.test(e.name))return D(`#reg_name`,`名称格式错误，只能使用2-5位中文字符`);if(e.str<15||e.str>30)return D(`#reg_name`,`臂力需要在15-30之间`);if(e.con<15||e.con>30)return D(`#reg_name`,`根骨需要在15-30之间`);if(e.dex<15||e.dex>30)return D(`#reg_name`,`身法需要在15-30之间`);if(e.int<15||e.int>30)return D(`#reg_name`,`悟性需要在15-30之间`);if(e.str+e.con+e.dex+e.int!=80)return D(`#reg_name`,`先天属性需要在15-30之间，并且总和等于80`);k(`正在创建角色`,`#addrole_panel`),SendCommand(`createrole `+e.name+` `+e.gender+` `+e.str+` `+e.con+` `+e.dex+` `+e.int)}},Hn=class extends K{constructor(){super(),this.template=`
...
`}relogin(){O($(`#login_panel`));var e=new Date;e.setTime(-1e3);for(var t=document.cookie.split(`; `),n=0;n<t.length;n++){var r=t[n].split(`=`);document.cookie=r[0]+`=''; expires=`+e.toGMTString()}}loginIn(){var e=$(`#login_name`).val().toLowerCase(),t=$(`#login_pwd`).val();if(!e)return D(`#login_name`,`请输入用户名`);if(!/^[a-z0-9]{5,15}$/.test(e))return D(`#login_name`,`用户名格式错误,需要5-15位字母开头的字母，数字或下划线，不区分大小写`);if(!t)return D(`#login_pwd`,`请输入密码`);if(t.length<6||t.length>20)return D(`#login_pwd`,`密码长度在6到20之间`);k(`正在登录`,`#login_panel`),q.Login(e,t,e=>{e.code?Wn.showServers():(D(`#login_name`,e.result||`登陆失败`),O(`#login_panel`))})}},Kn=new Dn,qn=new Gn,Jn=new On,Y=new Ln,Yn=new Rn,Xn=new zn,Zn=new Bn,Qn=new Vn,$n=new Hn;function er(){let e=$(this).attr(`nid`);O($(`#new_panel `)),$(`#news_frame`).attr(`src`,`/news/`+e+`.html`)}var tr=class extends K{constructor(){super(),this.template=`
...
`}on_mount(){if($(`.login-content`).on(`click`,`.panel_item`,e=>this.LoginCommand(e)),$(`.role-list`).on(`click`,`.role-item`,function(){$(this).parent().find(`.select`).removeClass(`select`),$(this).addClass(`select`)}),$(`.new-list>li`).on(`click`,er),!b.GetUserCookie(`p`))return $(`#login_panel`).show();Kn.showServers()}LoginCommand(e){switch($(e.currentTarget).attr(`command`)){case`ToRolePanel`:O($(`#role_panel`));break;case`ToServerPanel`:ve(),O($(`#slist_panel`));break;case`ToLogin`:O($(`#login_panel`));break;case`Forget`:O($(`#reset_panel`));break;case`CancleRegist`:O($(`#login_panel`));break;case`Down`:O($(`#download`));break;case`ToRegist`:O($(`#regist_panel`)),Jn.open();break;case`Regist`:Jn.regist();break;case`SelectServer`:Kn.selectServer();break;case`LoginIn`:qn.loginIn();break;case`ResetPwd`:Yn.reset();break;case`AddRole`:Y.addRole();break;case`SelectRole`:Y.select();break;case`CreateRole`:Qn.create();break;case`BindPhone`:Zn.bind();break;case`CheckValid`:Zn.check();break;case`UpdatePwd`:Xn.update();break;case`ToUpdate`:Xn.open();break;case`ReLogin`:qn.relogin();break;case`DeleteRole`:Y.delete();break}}},nr={rally:`集结准备`,normal:`常规战斗`,side:`副位机制`,aspect:`四象化劫`,vulnerability:`逆脉易伤`,enrage:`七灭归劫`,terminal:`战斗终结`},rr={rally:`phase-rally`,normal:`phase-normal`,side:`phase-side`,aspect:`phase-aspect`,vulnerability:`phase-vulnerability`,enrage:`phase-enrage`,terminal:`phase-terminal`},ir=[`东`,`南`,`西`,`北`];function X(e){return e?String(e).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`).replace(/'/g,`&#39;`):``}var ar={currentEventId:null,isTest:!1,stateData:null,playerId:null,expanded:!1,raidMessages:[],raidMembers:[],timerHandle:null,serverTimeOffset:0,domMounted:!1,getPlayerId(e){let t=e=>{if(e==null)return null;if(typeof e==`object`)return e.id!==void 0&&e.id!==null?String(e.id):e.playerId!==void 0&&e.playerId!==null?String(e.playerId):e.userid!==void 0&&e.userid!==null?String(e.userid):e.user_id!==void 0&&e.user_id!==null?String(e.user_id):null;let t=String(e).trim();return t.length>0?t:null};if(e!=null){let n=t(e);if(n)return n}if(typeof Process<`u`&&Process.player){let e=t(Process.player);if(e)return e}return t(this.playerId)},init(){this.domMounted||=(this.bindEvents(),!0)},bindEvents(){let e=$(`.container`);e.length&&(e.off(`.qimie`),e.on(`click.qimie`,`.qimie-bar-toggle, .qimie-summary-clickable`,e=>{e.stopPropagation(),this.toggleExpand()}),e.on(`click.qimie`,`.qimie-raid-btn`,e=>{e.stopPropagation(),this.openRaidChat()}))},toggleExpand(){this.expanded=!this.expanded;let e=$(`.qimie-status-bar`);if(e.length){e.toggleClass(`expanded`,this.expanded);let t=e.find(`.qimie-bar-toggle .glyphicon`);this.expanded?(t.removeClass(`glyphicon-chevron-down`).addClass(`glyphicon-chevron-up`),e.find(`.qimie-bar-toggle .toggle-text`).text(`收起`)):(t.removeClass(`glyphicon-chevron-up`).addClass(`glyphicon-chevron-down`),e.find(`.qimie-bar-toggle .toggle-text`).text(`详情`))}},openRaidChat(){let e=$(`.chat-panel`);if(e.length){e.removeClass(`hide`);let t=e.find(`.channel-box`);t.find(`span`).removeClass(`selected`);let n=t.find(`span[channel="raid"]`);if(!n.length){let e=t.find(`span[channel="pty"]`);n=e.length?$(`<span channel="raid">战团</span>`).insertAfter(e):$(`<span channel="raid">战团</span>`).appendTo(t)}n.addClass(`selected`),t.attr(`channel`,`raid`);let r=e.find(`.sender-box`);r.length&&r.val(``).focus()}else typeof T==`function`&&T(`raid `)},onBossEvent(e,t){if(e){if(t?this.playerId=t:typeof Process<`u`&&Process.player&&(this.playerId=Process.player),this.init(),e.action===`clear`){(!this.currentEventId||!e.eventId||this.currentEventId===e.eventId)&&this.clear();return}e.eventId&&this.currentEventId&&this.currentEventId!==e.eventId&&(this.clear(),t?this.playerId=t:typeof Process<`u`&&Process.player&&(this.playerId=Process.player),this.init()),this.currentEventId=e.eventId||this.currentEventId,this.isTest=!!(this.currentEventId&&String(this.currentEventId).indexOf(`test`)>=0),typeof e.serverNow==`number`&&!isNaN(e.serverNow)&&e.serverNow>0&&(this.serverTimeOffset=e.serverNow-Date.now()),this.stateData=Object.assign({},this.stateData||{},e),this.render(),this.startTimer()}},onRaid(e){if(e){if(this.init(),e.action===`close`){(!this.currentEventId||!e.eventId||this.currentEventId===e.eventId)&&(this.raidMessages=[],this.raidMembers=[],this.renderRaidSection());return}if(e.eventId&&this.currentEventId&&this.currentEventId!==e.eventId&&(this.raidMessages=[],this.raidMembers=[]),e.eventId&&(this.currentEventId=e.eventId),e.action===`state`)Array.isArray(e.members)?this.raidMembers=e.members:e.members&&typeof e.members==`object`&&(this.raidMembers=Object.values(e.members)),e.messages&&Array.isArray(e.messages)&&(this.raidMessages=e.messages.slice(-20).map(e=>({id:e.id||e.playerId||e.senderId,name:e.name||e.senderName||`侠士`,content:e.content||e.message||``,time:e.time||e.at||Date.now()}))),this.renderRaidSection();else if(e.action===`message`||e.content||e.message){let t={id:e.id||e.playerId||e.senderId,name:e.name||e.senderName||`侠士`,content:e.content||e.message||``,time:e.time||e.at||Date.now()};this.raidMessages.push(t),this.raidMessages.length>20&&this.raidMessages.splice(0,this.raidMessages.length-20),this.renderRaidSection()}}},clear(){this.timerHandle&&=(clearInterval(this.timerHandle),null),this.currentEventId=null,this.stateData=null,this.raidMessages=[],this.raidMembers=[],this.isTest=!1,this.playerId=null,this.serverTimeOffset=0,$(`.qimie-status-bar`).remove()},startTimer(){this.timerHandle||=setInterval(()=>{this.updateCountdown()},1e3)},getNow(){return Date.now()+(this.serverTimeOffset||0)},updateCountdown(){if(!this.stateData)return;let e=this.getNow(),t=Number(this.stateData.expiresAt);if(!isNaN(t)&&t>0){let n=Math.max(0,Math.ceil((t-e)/1e3)),r=Math.floor(n/60),i=n%60,a=(r<10?`0`+r:r)+`:`+(i<10?`0`+i:i);$(`.qimie-timer-val`).text(a)}else $(`.qimie-timer-val`).text(`--:--`);let n=this.stateData.mechanic,r=n?Number(n.expiresAt):NaN;if(!isNaN(r)&&r>0){let t=Math.max(0,Math.ceil((r-e)/1e3));$(`.qimie-mech-timer`).text(t+`s`)}else $(`.qimie-mech-timer`).text(``)},render(){if(!this.stateData)return;let e=$(`.qimie-status-bar`);if(!e.length){e=$(`<div class="qimie-status-bar"></div>`);let t=$(`.content-message`);t.length?e.insertBefore(t):$(`.container`).append(e)}let t=this.stateData,n=t.phase||`rally`,r=(this.isTest?`【测试】`:``)+(nr[n]||n),i=rr[n]||`phase-normal`,a=t.stacks||0,o=a>=6?`stacks-danger`:a>=3?`stacks-warn`:`stacks-normal`,s=Math.max(0,Number(t.bossHp)||0),c=Math.max(1,Number(t.bossMaxHp)||1),l=Math.min(100,Math.max(0,Math.round(s/c*1e3)/10)),u=``,d=!1;if(t.mechanic&&t.mechanic.type){let e=t.mechanic,n=this.getPlayerId();Array.isArray(e.targetIds)&&n&&e.targetIds.some(e=>this.getPlayerId(e)===n||String(e)===n)&&(d=!0);let r=e.type;e.type===`yinyang`?r=`阴阳断脉`:e.type===`tiangang`?r=`天罡共劫`:e.type===`aspect`?r=`四象化劫`:e.type===`miehun`||e.type===`灭魂指`?r=`灭魂指`:(e.type===`tianyin`||e.type===`七灭天音`)&&(r=`七灭天音`);let i=``;d?i=`<span class="qimie-target-me">【你被点名】</span>`:e.targetRoom?i=`<span>地点: ${X({center:`镇劫中宫`,east:`东·问剑台`,south:`南·焚天台`,west:`西·断金台`,north:`北·镇魂台`}[e.targetRoom]||e.targetRoom)}</span>`:Array.isArray(e.targetRooms)&&e.targetRooms.length?i=`<span>地点: ${X(e.targetRooms.map(e=>({center:`镇劫中宫`,east:`东·问剑台`,south:`南·焚天台`,west:`西·断金台`,north:`北·镇魂台`})[e]||e).join(`/`))}</span>`:Array.isArray(e.targetIds)&&e.targetIds.length&&(i=`<span>点名: ${e.targetIds.length}人</span>`);let a=``;if(e.requiredCount){let t=0;Array.isArray(e.participants)?t=e.participants.length:typeof e.participants==`number`?t=e.participants:e.participants&&typeof e.participants==`object`&&(t=Object.keys(e.participants).length),a=`<span>分摊: ${t}/${e.requiredCount}</span>`}u=`
                <div class="qimie-mech-pill ${d?`mech-danger`:``}">
                    <span class="mech-name">${X(r)}</span>
                    <span class="qimie-mech-timer"></span>
                    ${i}
                    ${a}
                </div>
            `}let f=``;if(t.aspects&&Array.isArray(t.aspects)&&t.aspects.length){f=`<div class="qimie-aspects-grid">`;for(let e=0;e<t.aspects.length;e++){let n=t.aspects[e]||{},r=ir[e]||``,i=Math.max(0,Number(n.hp)||0),a=Math.max(1,Number(n.maxHp)||1),o=Math.min(100,Math.max(0,Math.round(i/a*100))),s=n.alive!==!1&&i>0;f+=`
                    <div class="qimie-aspect-card ${s?`aspect-alive`:`aspect-dead`}">
                        <div class="aspect-header">
                            <span class="aspect-dir">${X(r)}</span>
                            <span class="aspect-name">${X(n.name||`法身`)}</span>
                            <span class="aspect-status">${s?`存活`:`已灭`}</span>
                        </div>
                        <div class="aspect-bar-box">
                            <div class="aspect-hp-bar" style="width: ${s?o:0}%"></div>
                        </div>
                        <div class="aspect-details">
                            <span class="aspect-room">${X(n.room||``)}</span>
                            <span class="aspect-buff">${X(n.buff||``)}</span>
                        </div>
                    </div>
                `}f+=`</div>`}let p=`
            ...
            <div class="qimie-bar-summary qimie-summary-clickable">
                <div class="qimie-summary-left">
                    <span class="qimie-phase-badge ${i}">${X(r)}</span>
                    <span class="qimie-timer"><span class="glyphicon glyphicon-time"></span> <span class="qimie-timer-val">--:--</span></span>
                    <span class="qimie-stacks ${o}">劫印: <strong>${a}</strong>/6</span>
                </div>
                <div class="qimie-summary-mid">
                    <div class="qimie-boss-hp-wrap">
                        <div class="qimie-boss-hp-bar" style="width: ${l}%"></div>
                        <span class="qimie-boss-hp-text">七灭尊者: ${l}% (${X(s)}/${X(c)})</span>
                    </div>
                    ${u}
                </div>
            ...
            <div class="qimie-bar-detail">
                <div class="qimie-detail-section">
                    ...
                    ${f||`<div class="qimie-no-aspects">暂无法身</div>`}
                </div>
        ...
        `;e.html(p),e.toggleClass(`expanded`,this.expanded),this.renderRaidSection(),this.updateCountdown()},renderRaidSection(){let e=$(`.qimie-raid-messages`);if(!e.length)return;if(!this.raidMessages.length){e.html(`<div class="qimie-raid-empty">暂无战团消息，使用战团频道或发送 raid &lt;内容&gt; 交流</div>`);return}let t=``;for(let e of this.raidMessages){let n=X(e.name||`侠士`),r=X(e.content||``),i=``;if(e.time){let t=new Date(e.time);i=(t.getHours()<10?`0`+t.getHours():t.getHours())+`:`+(t.getMinutes()<10?`0`+t.getMinutes():t.getMinutes())}t+=`
                <div class="qimie-raid-msg-item">
                    <span class="raid-msg-time">[${i}]</span>
                    <span class="raid-msg-sender">【战团】${n}：</span>
                    <span class="raid-msg-body">${r}</span>
                </div>
            `}e.html(t),e[0]&&e.scrollTop(e[0].scrollHeight)}},or=()=>$(`.content-message`),sr={append:e=>$(`.content-message`).append(e)},Z={itemsElement:null,contentScroll:!0,message:null,channel:null,relogin(){O(`#login_panel`)},clear:function(){Dialog.pack.items=null,Dialog.skills.items=null,Dialog.skills.skills=null,Dialog.skills.autoPfm=null,Dialog.skills.performSkills=null,Dialog.skills.sk_group=null,I.Skills=null,I.dis_pfms=[],this.state(null),ar.clear()},init:function(){Z.itemsElement=$(`.room_items`),this.message=En.create($(`.content-message`)),this.ChannelElement=$(`.channel`),this.ChannelElement.on(`click`,Dialog.channel.show.bind(Dialog.channel)),this.channel=En.create(this.ChannelElement,4,200)},startMoveMessage:function(e){window.addEventListener(`mousemove`,Z.moveMessage),window.addEventListener(`mouseup`,Z.endMoveMessage),Z.mouseY=e.clientY},moveMessage:function(e){let t=Z.mouseY-e.clientY,n=or(),r=n[0],i=n.height(),a=r.style.marginBottom;if(a=a?parseInt(a.replace(`px`,``)):0,a+=t,a<0)a=0;else if(a>i*.7)return;r.style.marginBottom=a+`px`,Z.mouseY=e.clientY,e.preventDefault()},endMoveMessage:function(){window.removeEventListener(`mousemove`,Z.moveMessage),window.removeEventListener(`mouseup`,Z.endMoveMessage)},regist:function(e){e.result&&(O(`#addrole_panel`),$(`#addrole_panel .input-error`).html(e.result))},emote:function(e){Z.emotes=e.items||0;for(var t=[],n=0;n<Z.emotes.length;n++)t.push(`<span>`),t.push(Z.emotes[n]),t.push(`</span>`);$(`.channel-emotes`).html(t.join(``))},deleterole:function(e){if(e.result){var t=$(`#role_panel>ul>.content>.role-list>.role-item[roleid='`+e.id+`']`);t.remove();var n=$(`#role_panel>ul>.content>.role-list>.role-item`);t.is(`.select`)&&n.length?$(n[0]).addClass(`select`):n.length||Y.addRole()}else Confirm.Show({content:`<span class='input-error'>`+(e.message||`删除失败`)+`</span>`})},cross:function(e){for(var t=null,n=0;n<J.length;n++)J[n].ID==e.sid&&(t=J[n]);t&&(w.ChangeServer=!0,w.Close(),Dialog.pack.items=null,e.cross_type==`duizhan`&&(Dialog.skills.items=null,Dialog.skills.isShow=!1),console.log(`重新连接到`,t.Name),e.pid||Z.die({relive:!0}),me(t,e.pid))},roles:function(e){var t=e.roles;if(!t.length)Y.addRole();else{O(`#role_panel`);for(var n=[],r=0;r<t.length;r++)n.push(`<li class='role-item`),r==0&&n.push(` select`),n.push(`' roleid='`+t[r].id+`'>`),n.push(t[r].title),n.push(`&nbsp;&nbsp;`),n.push(t[r].name),n.push(`</li>`);$(`.role-list`).html(n.join(``))}},loginerror:function(e){$(`.container`).hide(),$(`.login-content`).show(),k(`<strong>登陆失败：</strong>`+e.msg)},login:function(e){Z.player||O(`.container`),Z.player=e.id,Z.level=e.level,P.load(e.setting),ge()},levelup:function(e){Z.level=e.level},selectItem:function(e){if($(e.target).is(`.status-item`)){var t=e.target.getAttribute(`sid`);let n=$(e.target).closest(`.room-item`).attr(`itemid`);return t?n===Z.player?T(`status `+t):T(`status `+t+` `+n):void 0}var n=$(this).attr(`itemid`);if(console.log(n),n){if(n==Z.player){T(`select `+n);return}T(`select `+n)}},countwidth:function(e,t){var n=e*100/t;return n<0&&(n=0),n>100&&(n=100),n},itemremove:function(e){var t=I.STATUS[e.id];if(t){for(var n in t.items)clearInterval(t.items[n].handler);var r=t.elem.parent();r.next().is(`.item-commands`)&&r.next().remove(),r.remove(),delete I.STATUS[e.id]}Z.cur_room.items.RemoveAt(t=>t.id===e.id)},itemadd:function(e){if(!(P.off_plist&&e.p&&e.id!=Z.player)){var t=e,n=P.item_firstme&&t.id==Z.player?$(Z.create_roomitem(t)).prependTo(Z.itemsElement):$(Z.create_roomitem(t)).appendTo(Z.itemsElement);I.STATUS[e.id]&&Z.itemremove(e),I.AppendStatusItem(t.id,n.find(`.item-status-bar`),t.status),Z.cur_room.items.push(t)}},items:function(e){Z.itemsElement.empty(),I.STATUS={};for(var t=0;t<e.items.length;t++){var n=e.items[t];if(n&&(n.player=n.p,n.m&&(n.type=`师父`,n.master=1),n.f&&(n.type=`随从`,n.follower=1),n.l&&(n.type=`商人`,n.trader=1),!(P.off_plist&&n.p&&n.id!=Z.player))){var r=P.item_firstme&&n.id==Z.player?$(Z.create_roomitem(n)).prependTo(Z.itemsElement):$(Z.create_roomitem(n)).appendTo(Z.itemsElement);I.AppendStatusItem(n.id,r.find(`.item-status-bar`),n.status)}}Z.cur_room||={},Z.cur_room.items=e.items},get_hpnum:function(e,t){var n=e/t;return n>.8?`<hiy>`+e+`</hiy>`:n>.5?`<yel>`+e+`</yel>`:n>.2?`<red>`+e+`</red>`:`<hir>`+e+`</hir>`},create_roomitem:function(e){var t=[];return t.push(`<div class='room-item' itemid='`+e.id+`'>`),e.max_hp&&(t.push(`<div class="item-status"`),(!I.IsShow||P.off_hp)&&t.push(` style="display:none;"`),t.push(`>`),t.push(`<div class="progress hp"><div class="progress-bar" max="`+e.max_hp+`"  style="width:`+Z.countwidth(e.hp,e.max_hp)+`%"></div></div>`),t.push(`<div class="progress mp"><div class="progress-bar" max="`+e.max_mp+`"   style="width:`+Z.countwidth(e.mp,e.max_mp)+`%"></div></div>`),t.push(`</div>`)),t.push(`<span class='item-status-bar'>`),t.push(`</span>`),t.push(`<span class='item-name'>`),t.push(e.name),P.show_hpnum&&e.max_hp&&t.push(`<span class="progress-num">[`+this.get_hpnum(e.hp,e.max_hp)+`<nor>/</nor><hiy>`+e.max_hp+`</hiy>]</span>`),t.push(`</span>`),t.push(`</div>`),t.join(``)},room:function(e){$(`.room_items`).html(``),$(`.room-name`).html(e.name),$(`.room_desc`).html(e.desc),Z.room_name=e.name,P.keep_msg?P.keep_msg&&Q(`你来到了`+e.name+`。`):Z.message.clear(),Z.room_path!=e.path&&(P.show_roomitem&&Z.searchItems(e),I.ShowRoomCommands(e),Z.room_path=e.path,Z.cur_room=e,A.SetRoom(e))},roomHiddenItemsReg:/<\w{3}\scmd=['"](.+?)['"]>(.+?)<\/\w{3}>/g,searchItems:function(e){for(var t=null,n=e.desc;(t=this.roomHiddenItemsReg.exec(n))!==null;)e.commands.push({cmd:t[1],name:t[2]})},exits:function(e){var t=e?e.items:Z.room_exits;if(t)if(Z.room_exits=t,A.SetExits(t),P.exits_dir==1){for(var n=[`这里明显的出口有：`],r=[],i=0;i<A.DIRS.length;i++)t[A.DIRS[i]]&&r.push(A.DIRS[i]);for(var i=0;i<r.length;i++)i>0&&n.push(i==r.length-1?` 和 `:`、`),n.push(`<span class='exits-item' dir='`+r[i]+`'>`+r[i]+`</span>`);r.length?$(`.room_exits`).html(n.join(``)):$(`.room_exits`).html(`<HIK>这里没有明显的出口。<HIK>`)}else $(`.room_exits`).html(A.CreateExitsMap(t,$(`.container`).width(),Z.room_name))},before_click_exits:function(e){var t=$(e.target);t.attr(`dir`)&&(t.is(`rect`)?t.attr(`fill`,`var(--theme-surface-2)`):t.is(`text`)&&t.prev().attr(`fill`,`var(--theme-surface-2)`))},click_exits:function(e){var t=$(e.target),n=t.attr(`dir`);n&&(t.is(`rect`)?t.attr(`fill`,`var(--theme-surface)`):t.is(`text`)&&t.prev().attr(`fill`,`var(--theme-surface)`),T(`go `+n))},query_rmitem:function(e){if(!(!this.cur_room||!this.cur_room.items)){for(let t of this.cur_room.items)if(t.id===e)return t}},isLivingRoomItem:function(e){return!!(e&&(e.me||e.p||e.m||e.f||e.l||e.hp>0||e.max_hp>0))},item:function(e){e.commands=e.commands??[];let t=Z.query_rmitem(e.id);t&&(e=Object.assign(e,t)),j.LAST_OBJ=e,Dialog.extend.append(e.commands,`item`,e);for(var n=[`<div class='item-commands'>`],r=0;r<e.commands.length;r++)n.push(`<span cmd='`+e.commands[r].cmd+`'>`),n.push(e.commands[r].name),n.push(`</span>`);if(n.push(`</div>`),Z.isLivingRoomItem(e)){Z.itemsElement.find(`.item-commands`).remove(),Dialog.item.open(e);return}if(Q(e.desc),P.show_command&&I.STATUS[e.id]){Z.itemsElement.find(`.item-commands`).remove();var i=I.STATUS[e.id].elem.parent();return $(n.join(``)).insertAfter(i),Z.message.scroll2end()}Q(n.join(``))},actions:function(e){I.ShowActions(e)},cmds:function(e){if(e.items){if(Dialog.curItem===`item`&&Dialog.item&&Dialog.item.isShow){if(Dialog.item.isCapturingInteraction&&Dialog.item.isCapturingInteraction()){Dialog.item.appendInteractionCommands(e.items);return}Dialog.item.appendCommands(e.items);return}if(Dialog.isShow){S.Show(e.items);return}var t=[`<div class='item-commands'>`];e.items.length||(e.items=[e.items]);for(var n=0;n<e.items.length;n++)t.push(`<span cmd='`+e.items[n].cmd+`'>`),t.push(e.items[n].name),t.push(`</span>`);t.push(`</div>`),Q(t.join(``))}},map:function(e){A.SetMapBuffer(e.map,e.path),A.ShowMap(e.map,e.path),A.OpenDialogAfterLoad&&(A.OpenDialogAfterLoad=!1,$(`.map-panel`).hide(),Dialog.show(`map`))},updatemap:function(e){A.UpdateMap(e.map,e)},path:function(e){e.message&&Q(e.message),e.action==`stop`&&A.StopAutoMove()},dialog:function(e){Dialog.show(e.dialog,e)},sc:function(e){I.StatusChanged(e)},perform:function(e){I.ShowPFM(e),Dialog.skills.updatePerformSkills(e.skills)},disobj:function(e){I.DisObj(e)},changepfm:function(e){I.ChangeDistime(e)},clearDistime:function(e){I.ClearDistime(e)},pay:function(e){if(e.pay===3){Q(`<yel>请打开微信扫描二维码支付：</yel>
`);let t=$(`<div style="width:100%;text-align:center;"><img style="border:solid 2px #808088" src="`+e.url+`"/></div>`);t.children(0).on(`load`,function(){Q(``)}),sr.append(t)}else window.location.href=e.url},dispfm:function(e){I.On_Perform(e)},status:function(e){I.StatusItemChanged(e)},combat:function(e){e.start&&(P.auto_showcombat==1&&!I.IsShow&&I.Show(),P.auto_hideroom==1&&(P.hide_roomdesc||$(`.room_desc`).hide())),e.end&&P.auto_hideroom==1&&(P.hide_roomdesc||$(`.room_desc`).show())},state:function(e){if(e&&e.state){var t=[`<span class='title'>`+e.state+`</span>`];if(e.commands)for(var n=0;n<e.commands.length;n++)t.push(`<span class='item-command' cmd='`+e.commands[n].cmd+`'>`),t.push(e.commands[n].name),t.push(`</span>`);$(`.state-bar`).html(t.join(``)).css(`visibility`,`visible`),e.no_stop?$(`.state-tool`).hide():$(`.state-tool`).show(),Z.states=e.desc,Z.timer&&clearInterval(Z.timer),Z.states&&Z.states.length&&(typeof Z.states==`string`&&(Z.states=[Z.states]),Z.timer=setInterval(Z.updatestate,e.interval||5e3))}else $(`.state-bar`).empty().css(`visibility`,`hidden`),$(`.state-tool`).hide(),clearInterval(Z.timer)},updatestate:function(){if(Z.states&&he()){var e=Z.states.length;Q(Z.states[parseInt(Math.random()*e)])}},die:function(e){if(e.relive)return Z.state({});Z.state({state:`<hiw>你已经死亡：</hiw>`,no_stop:!0,desc:[`<blk>一股阴冷的气息包围着你。</blk>`,`<blu>朦胧中你好像听到有人在喊：过来吧，过来吧！</blu>`],commands:e.commands,interval:12e3})},warn:function(e){Warn.Show(e)},msg:function(e){var t=Dialog.channel.createElement(e,!P.no_spmsg);t&&(P.no_spmsg?Q(t):(Z.channel.push(t),Z.channel.scroll2end()))},addAction:function(e){I.AddObj(e.id,e.name,e.distime)},removeAction:function(e){I.DisObj({id:e.id,remove:!0})},boss_event:function(e){ar.onBossEvent(e,Z.player)},raid:function(e){ar.onRaid(e)}};function Q(e){Z.message.push(e),Z.message.scroll2end()}var cr=!1,lr=[`fight`,`kill`,`perform`,`biwu`];function ur(e){if(!e)return!1;var t=e.trim().split(/\s+/,1)[0];return lr.indexOf(t)>=0}function dr(){G.isShow&&G.hide(),x.Close(),S.Close()}var fr=class extends K{constructor(){super(),this.template=`
...
`}on_mount(){$(`.container`).on(`click`,Sr),$(`.channel-box`).on(`click`,`span`,br),$(`.combat-commands`).on(`click`,`.pfm-item`,I.Perform).on(`wheel`,I.Scroll),$(`.room-commands`).on(`wheel`,I.Scroll),$(`.sender-box`).on(`keyup`,vr),$(`.room_items`).on(`click`,`.room-item`,Z.selectItem),$(`.bottom-bar`).on(`click`,`.tool-item,.state-bar,.item-command`,hr),$(`.map-panel`).on(`click`,mr),$(`.sender-btn`).on(`click`,yr),$(`.room_exits`).on(`pointerdown`,Z.before_click_exits).on(`pointerup`,Z.click_exits),$(`.room-title>.map-icon`).on(`click`,A.OpenDialog.bind(A))}},pr=0;function mr(e){if(!$(e.currentTarget).closest(`.dialog-map`).length&&!A.SuppressRoomClick){if(pr||=0,Date.now()-pr>500){pr=Date.now();return}A.OpenDialog()}}function hr(e){var t=$(this).attr(`command`);return t?gr(t):(t=$(this).attr(`cmd`),t&&SendCommand(t),!1)}function gr(e){switch(e){case`showtool`:Ct();break;case`showchat`:return _r();case`showcombat`:return I.Show();case`stopstate`:if(G.extend.is_record)return G.extend.stop_record();SendCommand(`state stop`);break;case`stateinfo`:SendCommand(`state info`);break;default:G.show(e);break}return!1}function _r(){var e=$(`.chat-panel`).toggleClass(`hide`);return e.is(`.hide`)?cr=!1:(cr=!0,e.find(`input`).val(``).focus()),!1}function vr(e){e.keyCode==13&&yr()}function yr(){var e=$(`.sender-box`),t=e.val(),n=t?String(t).trim():``;if(!n){e.val(``).focus();return}if(n.length>100)return ReceiveMessage(`<hir>你输入的内容太多了。</hir>`);var r=$(`.channel-box`).attr(`channel`)||`chat`;e.val(``).focus(),SendCommand(r+` `+n)}function br(){var e=$(this),t=e.attr(`channel`);if(t==`emote`)return xr();if(!e.is(`.selected`)){var n=e.parent();return n.children().removeClass(`selected`),e.addClass(`selected`),n.attr(`channel`,t),$(`.sender-box`).focus(),!1}}function xr(){var e=$(`.channel-emotes`);e.is(`.hide`)?(e.removeClass(`hide`),Z.emtoes||(SendCommand(`emote`),Z.emtoes=[],$(`.sender-box`).blur(),e.on(`click`,`span`,function(){var e=$(this).html();$(`.sender-box`).val(`*`+e).focus(),$(`.channel-emotes`).addClass(`hide`)}))):$(`.channel-emotes`).addClass(`hide`)}function Sr(e){var t=$(e.target);if(G.isShow&&t.is(`.container.dialog-open`))return G.hide(),x.Close(),S.Close(),!1;var n=t.attr(`cmd`);if(n||=t.parent().attr(`cmd`),n){let e=n[0];if(e==`_`){var r=n.split(` `);switch(r[0]){case`_confirm`:x.Process(r);break;case`_setting`:P.save(r[1],r[2]);break;case`_trade`:G.trade.confirm(r[1]);break;case`_close`:ce.Close(t);break;case`_hide`:break;case`_closed`:G.hide();case`_party`:G.party.command(r[1]);break}}else e===`#`?j.run(n):(ur(n)?dr():G.curItem===`item`&&G.item&&G.item.isShow&&t.closest(`.dialog-item-dialog`).length>0?G.item.captureNextPrompt():G.isShow&&S.captureNext(),SendCommand(n),!t.closest(`.dialog-fb`).length&&!t.closest(`.dialog-item-subdialog`).length&&t.closest(`.dialog-content`).length>0&&t.closest(`.item-commands`).remove());return!1}else cr&&(t.closest(`.chat-panel`).length||($(`.chat-panel`).addClass(`hide`),cr=!1));x.Close(),S.Close()}window.HandlerMenuCommand=gr;var Cr=new tr,wr=new fr,Tr=class extends K{constructor(){super(),this.template=`${Cr.render()}\n${wr.render()}`}on_mount(){Cr.on_mount(),wr.on_mount()}};globalThis.$=e,globalThis.SendCommand=T,globalThis.ReceiveMessage=E,globalThis.Confirm=x,globalThis.Warn=ce,globalThis.CmdPrompt=S,globalThis.Process=Z,globalThis.Dialog=G;function Er(){let e=new Tr;e.onCompile(),e.mount(document.body),Z.init()}e(Er);