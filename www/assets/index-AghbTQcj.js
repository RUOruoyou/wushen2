...
`,Xe=`  <div class="setting dialog-custom">
        ...
        </div>`,Ze=` <div class="setting dialog-skeys"></div>`,Qe=` <div class="setting dialog-extend"></div>`,$e=`
.setting {
...
`;function ut(e){var t=Math.max(0,e||0)/1e3;return(t%1?t.toFixed(1):t.toFixed(0))+`秒`}var dt=`
...
`}showServers(){if(!Y){O(`正在获取服务器列表`),J.GetServer(e=>{if(!e||typeof e==`string`){E(`#login_pwd`,`获取服务器列表出错`);return}Y=e,this.displayServer(e),this.showServers()});return}var e=Y;if(!e||!e.length)D(`#login_panel`),E(`#login_pwd`,`获取服务器列表出错`);else{var t=x.GetUserCookie(`s`),n=t?Y[t]:e.length==1?Y[0]:null;if(n)return O(`正在连接服务器`),fe(n);D(`#slist_panel`)}}selectServer(){if(Y){var e=parseInt($(`.server-list>.select`).attr(`index`));if(!(e>=0&&e<Y.length))return k.Show({content:`你没有选择要连接的服务器。`});var t=Y[e];t||k.Show({content:`你没有选择要连接的服务器。`}),O(`正在连接服务器`),fe(t),x.SetCookie(`s`,e)}}displayServer(){if(Y){var e=location.hostname.startsWith(`127.0.0.1`)||location.hostname.startsWith(`localhost`),t=location.search.startsWith(`?test`);e&&Y.push({id:100,name:`本地测试1`,ip:`127.0.0.1`,port:31300});for(var n=[],r=`武神传说2`,i=0;i<Y.length;i++)!t&&!e&&Y[i].istest||(n.push(`<li class='role-item`),i==0&&n.push(` select`),n.push(`' index='`+i+`'>`),n.push(r),n.push(`&nbsp;&nbsp;`),n.push(Y[i].name),Y[i].isdef&&n.push(`<span class='server-badge'>&nbsp;（推荐）</span>`),n.push(`</li>`));$(`.server-list`).html(n.join(``)).on(`click`,`li`,function(){var e=$(this);e.is(`.select`)||(e.parent().find(`.select`).removeClass(`select`),e.addClass(`select`))})}}},tn=class extends q{constructor(){super(),this.initReg=!1,this.template=`
...
`}on_mount(){}open(){this.initReg||=(this.GetValidationImage(),$(`.validnum-box>.validnum-img`).on(`click`,()=>this.GetValidationImage()),!0)}regist(){var e=$(`#regist_name`).val().toLowerCase(),t=$(`#regist_pwd1`).val();if(!e)return E(`#regist_name`,`请输入用户名`);if(!/^[a-z0-9]{5,15}$/.test(e))return E(`#regist_name`,`用户名需要是5-10个英文字符`);if(!t)return E(`#regist_pwd1`,`请输入密码`);if(t.length<6||t.length>20)return E(`#regist_pwd1`,`密码长度在6到20之间`);if(t!=$(`#regist_pwd2`).val())return E(`#regist_pwd2`,`重复密码输入不一致，请重新输入`);var n=$(`#regist_val`).val();if(!n)return E(`#regist_valpanel`,`请输入图片中的验证码`);if(n.length!=4)return E(`#regist_valpanel`,`请输入图片中的四位验证码`);let r=0,i=/u(\d+)/.exec(location.pathname);i&&(r=parseInt(i[1]),r>0||(r=0)),O(`正在注册账号`),J.Regist({name:e,pwd:t,valno:n,guider:r},e=>{e.code==1?(O(`注册成功，正在获取服务器列表`),setTimeout(()=>window.location.reload(),500)):(E(`#regist_name`,e.result||`注册失败`),D($(`#regist_panel`)))})}GetValidationImage(){J.ValidationImage(function(e){$(`.validnum-box>.validnum-img`).attr(`src`,`data:image/svg+xml;base64,`+e)})}},nn=`万俟司马上官欧阳夏侯诸葛闻人东方赫连皇甫尉迟公羊澹台公冶宗政濮阳淳于单于太叔申屠公孙仲孙轩辕令狐锺离宇文长孙慕容鲜于闾丘司徒司空丌官司寇子车颛孙端木巫马公西乐正公良拓拔夹谷谷梁梁丘左丘东门西门`,rn=`赵钱孙李周吴郑王冯陈楮卫蒋沈韩杨朱秦尤许何吕施张孔曹严华金魏陶姜戚谢邹喻柏水窦章云苏潘葛奚范彭郎`,an=`世舜丞主产仁仇仓仕仞任伋众伸佐佺侃侪促俟信俣修倝倡倧偿储僖僧僳儒俊伟列则刚创前剑助劭势勘参叔吏嗣士壮孺守宽宾宋宗宙宣实宰尊峙峻崇崈川州巡帅庚战才承拯操斋昌晁暠曹曾珺玮珹琒琛琩琮琸瑎玚璟璥瑜生畴矗矢石磊砂碫示社祖祚祥禅稹穆竣竦综缜绪舱舷船蚩襦轼辑轩子杰榜碧葆莱蒲天乐东钢铎铖铠铸铿锋镇键镰馗旭骏骢骥驹驾骄诚诤赐慕端征坚建弓强彦御悍擎攀旷昂晷健冀凯劻啸柴木林森朴骞寒函高魁魏鲛鲲鹰丕乒候冕勰备宪宾密封山峰弼彪彭旁日明昪昴胜汉涵汗浩涛淏清澜浦澉澎澔瀚瀛灏沧虚豪豹辅辈迈邶合部阔雄霆震韩俯颁颇频颔风飒飙飚马亮仑仝代儋利力劼勒卓哲喆展帝弛弢弩彰征律德志忠思振挺掣旲旻昊昮晋晟晸朕朗段殿泰滕炅炜煜煊炎选玄勇君稼黎利贤谊金鑫辉墨欧有友闻问`,on=`筠柔竹霭凝晓欢霄枫芸菲寒伊亚宜姬舒影荔枝思丽秀娟英华慧巧美娜静淑惠珠翠雅芝玉萍红娥玲芬芳燕彩春菊勤珍贞莉兰凤洁梅琳素云莲真环雪荣妹霞香月莺媛艳瑞凡佳嘉琼桂娣叶璧璐娅琦晶妍茜秋珊莎锦黛青倩婷姣婉娴瑾颖露瑶怡婵雁蓓纨仪荷丹蓉眉君琴蕊薇菁梦岚苑婕馨瑗琰韵融园艺咏卿聪澜纯毓悦昭冰爽琬茗羽希宁欣飘育滢馥`;function sn(e,t){t||=parseInt(Math.random()*2)+1;var n=[];if(t==2){var r=parseInt(Math.random()*nn.length);r%2==1&&--r,n.push(nn[r++]),n.push(nn[r])}else n.push(rn[parseInt(Math.random()*rn.length)]);return e==0?n.push(an[parseInt(Math.random()*an.length)]):n.push(on[parseInt(Math.random()*on.length)]),parseInt(Math.random()*4)>1&&(e==0?n.push(an[parseInt(Math.random()*an.length)]):n.push(on[parseInt(Math.random()*on.length)])),n.join(``)}function cn(){for(var e=`abcdefghijklmnopqrstuvwxyz`,t=`123456789`,n=[],r=parseInt(Math.random()*3)+3,i=0;i<r;i++)i<3?n.push(e[parseInt(Math.random()*e.length)]):n.push(t[parseInt(Math.random()*t.length)]);return n.join(``)}function ln(){for(var e=20,t=[],n=0;n<4;n++){var r=parseInt(Math.random()*15+1);e>=r?(n==3?r=e:e-=r,t[n]=r):(t[n]=e,e=0)}var i={};return i.str=t[0]+15,i.con=t[1]+15,i.dex=t[2]+15,i.int=t[3]+15,i}function un(e){switch(e){case`name`:$(`#reg_name`).val(sn(+!$(`#gender_0`).is(`:checked`)));break;case`id`:$(`#reg_id`).val(cn());break;case`prop`:var t=ln();$(`#reg_str`).val(t.str),$(`#reg_con`).val(t.con),$(`#reg_dex`).val(t.dex),$(`#reg_int`).val(t.int);break}}var dn=class extends q{constructor(){super(),this.template=`
...
`}select(){var e=$(`.role-list>.select`);if(e.length){var t=e.attr(`roleid`);SendCommand(`login `+t),O(`正在进入游戏`,`#role_panel`)}}addRole(){if($(`.role-list>.role-item`).length>4)return k.Show({content:`你只能最多创建五个角色`});D($(`#addrole_panel`)),un(`name`),un(`prop`),un(`id`)}delete(){var e=$(`.role-list>.select`);if(e.length){var t=e.attr(`roleid`);t&&k.Show({content:`是否确认删除角色：`+e.html(),onOK:function(){SendCommand(`deleterole `+t)}})}}},fn=class extends q{constructor(){super(),this.template=`
...
`}on_mount(){}reset(){var e=$(`#reset_name`).val();if(!e)return E(`#reset_name`,`请输入用户名`);if(!/^[a-z0-9]{5,15}$/.test(e))return E(`#reset_name`,`用户名格式错误,需要5-15位字母开头的字母，数字或下划线，不区分大小写`);var t=$(`#reset_phone`).val();if(!t)return E(`#reset_phone`,`请输入你的帐号绑定的手机号码`);if(!/^1\d{10}$/.test(t))return E(`#reset_phone`,`手机号码格式错误`);var n=``,r=$(`#reset_pwd1`).val();if(!r)return E(`#reset_pwd1`,`请输入你的新密码`);var i=$(`#reset_pwd2`).val();if(!i)return E(`#reset_pwd2`,`请重复输入你的新密码`);if(i.length<6||i.length>20)return E(`#update_pwd2`,`密码长度在6到20之间`);if(i!=r)return E(`#reset_pwd2`,`两次密码输入不一致`);O(`正在修改密码`,`#reset_panel`),J.ResetPasswordByPhone(e,t,n,r,function(e){e.code?D(`#login_panel`):(E(`#reset_pwd2`,e.result??`重置失败`),D(`#reset_panel`))})}},pn=class extends q{constructor(){super(),this.template=`
...
`}open(){D(`#pwd_panel`),J.GetPhone(function(e){if(e.code!==1)return E(`#update_pwd1`,`获取绑定的手机号失败`);e.result?($(`#pwd_phone`).prop(`disabled`,!0).val(e.result),$(`#pwd_bind`).show()):($(`#pwd_phone`).prop(`disabled`,!1).val(``),$(`#pwd_bind`).hide())})}update(){$(`#pwd_panel`).find(`.input-error`).remove();var e=$(`#update_pwd1`).val(),t=$(`#update_pwd2`).val(),n=$(`#update_pwd3`).val();if(e.length<6||e.length>20)return E(`#update_pwd1`,`密码长度在6到20之间`);if(t.length<6||t.length>20)return E(`#update_pwd2`,`密码长度在6到20之间`);if(n!=t)return E(`#update_pwd3`,`两次密码输入不一致`);var r;if($(`#pwd_bind`).is(`:visible`)&&(r=$(`#pwd_no`).val(),!r||!/^\d{4}$/.test(r)))return E($(`#pwd_no`).parent(),`请输入你绑定的手机尾号`);O(`正在修改密码`,`#pwd_panel`),J.ChangePassword(e,t,r,function(e){e.code?D($(`#slist_panel`)):(E(`#update_pwd1`,e.result||`修改失败`),D(`#pwd_panel`))})}},mn=class extends q{constructor(){super(),this.template=`
...
`}bind(){D(`#bind_panel`),J.GetPhone(function(e){if($(`#phone_valid`).val(``),$(`#phone_pwd`).val(``),e.code!==1)return $(`.input-error`).html(e.result);$(`.input-error`).remove();let t=e.result;t?($(`#phone_no`).prop(`disabled`,!0).val(t),$(`#phone_valid`).parent().show().prev().show(),$(`#phone_no`).prev().html(`你已绑定手机，再次验证会取消绑定`),$(`#phone_no`).parent().next().find(`span:last()`).html(`解除绑定`)):($(`#phone_no`).prop(`disabled`,!1).val(``),$(`#phone_no`).prev().html(`你要绑定的手机(不验证，目前仅作为二级密码验证使用)`),$(`#phone_valid`).parent().hide().prev().hide(),$(`#phone_no`).parent().next().find(`span:last()`).html(`绑定`))})}check(){var e=$(`#phone_no`),t=``,n=``;if(e.is(`:disabled`)){if(n=$(`#phone_valid`).val(),!n)return E($(`#phone_valid`).parent(),`请输入你接收到的六位验证码`);if(!/^\d{4}$/.test(n))return E($(`#phone_valid`).parent(),`请输入六位数字的验证码`)}else{if(t=e.val(),!t)return E(`#phone_no`,`请输入你的帐号绑定的手机号码`);if(!/^1\d{10}$/.test(t))return E(`#phone_no`,`手机号码格式错误`)}var r=$(`#phone_pwd`).val();if(!r)return E(`#phone_pwd`,`请重复输入你的新密码`);if(r.length<6||r.length>20)return E(`#phone_pwd`,`密码长度在6到20之间`);J.BindPhone(n,t,r,function(e){e.code<1?(E($(`#phone_valid`).parent(),e.result??`绑定失败`),D(`#bind_panel`)):D(`#role_panel`)})}};window.RefreshInput=un;var hn=class extends q{constructor(){super(),this.template=`
...
`}create(){var e={};if(e.name=$(`#reg_name`).val(),e.gender=$(`#gender_0`).is(`:checked`)?1:2,e.str=parseInt($(`#reg_str`).val()),e.con=parseInt($(`#reg_con`).val()),e.dex=parseInt($(`#reg_dex`).val()),e.int=parseInt($(`#reg_int`).val()),!/^[\u4E00-\u9FA5]{2,5}$/.test(e.name))return E(`#reg_name`,`名称格式错误，只能使用2-5位中文字符`);if(e.str<15||e.str>30)return E(`#reg_name`,`臂力需要在15-30之间`);if(e.con<15||e.con>30)return E(`#reg_name`,`根骨需要在15-30之间`);if(e.dex<15||e.dex>30)return E(`#reg_name`,`身法需要在15-30之间`);if(e.int<15||e.int>30)return E(`#reg_name`,`悟性需要在15-30之间`);if(e.str+e.con+e.dex+e.int!=80)return E(`#reg_name`,`先天属性需要在15-30之间，并且总和等于80`);O(`正在创建角色`,`#addrole_panel`),SendCommand(`createrole `+e.name+` `+e.gender+` `+e.str+` `+e.con+` `+e.dex+` `+e.int)}},gn=class extends q{constructor(){super(),this.template=`
...
`}relogin(){D($(`#login_panel`));var e=new Date;e.setTime(-1e3);for(var t=document.cookie.split(`; `),n=0;n<t.length;n++){var r=t[n].split(`=`);document.cookie=r[0]+`=''; expires=`+e.toGMTString()}}loginIn(){var e=$(`#login_name`).val().toLowerCase(),t=$(`#login_pwd`).val();if(!e)return E(`#login_name`,`请输入用户名`);if(!/^[a-z0-9]{5,15}$/.test(e))return E(`#login_name`,`用户名格式错误,需要5-15位字母开头的字母，数字或下划线，不区分大小写`);if(!t)return E(`#login_pwd`,`请输入密码`);if(t.length<6||t.length>20)return E(`#login_pwd`,`密码长度在6到20之间`);O(`正在登录`,`#login_panel`),J.Login(e,t,e=>{e.code?vn.showServers():(E(`#login_name`,e.result||`登陆失败`),D(`#login_panel`))})}},bn=new en,xn=new yn,Sn=new tn,X=new dn,Cn=new fn,wn=new pn,Tn=new mn,En=new hn,Dn=new gn;function On(){let e=$(this).attr(`nid`);D($(`#new_panel `)),$(`#news_frame`).attr(`src`,`/news/`+e+`.html`)}var kn=class extends q{constructor(){super(),this.template=`
...
`}on_mount(){if($(`.login-content`).on(`click`,`.panel_item`,e=>this.LoginCommand(e)),$(`.role-list`).on(`click`,`.role-item`,function(){$(this).parent().find(`.select`).removeClass(`select`),$(this).addClass(`select`)}),$(`.new-list>li`).on(`click`,On),!x.GetUserCookie(`p`))return $(`#login_panel`).show();bn.showServers()}LoginCommand(e){switch($(e.currentTarget).attr(`command`)){case`ToRolePanel`:D($(`#role_panel`));break;case`ToServerPanel`:ge(),D($(`#slist_panel`));break;case`ToLogin`:D($(`#login_panel`));break;case`Forget`:D($(`#reset_panel`));break;case`CancleRegist`:D($(`#login_panel`));break;case`Down`:D($(`#download`));break;case`ToRegist`:D($(`#regist_panel`)),Sn.open();break;case`Regist`:Sn.regist();break;case`SelectServer`:bn.selectServer();break;case`LoginIn`:xn.loginIn();break;case`ResetPwd`:Cn.reset();break;case`AddRole`:X.addRole();break;case`SelectRole`:X.select();break;case`CreateRole`:En.create();break;case`BindPhone`:Tn.bind();break;case`CheckValid`:Tn.check();break;case`UpdatePwd`:wn.update();break;case`ToUpdate`:wn.open();break;case`ReLogin`:xn.relogin();break;case`DeleteRole`:X.delete();break}}},An=()=>$(`.content-message`),jn={append:e=>$(`.content-message`).append(e)},Z={itemsElement:null,contentScroll:!0,message:null,channel:null,relogin(){D(`#login_panel`)},clear:function(){Dialog.pack.items=null,Dialog.skills.items=null,Dialog.skills.skills=null,Dialog.skills.autoPfm=null,Dialog.skills.performSkills=null,Dialog.skills.sk_group=null,F.Skills=null,F.dis_pfms=[],this.state(null)},init:function(){Z.itemsElement=$(`.room_items`),this.message=$t.create($(`.content-message`)),this.ChannelElement=$(`.channel`),this.ChannelElement.on(`click`,Dialog.channel.show.bind(Dialog.channel)),this.channel=$t.create(this.ChannelElement,4,200)},startMoveMessage:function(e){window.addEventListener(`mousemove`,Z.moveMessage),window.addEventListener(`mouseup`,Z.endMoveMessage),Z.mouseY=e.clientY},moveMessage:function(e){let t=Z.mouseY-e.clientY,n=An(),r=n[0],i=n.height(),a=r.style.marginBottom;if(a=a?parseInt(a.replace(`px`,``)):0,a+=t,a<0)a=0;else if(a>i*.7)return;r.style.marginBottom=a+`px`,Z.mouseY=e.clientY,e.preventDefault()},endMoveMessage:function(){window.removeEventListener(`mousemove`,Z.moveMessage),window.removeEventListener(`mouseup`,Z.endMoveMessage)},regist:function(e){e.result&&(D(`#addrole_panel`),$(`#addrole_panel .input-error`).html(e.result))},emote:function(e){Z.emotes=e.items||0;for(var t=[],n=0;n<Z.emotes.length;n++)t.push(`<span>`),t.push(Z.emotes[n]),t.push(`</span>`);$(`.channel-emotes`).html(t.join(``))},deleterole:function(e){if(e.result){var t=$(`#role_panel>ul>.content>.role-list>.role-item[roleid='`+e.id+`']`);t.remove();var n=$(`#role_panel>ul>.content>.role-list>.role-item`);t.is(`.select`)&&n.length?$(n[0]).addClass(`select`):n.length||X.addRole()}else Confirm.Show({content:`<span class='input-error'>`+(e.message||`删除失败`)+`</span>`})},cross:function(e){for(var t=null,n=0;n<Y.length;n++)Y[n].ID==e.sid&&(t=Y[n]);t&&(C.ChangeServer=!0,C.Close(),Dialog.pack.items=null,e.cross_type==`duizhan`&&(Dialog.skills.items=null,Dialog.skills.isShow=!1),console.log(`重新连接到`,t.Name),e.pid||Z.die({relive:!0}),fe(t,e.pid))},roles:function(e){var t=e.roles;if(!t.length)X.addRole();else{D(`#role_panel`);for(var n=[],r=0;r<t.length;r++)n.push(`<li class='role-item`),r==0&&n.push(` select`),n.push(`' roleid='`+t[r].id+`'>`),n.push(t[r].title),n.push(`&nbsp;&nbsp;`),n.push(t[r].name),n.push(`</li>`);$(`.role-list`).html(n.join(``))}},loginerror:function(e){$(`.container`).hide(),$(`.login-content`).show(),O(`<strong>登陆失败：</strong>`+e.msg)},login:function(e){Z.player||D(`.container`),Z.player=e.id,Z.level=e.level,M.load(e.setting),me()},levelup:function(e){Z.level=e.level},selectItem:function(e){if($(e.target).is(`.status-item`)){var t=e.target.getAttribute(`sid`);let n=$(e.target).closest(`.room-item`).attr(`itemid`);return t?n===Z.player?w(`status `+t):w(`status `+t+` `+n):void 0}var n=$(this).attr(`itemid`);if(console.log(n),n){if(n==Z.player){w(`select `+n);return}w(`select `+n)}},countwidth:function(e,t){var n=e*100/t;return n<0&&(n=0),n>100&&(n=100),n},itemremove:function(e){var t=F.STATUS[e.id];if(t){for(var n in t.items)clearInterval(t.items[n].handler);var r=t.elem.parent();r.next().is(`.item-commands`)&&r.next().remove(),r.remove(),delete F.STATUS[e.id]}Z.cur_room.items.RemoveAt(t=>t.id===e.id)},itemadd:function(e){if(!(M.off_plist&&e.p&&e.id!=Z.player)){var t=e,n=M.item_firstme&&t.id==Z.player?$(Z.create_roomitem(t)).prependTo(Z.itemsElement):$(Z.create_roomitem(t)).appendTo(Z.itemsElement);F.STATUS[e.id]&&Z.itemremove(e),F.AppendStatusItem(t.id,n.find(`.item-status-bar`),t.status),Z.cur_room.items.push(t)}},items:function(e){Z.itemsElement.empty(),F.STATUS={};for(var t=0;t<e.items.length;t++){var n=e.items[t];if(n&&(n.player=n.p,n.m&&(n.type=`师父`,n.master=1),n.f&&(n.type=`随从`,n.follower=1),n.l&&(n.type=`商人`,n.trader=1),!(M.off_plist&&n.p&&n.id!=Z.player))){var r=M.item_firstme&&n.id==Z.player?$(Z.create_roomitem(n)).prependTo(Z.itemsElement):$(Z.create_roomitem(n)).appendTo(Z.itemsElement);F.AppendStatusItem(n.id,r.find(`.item-status-bar`),n.status)}}Z.cur_room||={},Z.cur_room.items=e.items},get_hpnum:function(e,t){var n=e/t;return n>.8?`<hiy>`+e+`</hiy>`:n>.5?`<yel>`+e+`</yel>`:n>.2?`<red>`+e+`</red>`:`<hir>`+e+`</hir>`},create_roomitem:function(e){var t=[];return t.push(`<div class='room-item' itemid='`+e.id+`'>`),e.max_hp&&(t.push(`<div class="item-status"`),(!F.IsShow||M.off_hp)&&t.push(` style="display:none;"`),t.push(`>`),t.push(`<div class="progress hp"><div class="progress-bar" max="`+e.max_hp+`"  style="width:`+Z.countwidth(e.hp,e.max_hp)+`%"></div></div>`),t.push(`<div class="progress mp"><div class="progress-bar" max="`+e.max_mp+`"   style="width:`+Z.countwidth(e.mp,e.max_mp)+`%"></div></div>`),t.push(`</div>`)),t.push(`<span class='item-status-bar'>`),t.push(`</span>`),t.push(`<span class='item-name'>`),t.push(e.name),M.show_hpnum&&e.max_hp&&t.push(`<span class="progress-num">[`+this.get_hpnum(e.hp,e.max_hp)+`<nor>/</nor><hiy>`+e.max_hp+`</hiy>]</span>`),t.push(`</span>`),t.push(`</div>`),t.join(``)},room:function(e){$(`.room_items`).html(``),$(`.room-name`).html(e.name),$(`.room_desc`).html(e.desc),Z.room_name=e.name,M.keep_msg?M.keep_msg&&Q(`你来到了`+e.name+`。`):Z.message.clear(),Z.room_path!=e.path&&(M.show_roomitem&&Z.searchItems(e),F.ShowRoomCommands(e),Z.room_path=e.path,Z.cur_room=e,A.SetRoom(e))},roomHiddenItemsReg:/<\w{3}\scmd=['"](.+?)['"]>(.+?)<\/\w{3}>/g,searchItems:function(e){for(var t=null,n=e.desc;(t=this.roomHiddenItemsReg.exec(n))!==null;)e.commands.push({cmd:t[1],name:t[2]})},exits:function(e){var t=e?e.items:Z.room_exits;if(t)if(Z.room_exits=t,A.SetExits(t),M.exits_dir==1){for(var n=[`这里明显的出口有：`],r=[],i=0;i<A.DIRS.length;i++)t[A.DIRS[i]]&&r.push(A.DIRS[i]);for(var i=0;i<r.length;i++)i>0&&n.push(i==r.length-1?` 和 `:`、`),n.push(`<span class='exits-item' dir='`+r[i]+`'>`+r[i]+`</span>`);r.length?$(`.room_exits`).html(n.join(``)):$(`.room_exits`).html(`<HIK>这里没有明显的出口。<HIK>`)}else $(`.room_exits`).html(A.CreateExitsMap(t,$(`.container`).width(),Z.room_name))},before_click_exits:function(e){var t=$(e.target);t.attr(`dir`)&&(t.is(`rect`)?t.attr(`fill`,`var(--theme-surface-2)`):t.is(`text`)&&t.prev().attr(`fill`,`var(--theme-surface-2)`))},click_exits:function(e){var t=$(e.target),n=t.attr(`dir`);n&&(t.is(`rect`)?t.attr(`fill`,`var(--theme-surface)`):t.is(`text`)&&t.prev().attr(`fill`,`var(--theme-surface)`),w(`go `+n))},query_rmitem:function(e){if(!(!this.cur_room||!this.cur_room.items)){for(let t of this.cur_room.items)if(t.id===e)return t}},isLivingRoomItem:function(e){return!!(e&&(e.me||e.p||e.m||e.f||e.l||e.hp>0||e.max_hp>0))},item:function(e){e.commands=e.commands??[];let t=Z.query_rmitem(e.id);t&&(e=Object.assign(e,t)),j.LAST_OBJ=e,Dialog.extend.append(e.commands,`item`,e);for(var n=[`<div class='item-commands'>`],r=0;r<e.commands.length;r++)n.push(`<span cmd='`+e.commands[r].cmd+`'>`),n.push(e.commands[r].name),n.push(`</span>`);if(n.push(`</div>`),Z.isLivingRoomItem(e)){Z.itemsElement.find(`.item-commands`).remove(),Dialog.item.open(e);return}if(Q(e.desc),M.show_command&&F.STATUS[e.id]){Z.itemsElement.find(`.item-commands`).remove();var i=F.STATUS[e.id].elem.parent();return $(n.join(``)).insertAfter(i),Z.message.scroll2end()}Q(n.join(``))},actions:function(e){F.ShowActions(e)},cmds:function(e){if(e.items){if(Dialog.curItem===`item`&&Dialog.item&&Dialog.item.isShow){if(Dialog.item.isCapturingInteraction&&Dialog.item.isCapturingInteraction()){Dialog.item.appendInteractionCommands(e.items);return}Dialog.item.appendCommands(e.items);return}var t=[`<div class='item-commands'>`];e.items.length||(e.items=[e.items]);for(var n=0;n<e.items.length;n++)t.push(`<span cmd='`+e.items[n].cmd+`'>`),t.push(e.items[n].name),t.push(`</span>`);t.push(`</div>`),Q(t.join(``))}},map:function(e){A.SetMapBuffer(e.map,e.path),A.ShowMap(e.map,e.path),A.OpenDialogAfterLoad&&(A.OpenDialogAfterLoad=!1,$(`.map-panel`).hide(),Dialog.show(`map`))},updatemap:function(e){A.UpdateMap(e.map,e)},path:function(e){e.message&&Q(e.message),e.action==`stop`&&A.StopAutoMove()},dialog:function(e){Dialog.show(e.dialog,e)},sc:function(e){F.StatusChanged(e)},perform:function(e){F.ShowPFM(e),Dialog.skills.updatePerformSkills(e.skills)},disobj:function(e){F.DisObj(e)},changepfm:function(e){F.ChangeDistime(e)},clearDistime:function(e){F.ClearDistime(e)},pay:function(e){if(e.pay===3){Q(`<yel>请打开微信扫描二维码支付：</yel>
`);let t=$(`<div style="width:100%;text-align:center;"><img style="border:solid 2px #808088" src="`+e.url+`"/></div>`);t.children(0).on(`load`,function(){Q(``)}),jn.append(t)}else window.location.href=e.url},dispfm:function(e){F.On_Perform(e)},status:function(e){F.StatusItemChanged(e)},combat:function(e){e.start&&(M.auto_showcombat==1&&!F.IsShow&&F.Show(),M.auto_hideroom==1&&(M.hide_roomdesc||$(`.room_desc`).hide())),e.end&&M.auto_hideroom==1&&(M.hide_roomdesc||$(`.room_desc`).show())},state:function(e){if(e&&e.state){var t=[`<span class='title'>`+e.state+`</span>`];if(e.commands)for(var n=0;n<e.commands.length;n++)t.push(`<span class='item-command' cmd='`+e.commands[n].cmd+`'>`),t.push(e.commands[n].name),t.push(`</span>`);$(`.state-bar`).html(t.join(``)).css(`visibility`,`visible`),e.no_stop?$(`.state-tool`).hide():$(`.state-tool`).show(),Z.states=e.desc,Z.timer&&clearInterval(Z.timer),Z.states&&Z.states.length&&(typeof Z.states==`string`&&(Z.states=[Z.states]),Z.timer=setInterval(Z.updatestate,e.interval||5e3))}else $(`.state-bar`).empty().css(`visibility`,`hidden`),$(`.state-tool`).hide(),clearInterval(Z.timer)},updatestate:function(){if(Z.states&&pe()){var e=Z.states.length;Q(Z.states[parseInt(Math.random()*e)])}},die:function(e){if(e.relive)return Z.state({});Z.state({state:`<hiw>你已经死亡：</hiw>`,no_stop:!0,desc:[`<blk>一股阴冷的气息包围着你。</blk>`,`<blu>朦胧中你好像听到有人在喊：过来吧，过来吧！</blu>`],commands:e.commands,interval:12e3})},warn:function(e){Warn.Show(e)},msg:function(e){var t=Dialog.channel.createElement(e,!M.no_spmsg);t&&(M.no_spmsg?Q(t):(Z.channel.push(t),Z.channel.scroll2end()))},addAction:function(e){F.AddObj(e.id,e.name,e.distime)},removeAction:function(e){F.DisObj({id:e.id,remove:!0})}};function Q(e){Z.message.push(e),Z.message.scroll2end()}var Mn=!1,Nn=[`fight`,`kill`,`perform`,`biwu`];function Pn(e){if(!e)return!1;var t=e.trim().split(/\s+/,1)[0];return Nn.indexOf(t)>=0}function Fn(){K.isShow&&K.hide(),k.Close()}var In=class extends q{constructor(){super(),this.template=`
...
`}on_mount(){$(`.container`).on(`click`,Kn),$(`.channel-box`).on(`click`,`span`,Wn),$(`.combat-commands`).on(`click`,`.pfm-item`,F.Perform).on(`wheel`,F.Scroll),$(`.room-commands`).on(`wheel`,F.Scroll),$(`.sender-box`).on(`keyup`,Hn),$(`.room_items`).on(`click`,`.room-item`,Z.selectItem),$(`.bottom-bar`).on(`click`,`.tool-item,.state-bar,.item-command`,zn),$(`.map-panel`).on(`click`,Rn),$(`.sender-btn`).on(`click`,Un),$(`.room_exits`).on(`pointerdown`,Z.before_click_exits).on(`pointerup`,Z.click_exits),$(`.room-title>.map-icon`).on(`click`,A.OpenDialog.bind(A))}},Ln=0;function Rn(e){if(!$(e.currentTarget).closest(`.dialog-map`).length&&!A.SuppressRoomClick){if(Ln||=0,Date.now()-Ln>500){Ln=Date.now();return}A.OpenDialog()}}function zn(e){var t=$(this).attr(`command`);return t?Bn(t):(t=$(this).attr(`cmd`),t&&SendCommand(t),!1)}function Bn(e){switch(e){case`showtool`:yt();break;case`showchat`:return Vn();case`showcombat`:return F.Show();case`stopstate`:if(K.extend.is_record)return K.extend.stop_record();SendCommand(`state stop`);break;case`stateinfo`:SendCommand(`state info`);break;default:K.show(e);break}return!1}function Vn(){var e=$(`.chat-panel`).toggleClass(`hide`);return e.is(`.hide`)?Mn=!1:(Mn=!0,e.find(`input`).val(``).focus()),!1}function Hn(e){e.keyCode==13&&Un()}function Un(){var e=$(`.sender-box`).val();if(e){if(e.length>100)return ReceiveMessage(`<hir>你输入的内容太多了。</hir>`);var t=$(`.channel-box`).attr(`channel`);$(`.sender-box`).val(``).focus(),SendCommand(t+` `+e)}}function Wn(){var e=$(this),t=e.attr(`channel`);if(t==`emote`)return Gn();if(!e.is(`.selected`)){var n=e.parent();return n.children().removeClass(`selected`),e.addClass(`selected`),n.attr(`channel`,t),$(`.sender-box`).focus(),!1}}function Gn(){var e=$(`.channel-emotes`);e.is(`.hide`)?(e.removeClass(`hide`),Z.emtoes||(SendCommand(`emote`),Z.emtoes=[],$(`.sender-box`).blur(),e.on(`click`,`span`,function(){var e=$(this).html();$(`.sender-box`).val(`*`+e).focus(),$(`.channel-emotes`).addClass(`hide`)}))):$(`.channel-emotes`).addClass(`hide`)}function Kn(e){var t=$(e.target);if(K.isShow&&t.is(`.container.dialog-open`))return K.hide(),k.Close(),!1;var n=t.attr(`cmd`);if(n||=t.parent().attr(`cmd`),n){let e=n[0];if(e==`_`){var r=n.split(` `);switch(r[0]){case`_confirm`:k.Process(r);break;case`_setting`:M.save(r[1],r[2]);break;case`_trade`:K.trade.confirm(r[1]);break;case`_close`:ye.Close(t);break;case`_hide`:break;case`_closed`:K.hide();case`_party`:K.party.command(r[1]);break}}else e===`#`?j.run(n):(Pn(n)?Fn():K.curItem===`item`&&K.item&&K.item.isShow&&t.closest(`.dialog-item-dialog`).length>0&&K.item.captureNextPrompt(),SendCommand(n),!t.closest(`.dialog-fb`).length&&!t.closest(`.dialog-item-subdialog`).length&&t.closest(`.dialog-content`).length>0&&t.closest(`.item-commands`).remove());return!1}else Mn&&(t.closest(`.chat-panel`).length||($(`.chat-panel`).addClass(`hide`),Mn=!1));k.Close()}window.HandlerMenuCommand=Bn;var qn=new kn,Jn=new In,Yn=class extends q{constructor(){super(),this.template=`${qn.render()}\n${Jn.render()}`}on_mount(){qn.on_mount(),Jn.on_mount()}};globalThis.$=e,globalThis.SendCommand=w,globalThis.ReceiveMessage=T,globalThis.Confirm=k,globalThis.Warn=ye,globalThis.Process=Z,globalThis.Dialog=K;function Xn(){let e=new Yn;e.onCompile(),e.mount(document.body),Z.init()}e(Xn);