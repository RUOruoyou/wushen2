...
this.run = function () {
    ...
    this.send_weapons();
    this.send_family();
    COMMAND.DO("sys", "排行榜奖励发放完毕，请进入社交系统邮箱领取。");
...
this.send_score = function () {
    ...
    for (let key of FAMS_TATAS) {
        ...
        for (var i = 0; i < stats.length; i++) {
            ...
            if (i > 19) break;
            if (!user.id) continue;
            let online_user = WORLD.getUser(user.id);
            if (online_user && WORLD.is_admin && WORLD.is_admin(online_user)) continue;
...
this.send_tops = function () {
    for (let key of FAMS_TATAS) {
        ...
        for (let i = 0; i < tops.length; i++) {
            ...
            let online_user = WORLD.getUser(user.userid);
            if (online_user && WORLD.is_admin && WORLD.is_admin(online_user)) continue;
...
this.send_weapons = function () {
    ...
    for (let stats of eqs) {
        for (let i = 0; i < stats.length; i++) {
            ...
            let online_user = WORLD.getUser(item.user);
            if (online_user && WORLD.is_admin && WORLD.is_admin(online_user)) continue;
...
this.sort_family = function (fam) {
    ...
    if (fam.tops) {
        ...
        for (var key in fam.tops) {
            var top_user = WORLD.getUser(key);
            if (top_user && WORLD.is_admin && WORLD.is_admin(top_user)) continue;
    ...
    var online_user = WORLD.getUser(user.id);
    if (online_user && WORLD.is_admin && WORLD.is_admin(online_user)) return;
