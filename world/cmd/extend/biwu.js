...
this.regex = /^(emei|wudang|shaolin|huashan|xiaoyao|quanzhen|gaibang|shashou|xuedao|yihua|none)?\s*(\d+)(?:\s+(ok))?$/;
...
this.checkStats = function (player) {
    if (!STATS.isRankedPlayer(player)) {
        STATS.removePlayerStats(player);
        return;
    }
