...
this.on_user_login = function (user) {
    ...
    if (user.apply_month_card) {
        user.apply_month_card();
    }
    if (user.send_month_card_daily_mail) {
        user.send_month_card_daily_mail();
    }
    if (WORLD.MESSAGE && WORLD.MESSAGE.getUnreadCount) {
        user.send(JSON.stringify({
            type: "dialog",
            dialog: "message",
            unRead: WORLD.MESSAGE.getUnreadCount(user)
        }));
    }
