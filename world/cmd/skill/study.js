...
this.study_from_books = function (me, bindex) {
    ...
    var target = { color_name: skill.query_color_name(me), max_level: 100, };
    this.study_skill(me, skill, target);
}
...
this.study_skill = function (me, skill, target) {
    ...
    if (me.skills[skill.id + "2"]) return me.notify("你已经学会了的" + skill.query_color_name(me) + "的进阶武功，不用再学习了。");
    ...
    if (me.query_prop(skill.id)) return me.notify("你不能装备增加" + skill.query_color_name(me) + "等级的道具读书。");
    ...
    me.set_state({
        ...
        on_enter: do_learn,
        no_move: "读书要专心，不要乱跑。",
        desc: "[\"你对" + skill.query_color_name(me) + "似乎有些心得。\"]",
