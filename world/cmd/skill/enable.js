...
this.enter = function (me, base, skill, no_sent) {
    ...
    if (skill && skill !== "none") {
        ...
        sp_skill[base] = true;
        me.notify("你决定使用" + sp_skill_base.query_color_name(me) + "做为" + baseskill_base.query_color_name(me) + "的特殊技能。");
        if (me.is_player)
        ...
        baseskill.enable_skill = null;
        me.notify("你决定取消使用" + old_skill.query_color_name(me) + "做为" + baseskill_base.query_color_name(me) + "的特殊技能。");
        if (me.is_player)
            me.notify('{type:"dialog",dialog:"skills",id:"' + base + '",enable:false}');
        else
