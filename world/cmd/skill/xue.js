...
this.enter = function (me, par, sk, target) {
    ...
    if (target.is_player) {
        if (!me.master ||
            ...
            return me.notify(me.name + "不能向" + target.name + "请教" + skill_base.query_color_name(me) + "！");
        }
    } else {
        if (target.on_teach) {
    ...
    if (BAN_SKILLS[sk]) return me.send(skill_base.query_color_name(me) + '不能通过学习升级。');
    ...
    if (me.skills[sk + "2"]) return me.notify("你已经学会了的" + skill_base.query_color_name(me) + "的进阶武功，不用再学习了。");
    ...
    var max = skill_base.type === SKILL_TYPES.KNOWLEDGE ? 0 : me.query_skill_limit(sk);
    if (par) {
        max = parseInt(par);
    ...
    me.notify("<hic>你开始向" + target.name + "请教有关「" + skill_base.query_color_name(me) + "」的疑问。</hic>");
    me.set_state({
        id: "learn",
        title: "学习" + skill_base.query_color_name(me),
        target: target,
        max_level: max,
...
function on_check(me) {
    ...
    let target_lv = Math.min(me.query_skill_limit(skill.id), this.target.query_skill(skill.id, 0));
...
function to_next(me) {
    ...
    if (state.queues.length > 0) {
        ...
        me.notify("<hic>你开始向" + state.target.name
            + "请教有关「" + skill_base.query_color_name(me) + "」的疑问。</hic>");
...
function check_skill(me, skill, master, limit = true) {
    ...
    if (limit && me.state && me.state.max_level && lv >= me.state.max_level) {
        return me.notify_fail("你的" + skill.query_color_name(me) + "学习完毕。");
    }
    if (skill.type != SKILL_TYPES.KNOWLEDGE) {
...
function do_learn(me) {
    ...
    if (exp > me.pot && WORLD.is_admin(me)) {
        exp = me.pot;
    }
    if (exp > me.pot)
...
function count_speed(me) {
    ...
    return WORLD.apply_admin_test_multiplier(me, "study", pot);
}
