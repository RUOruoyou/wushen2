...
this.enter = function (me, oid) {
    if (oid) {
        ...
        if (orglevel > 0) {
            let item_path = obj.grade == 6 ? "st/yuanjing" : "st/xuanjing";
            let count = obj.grade == 6 ?
                (1 + orglevel) * orglevel / 2 :
                (Math.pow(2, orglevel + 1) - 2) * obj.grade;
            var item = me.add_obj(item_path, count);
        ...
        me.notify("铁匠说道：取消精炼后装备的等级变为初始等级，退回全部精炼材料，不论精炼等级统一收费。");
        me.notify(str.join(""));
    }
}
