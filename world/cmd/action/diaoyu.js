...
function on_check(me) {
    var exp = WORLD.DATA.exps[me.level]
        ...
        + me.query_prop('diaoyu1');

    var pot = exp + me.query_prop('lsj_qn') + me.query_prop('diaoyu1');
    let er = this.yuer;
...
function do_diaoyu(me) {
    ...
    let er = this.yuer;
    // 钓鱼竿效率越高，越容易钓到稀有的鱼；效率直接加到经验和潜能
    var dy = me.query_prop('diaoyu1');
    var base = Math.min(Math.floor(dy / 5) * 3, 15);
    let lv = base + me.random(18 - base);
    var obj = me.add_obj('res/yu#' + lv);
    if (obj)
    ...
    var exp = WORLD.DATA.get_exp(me)
        + WORLD.DATA.query_temp("diaoyu_exp", 0)
        + dy;
    var pot = exp + me.query_prop('lsj_qn') + dy;
