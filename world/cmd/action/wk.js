...
function on_check(me) {
    const reward = query_reward(me, false);
    var exp = reward.exp;
    var pot = reward.pot;
    me.send(`你正在挖矿，每10秒获得${exp}经验，${pot}潜能，有概率挖到<hig>玄晶</hig>和其他宝石。`);
}
function do_diaoyu(me) {
    // 铁镐挖矿效率越高，越容易挖到玄晶和高档宝石
    var kuang = me.query_prop('kuang1');
    var base = Math.floor(kuang / 10);
    var idx;
    if (base >= 3 && me.random(100) < 40) {
        idx = 0;                  // 顶级工具：玄晶保底
    } else {
        idx = base + me.random(9 - base);  // 抬高下限，少出碎裂宝石
    }
    let obj = me.add_obj(["st/xuanjing", "st/st_red#0", "st/st_blu#0", "st/st_gre#0", "st/st_yel#0"
        , "st/st_red#1", "st/st_blu#1", "st/st_gre#1", "st/st_yel#1"][idx]);
    if (obj) {
        me.notify("<hig>恭喜你得到一颗" + obj.color_name + "。</hig>");
    }


    const reward = query_reward(me, true);
    me.add_exp(reward.exp, reward.pot, 0);

}

function query_reward(me, randomize) {
    // 铁镐挖矿效率直接加到经验和潜能
    var kuang = me.query_prop('kuang1');
    var exp = (randomize ? WORLD.DATA.get_exp(me) : WORLD.DATA.exps[me.level])
        + WORLD.DATA.query_temp("kuang_exp", 0)
        + kuang;
    var pot = exp + me.query_prop('gsj_qn') + kuang;
    var multiplier = WORLD.admin_test_multiplier(me, "mining");
    if (multiplier > 1) {
        exp = parseInt(exp * multiplier);
        pot = parseInt(pot * multiplier);
    }
    return { exp, pot };
}
