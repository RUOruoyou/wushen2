...
function do_cai(me) {
    // 药王神篇效率越高，越容易采到稀有的药草；效率直接加到经验和潜能
    var cy = me.query_prop('caiyao1');
    var base = Math.min(Math.floor(cy / 5) * 3, 15);
    ...
    let exp = WORLD.DATA.get_exp(me)
        + WORLD.DATA.query_temp("caiyao_exp", 0)
        + cy;
    let pot = exp + me.query_prop('ly_qn') + cy;
