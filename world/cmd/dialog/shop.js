...
this.enter = function (me, par, count) {
    ...
    if (count != undefined) {
        if (String(par || "").indexOf("family_") === 0) {
            const familyShop = WORLD.COMMANDS.family_shop;
            if (!familyShop || !familyShop.buy) return me.notify("功绩商城暂时无法兑换。");
            return familyShop.buy(me, par, count);
        }
    ...
    return this.show(me, par === "merit" ? 2 : 0);
}

this.show = function (me, selected) {
    if (!this.list) this.load_selllist();
    const familyShop = WORLD.COMMANDS.family_shop;
    const selllist = [];
    for (const group of this.groups) {
        ...
        for (const item of group) {
            ...
            const discount = item.query_discount ? item.query_discount(me) : (item.discount ?? 1);
            const data = [item.id, item.name, item.desc, item.value, item.grade, discount];
            if (item.limit > 0) data.push(item.limit, me.query_temp(item.limit_key, 0));
            else if (item.max > 0) data.push(item.max, me.query_temp(item.max_key, 0));
            items.push(data);
        }
        selllist.push(items);
    }
    selllist.push(familyShop && familyShop.queryItems ? familyShop.queryItems(me) : []);
    me.send(JSON.stringify({
        ...
        selllist: selllist,
        money: [me.money, me.cash_money, me.query_temp("gongji", 0)],
        mtype: "点<hiy>师门功绩</hiy>",
        mname: "功绩",
        idx: this.idx
    }));
    return true;
...
function format_moneys(me) {
    return `money:[${me.money},${me.cash_money},${me.query_temp("gongji", 0)}],mtype:"点<hiy>师门功绩</hiy>",mname:"功绩"`;
}
...
this.groups = [
    [
        ...
        {
            name: "<hiy>归墟种</hiy>", unit: "枚", grade: 3,
            path: "cash/guixuzhong", desc: "开启江湖-秘境中的断剑冢，进入一次消耗一枚。", value: 1000
        },
        {
