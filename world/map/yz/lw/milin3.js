...
this.is_time = function () {
    const parts = LUNAR_DATE_FORMATTER.formatToParts(new Date());
    const lunar = {};
    for (const part of parts) {
        if (part.type !== "literal") lunar[part.type] = parseInt(part.value);
    }
    return lunar.month === 8 && lunar.day >= 15 && lunar.day <= 17
        && lunar.hour >= 21 && lunar.hour <= 23;
}
