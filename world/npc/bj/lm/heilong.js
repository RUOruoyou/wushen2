...
this.add_action('free', '放了它', function (me) {
    if (this.is_free) return me.send(this.name + '已经挣脱锁链。');
    if (this.fight_type) return me.notify_fail(this.name + '正在暴怒挣扎，无法靠近锁链。');
    if (me.mp < 500) return me.notify_fail("你试着拉扯玄铁锁链，内力不足，石柱纹丝不动。");

    me.add_mp(-500);
    me.send(this.name + '低声道：若你真有胆量，就将内力灌入石柱灵纹。');
    this.on_free(me);
    return true;
...
this.on_free = function (me) {
    ...
    if (!me.query_temp("fb/longmai/free")) {
        me.set_temp("fb/longmai/free", 1);
        me.add_fbscore(100);
    }
