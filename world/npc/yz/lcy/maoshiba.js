...
this.on_die = function (killer) {
    var shi = this.environment.find_by_path("yz/lcy/shisong");
    if (!shi || !killer) return;
    var player = killer.is_player ? killer : killer.master ? WORLD.getUser(killer.master) : null;
    if (!player || !player.is_player) return;
    shi.send_room("$N收起软鞭，对$n说道：反贼已除，这份功劳也有你一份。", player);
    player.add_fbscore(45, 100);
}
