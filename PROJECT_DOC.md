...
武道家族由 `world/extends/household/index.js` 提供 `HOUSEHOLD` 服务，数据写入角色存档顶层 `household`，不塞入 `temp`，也不复制随从技能、装备和背包。`HOUSEHOLD.ensure()` 将旧角色的 `temp.home=1/2` 映射为住宅等级，补齐设施、成员经营属性和幂等启动资源；没有住宅的角色可以查看面板但不能派工。

家族任务由统一时间戳结算，在登录、面板/家族命令、退出保存和在线心跳时调用 `HOUSEHOLD.tick()`，离线结算最多追溯 72 小时。无人值守任务保留低效率基础产出，成员岗位影响效率和职业成长；产出先进入任务待领取区，仓库容量不足时不会丢失。成员可以停止岗位、休息恢复疲劳和心情、培训职业经验，家族授课会增加家族资金与声望。

经营闭环还包括可逆事件分支（继续、投入、暂停）、自动生成的接单/交付订单、成就进度与一次性奖励、独立维护费支付、家族仓库产出兑换，以及练功房为玩家提供的有限潜能训练。家族丹药等可映射到真实物品的产出通过 `HOUSEHOLD.withdraw/useProduct` 校验背包容量后领取；无法安全映射的凭证留在仓库或兑换为家族资金。住宅经营房间 `home/kuangchang`、`home/yaopu`、`home/wuguan`、`home/xuetang`、`home/gongfang` 只负责环境和入口，业务仍统一由 `HOUSEHOLD` 处理。

随从仍由 `FOLLOWER.STORES/SAVE` 保存战斗属性、装备和背包，但服务端已在对象、组队、战斗、自动击杀和移动入口阻止家族成员进入战斗；关系面板中的旧工作入口转换为家族派工。家族系统的经营数据和玩家战斗成长保持分离。
